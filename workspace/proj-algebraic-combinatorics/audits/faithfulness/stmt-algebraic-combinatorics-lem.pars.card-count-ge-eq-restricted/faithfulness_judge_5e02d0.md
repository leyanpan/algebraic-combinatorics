## TARGET Nat.Partition.card_count_ge_eq_restricted (theorem) — ELABORATED SIGNATURE
∀ (I : Set ℕ) [inst : DecidablePred fun x => x ∈ I],
  (∀ i ∈ I, i > 0) →
    ∀ (n d j : ℕ),
      d ∈ I →
        d * j ≤ n →
          {p ∈ Nat.Partition.restricted n fun x => x ∈ I | Multiset.count d p.parts ≥ j}.card =
            (Nat.Partition.restricted (n - d * j) fun x => x ∈ I).card

Docstring: Partitions with count(d, p) ≥ j biject with partitions of n - d*j.
...
## INFORMAL STATEMENT
lem.pars.card-count-ge-eq-restricted

\leanhelper  Let $I \subseteq \{ 1, 2, 3, \ldots \} $, let $d \in I$, and let $d \cdot j \leq n$. Then 

\[  \# \{ p \in \operatorname {restricted}(n, I) : \operatorname {count}(d, p) \geq j\}  = \# \operatorname {restricted}(n - d \cdot j, I).  \]
...
{
  "verdict": "faithful",
  "justification": "The formal binders exactly encode the blueprint hypotheses: `\u2200 (I : Set \u2115) ... (\u2200 i \u2208 I, i > 0)` expresses `I \u2286 {1,2,3,\u2026}`, while `\u2200 (n d j : \u2115), d \u2208 I \u2192 d * j \u2264 n \u2192` matches \u201clet `d \u2208 I`, and let `d \u00b7 j \u2264 n`.\u201d The conclusion filters `Nat.Partition.restricted n (fun x => x \u2208 I)` by `Multiset.count d p.parts \u2265 j`, takes its `Finset.card`, and equates it with the card of `restricted (n - d * j) I`, exactly matching the stated cardinality identity. The `[DecidablePred fun x => x \u2208 I]` binder is only computational infrastructure required by `Nat.Partition.restricted` and `Finset.filter`, not an additional mathematical hypothesis."
}