## TARGET AlgebraicCombinatorics.FPS.summableFPS_fubini (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {ι : Type u_2} {κ : Type u_3} {f : ι × κ → PowerSeries R},
  AlgebraicCombinatorics.FPS.SummableFPS f →
    (∀ (i : ι), AlgebraicCombinatorics.FPS.SummableFPS fun j => f (i, j)) ∧
      ∀ (j : κ), AlgebraicCombinatorics.FPS.SummableFPS fun i => f (i, j)

Docstring: The Fubini rule for summable FPS families: interchange of summation is valid
...
## INFORMAL STATEMENT
prop.fps.summable-sums-rule

Sums of summable families of FPSs satisfy the usual rules for sums (such as the breaking-apart rule $\sum _{i\in S}a_{s}=\sum _{i\in X}a_{s}+\sum _{i\in Y}a_{s}$ when a set $S$ is the union of two disjoint sets $X$ and $Y$). See \cite[Proposition 7.2.11]{19s} for details. Again, the only \textbf{caveat} is about interchange of summation signs: The equality 

\[  \sum _{i\in I}\  \  \sum _{j\in J}\mathbf{a}_{i,j}=\sum _{j\in J}\  \  \sum _{i\in I}\mathbf{a}_{i,j}  \]

 holds when the family $\left(\mathbf{a}_{i,j}\right)_{\left(i,j\right) \in I\times J}$ is summable (i.e., when for each $n\in \mathbb {N}$, all but finitely many \textbf{pairs} $\left(i,j\right) \in I\times J$ satisfy $\left[x^{n}\right]\mathbf{a}_{i,j}=0$); it does not generally hold if we merely assume that the sums $\sum _{i\in I}\  \  \sum _{j\in J}\mathbf{a}_{i,j}$ and $\sum _{j\in J}\  \  \sum _{i\in I}\mathbf{a}_{i,j}$ are summable.
...
{
  "verdict": "unstatable",
  "justification": "The blueprint asserts the equality `\u2211 i, \u2211 j, a_{i,j} = \u2211 j, \u2211 i, a_{i,j}` when the product-indexed family is summable. The target concludes only `(\u2200 i, SummableFPS (fun j => f (i,j))) \u2227 \u2200 j, SummableFPS (fun i => f (i,j))`; it contains neither iterated sums nor their equality, and it also does not cover the stated general rules for sums. Moreover, the only project definition supplied, `SummableFPS`, is merely the predicate `\u2200 n, {i | coeff n (f i) \u2260 0}.Finite`; it defines no FPS-valued sum operation. Thus the blueprint\u2019s equality cannot be expressed with the available declarations. To make it statable and faithful, a dependency defining the sum of a `SummableFPS` family with the blueprint coefficient rule must be added, after which the target should assert equality of the two iterated sums (and, if formalizing the proposition in full, the other usual sum rules), rather than only rowwise and columnwise summability."
}