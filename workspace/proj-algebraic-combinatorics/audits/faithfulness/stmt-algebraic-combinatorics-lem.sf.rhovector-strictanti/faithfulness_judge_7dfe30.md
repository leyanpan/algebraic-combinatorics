## TARGET rhoVector_strictAnti (theorem) — ELABORATED SIGNATURE
∀ (N : ℕ), StrictAnti (rhoVector N)

Docstring: The ρ vector is strictly decreasing 
...
## BASE-LIBRARY REF StrictAnti
{α : Type u} → {β : Type v} → [Preorder α] → [Preorder β] → (α → β) → Prop

Docstring: A function `f` is strictly antitone if `a < b` implies `f b < f a`. 
...
## INFORMAL STATEMENT
lem.sf.rhoVector-strictAnti

\leanhelper  The $\rho $ vector is strictly decreasing: if $i < j$ (as indices in $\{ 0, 1, \ldots , N{-}1\} $), then $\rho _i > \rho _j$.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over every size, `\u2200 (N : \u2115), StrictAnti (rhoVector N)`. By the supplied definition of `StrictAnti`, this says that for all `i j : Fin N`, `i < j` implies `rhoVector N j < rhoVector N i`, exactly the informal claim \u201cif `i < j` (as indices in `{0, 1, \u2026, N\u22121}`), then `\u03c1_i > \u03c1_j`.\u201d The index type `Fin N` is precisely the naturals below `N`, and `rhoVector N i` unfolds through `AlgebraicCombinatorics.rho` to `N - 1 - \u2191i`, representing the stated tuple `(N\u22121, N\u22122, \u2026, 0)` under 0-based indexing. There are no added hypotheses or restricted quantifiers."
}