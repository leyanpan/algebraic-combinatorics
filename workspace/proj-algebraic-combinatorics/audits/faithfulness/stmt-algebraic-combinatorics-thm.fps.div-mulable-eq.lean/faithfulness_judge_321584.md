## TARGET PowerSeries.tprod_div_eq_div_tprod (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_2} {a b : I → PowerSeries R} (ha : PowerSeries.Multipliable a)
  (hb : PowerSeries.Multipliable b) (hb_inv : ∀ (i : I), IsUnit ((PowerSeries.coeff 0) (b i)))
  (hab : optParam (PowerSeries.Multipliable fun i => a i * Ring.inverse (b i)) ⋯),
  PowerSeries.tprod (fun i => a i * Ring.inverse (b i)) hab =
    PowerSeries.tprod a ha * Ring.inverse (PowerSeries.tprod b hb)

Docstring: The product of `(a_i / b_i)` equals the quotient of products.
...
## PROJECT DEPENDENCY PowerSeries.multipliable_div (theorem)
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_2} {a b : I → PowerSeries R},
  PowerSeries.Multipliable a →
    PowerSeries.Multipliable b →
      (∀ (i : I), IsUnit ((PowerSeries.coeff 0) (b i))) → PowerSeries.Multipliable fun i => a i * Ring.inverse (b i)

Docstring: If `(a_i)` and `(b_i)` are multipliable and each `b_i` is invertible,
...
## BASE-LIBRARY REF Ring.inverse
{M₀ : Type u_2} → [MonoidWithZero M₀] → M₀ → M₀

Docstring: Introduce a function `inverse` on a monoid with zero `M₀`, which sends `x` to `x⁻¹` if `x` is
...
## INFORMAL STATEMENT
thm.fps.div-mulable-eq.lean

\leanhelper  Under the hypotheses of Theorem~ \ref{thm.fps.div-mulable.lean}, 
...
{
  "verdict": "faithful",
  "justification": "The target matches the blueprint equation: `PowerSeries.tprod (fun i => a i * Ring.inverse (b i)) hab = PowerSeries.tprod a ha * Ring.inverse (PowerSeries.tprod b hb)` is exactly \u201cthe product of a\u1d62/b\u1d62 equals (the product of a\u1d62)/(the product of b\u1d62),\u201d using the informal definition `b/a = b \u00b7 a\u207b\u00b9`. The referenced hypotheses are represented by `ha : PowerSeries.Multipliable a`, `hb : PowerSeries.Multipliable b`, and `hb_inv : \u2200 i, IsUnit ((PowerSeries.coeff 0) (b i))`. For a formal power series over a commutative ring, invertibility is equivalent to invertibility of its constant coefficient, so `hb_inv` expresses that every `b i` is invertible. It also ensures the finitely determined constant coefficient of `PowerSeries.tprod b hb` is a finite product of units, hence the denominator product is invertible and `Ring.inverse` has the intended fractional meaning. The additional binder `hab : optParam (PowerSeries.Multipliable fun i => a i * Ring.inverse (b i)) \u22ef` is not a substantive added hypothesis: its default is supplied by `PowerSeries.multipliable_div` from `ha`, `hb`, and `hb_inv`, and it is only the proof argument required by the definition of `PowerSeries.tprod`."
}