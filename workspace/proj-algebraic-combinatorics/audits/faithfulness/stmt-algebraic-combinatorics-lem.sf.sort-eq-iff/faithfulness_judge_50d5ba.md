## TARGET AlgebraicCombinatorics.SymmetricFunctions.sortTuple_eq_iff (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} (a b : Fin N → ℕ),
  AlgebraicCombinatorics.SymmetricFunctions.sortTuple a = AlgebraicCombinatorics.SymmetricFunctions.sortTuple b ↔
    Multiset.map a Finset.univ.val = Multiset.map b Finset.univ.val

Docstring: Two tuples have the same sort if and only if one is a permutation of the other.
...
\leanhelper  Two tuples $a, b \in \mathbb {N}^N$ have the same sort if and only if they have the same multiset of values (equivalently, $b$ is a permutation of $a$): 
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over exactly two natural-valued N-tuples, `\u2200 {N : \u2115} (a b : Fin N \u2192 \u2115)`, matching `a, b \u2208 \u2115^N`. Its left side, `sortTuple a = sortTuple b`, uses `sortTuple`, whose body sorts `Multiset.map a Finset.univ.val` in weakly decreasing order and packages the result as an N-partition, matching `sort(a) = sort(b)`. Its right side, `Multiset.map a Finset.univ.val = Multiset.map b Finset.univ.val`, is precisely equality of the multisets of all values of the two tuples. There are no additional hypotheses or restricted quantifiers."
}