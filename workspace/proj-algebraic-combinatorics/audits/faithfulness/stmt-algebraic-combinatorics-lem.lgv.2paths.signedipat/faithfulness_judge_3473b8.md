## TARGET LGV1.ipatInvolution_isIntersecting (theorem) — ELABORATED SIGNATURE
∀ {A A' B B' : LGV1.LatticePoint} (spt : LGV1.SignedPathTuple2 A A' B B') (h : spt.isIntersecting),
  (LGV1.ipatInvolution spt h).isIntersecting

Docstring: The involution preserves being intersecting. 

## TARGET LGV1.ipatInvolution (def) — ELABORATED SIGNATURE
...
## TARGET LGV1.ipatInvolution_involutive (theorem) — ELABORATED SIGNATURE
∀ {A A' B B' : LGV1.LatticePoint} (spt : LGV1.SignedPathTuple2 A A' B B') (h : spt.isIntersecting),
  have spt' := LGV1.ipatInvolution spt h;
  ∃ (h' : spt'.isIntersecting), LGV1.ipatInvolution spt' h' = spt

Docstring: The involution is its own inverse.
...
## TARGET LGV1.ipatInvolution_sign (theorem) — ELABORATED SIGNATURE
∀ {A A' B B' : LGV1.LatticePoint} (spt : LGV1.SignedPathTuple2 A A' B B') (h : spt.isIntersecting),
  (LGV1.ipatInvolution spt h).sign = -spt.sign

Docstring: The involution is sign-reversing. 

## TARGET LGV1.splitPathAt (def) — ELABORATED SIGNATURE
(path : LGV1.LatticePath) →
  (start v : LGV1.LatticePoint) → v ∈ path.vertices start → LGV1.LatticePath × LGV1.LatticePath

Body:
fun path start v _hv =>
  have verts := path.vertices start;
  have idx := List.findIdx (fun x => x == v) verts;
  (List.take idx path, List.drop idx path)

Docstring: Split a path at a vertex, returning (head, tail) where head ends at v and tail starts at v. 
...
## PROJECT DEPENDENCY LGV1.SignedPathTuple2.sign (def)
{A A' B B' : LGV1.LatticePoint} → LGV1.SignedPathTuple2 A A' B B' → ℤ

Body:
fun {A A' B B'} spt => if spt.toBB' = true then 1 else -1

Docstring: The sign of a signed path tuple. 
...
## INFORMAL STATEMENT
lem.lgv.2paths.signedipat

\leanhelper  The sign-reversing involution on intersecting path tuples for $k = 2$. 

Given an intersecting path tuple $(p, p')$ in the set $\mathcal{X}$ of ipats from $(A, A')$ to $(B, B')$ or to $(B', B)$: 

\begin{enumerate} \item Since $(p, p') \in \mathcal{X}$, the paths $p$ and $p'$ have a vertex in common. Let $v$ be the first one. (The first one on $p$ or the first one on $p'$? These are the same, since our digraph is acyclic.) We call this vertex $v$ the \emph{first intersection} of $(p, p')$. 

\item Split each path at $v$ into a head (before $v$) and a tail (after $v$). 

\item Exchange the tails: $q = (\text{head of } p) \cup (\text{tail of } p')$ and $q' = (\text{head of } p') \cup (\text{tail of } p)$. 
...
{
  "verdict": "faithful",
  "justification": "The formal declarations collectively imply the blueprint claim. The domain is exactly the intersecting signed path tuples: `\u2200 {A A' B B'} (spt : SignedPathTuple2 A A' B B') (h : spt.isIntersecting)`, where `isIntersecting` is `\u2203 v \u2208 spt.path0.vertices A, v \u2208 spt.path1.vertices A'`, matching \u201cthe paths p and p' have a vertex in common.\u201d The body of `ipatInvolution` chooses `firstIntersection spt h`, splits both step lists with `List.take`/`List.drop` at its first occurrence, forms `head0 ++ tail1` and `head1 ++ tail0`, and flips `toBB'`, exactly implementing the prescribed tail exchange and corresponding sign swap. Closure on the set X is supplied by `ipatInvolution_isIntersecting`: `(ipatInvolution spt h).isIntersecting`. The involution property is supplied by `ipatInvolution_involutive`: `\u2203 (h' : spt'.isIntersecting), ipatInvolution spt' h' = spt`; proof irrelevance makes the proof argument immaterial, so this yields the required self-inverse map on intersecting tuples. Finally, `ipatInvolution_sign` states `(ipatInvolution spt h).sign = -spt.sign`, with `sign` defined to be `1` or `-1`. Although there is no separately named fixed-point theorem, sign reversal between the nonzero values \u00b11 implies that no tuple can equal its image. Thus the formal package also entails \u201cwith no fixed points,\u201d with no added mathematical hypotheses."
}