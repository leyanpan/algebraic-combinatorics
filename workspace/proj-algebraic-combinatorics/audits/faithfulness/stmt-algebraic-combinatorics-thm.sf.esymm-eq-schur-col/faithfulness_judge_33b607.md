## TARGET SymmetricFunctions.esymm_eq_schur_colPartition (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} {R : Type u_1} [inst : CommRing R] (n : ℕ) (hn : n ≤ N),
  MvPolynomial.esymm (Fin N) R n = SymmetricFunctions.schur (SymmetricFunctions.NPartition.colPartition N n hn)

Docstring: The elementary symmetric polynomial e_n equals the Schur polynomial
...
## PROJECT DEPENDENCY SymmetricFunctions.NPartition.colPartition (def)
(N n : ℕ) → n ≤ N → SymmetricFunctions.NPartition N

Body:
fun N n _hn => { parts := fun i => if ↑i < n then 1 else 0, weaklyDecreasing := ⋯ }

Docstring: The column partition (1, 1, ..., 1, 0, ..., 0) with n ones (n boxes in first column).
...
## INFORMAL STATEMENT
thm.sf.esymm-eq-schur-col

\leanhelper  For $n \leq N$, the elementary symmetric polynomial $e_n$ equals the Schur polynomial indexed by the column partition: 
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.sf.col-partition
def.sf.col-partition

\leanhelper  The \emph{column partition} $(1, 1, \ldots , 1, 0, \ldots , 0)$ with $n$ ones (i.e., $n$ boxes in the first column). This is the partition corresponding to $e_n$. Requires $n \leq N$.
...
{
  "verdict": "faithful",
  "justification": "The target states exactly that for `n : \u2115` with `hn : n \u2264 N`, `MvPolynomial.esymm (Fin N) R n = SymmetricFunctions.schur (SymmetricFunctions.NPartition.colPartition N n hn)`, matching \u201cFor $n \\leq N$, the elementary symmetric polynomial $e_n$ equals the Schur polynomial indexed by the column partition.\u201d The dependency `colPartition` has parts `if \u2191i < n then 1 else 0`, hence represents $(1^n,0^{N-n})`; `MvPolynomial.esymm` is documented as the sum of degree-`n` squarefree monomials; and `schur` is defined as the sum of `T.toMonomial` over all SSYT, matching the informal definitions. The use of variables indexed by `Fin N` is explicitly sanctioned by the informal convention. The binder `[CommRing R]` supplies the coefficient setting required by the project\u2019s `schur` definition, and quantifying over every commutative ring is at least as general as the unspecified coefficient setting of the blueprint, not a weakening."
}