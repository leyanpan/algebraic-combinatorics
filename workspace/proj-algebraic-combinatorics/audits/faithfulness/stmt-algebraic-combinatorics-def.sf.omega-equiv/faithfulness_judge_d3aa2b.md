## TARGET SymmetricFunctions.omegaInvolutionEquiv (def) — ELABORATED SIGNATURE
{σ : Type u_1} →
  [inst : Fintype σ] →
    [DecidableEq σ] →
      {R : Type u_2} →
...
fun {σ} [Fintype σ] [DecidableEq σ] {R} [CommRing R] {n} hn =>
  AlgEquiv.ofBijective (SymmetricFunctions.omegaInvolution hn) ⋯

Docstring: The ω-involution as an algebra equivalence. 
...
## PROJECT DEPENDENCY SymmetricFunctions.omegaInvolution_bijective (theorem)
∀ {σ : Type u_1} [inst : Fintype σ] [inst_1 : DecidableEq σ] {R : Type u_2} [inst_2 : CommRing R] {n : ℕ}
  (hn : Fintype.card σ = n), Function.Bijective ⇑(SymmetricFunctions.omegaInvolution hn)

Docstring: The ω-involution is bijective. 
...
## BASE-LIBRARY REF AlgEquiv.ofBijective
{R : Type uR} →
  {A₁ : Type uA₁} →
    {A₂ : Type uA₂} →
      [inst : CommSemiring R] →
        [inst_1 : Semiring A₁] →
          [inst_2 : Semiring A₂] →
            [inst_3 : Algebra R A₁] → [inst_4 : Algebra R A₂] → (f : A₁ →ₐ[R] A₂) → Function.Bijective ⇑f → A₁ ≃ₐ[R] A₂

Docstring: Promotes a bijective algebra homomorphism to an algebra equivalence. 
...
\leanhelper  The $\omega $-involution promoted to an algebra equivalence $\omega : \Lambda _R \xrightarrow {\sim } \Lambda _R$.
...
{
  "verdict": "faithful",
  "justification": "The blueprint asks for \u201cthe \u03c9-involution promoted to an algebra equivalence \u03c9 : \u039b_R \u2192\u223c \u039b_R.\u201d The target returns exactly an algebra self-equivalence, `\u21a5(MvPolynomial.symmetricSubalgebra \u03c3 R) \u2243\u2090[R] \u21a5(MvPolynomial.symmetricSubalgebra \u03c3 R)`, and its body is `AlgEquiv.ofBijective (SymmetricFunctions.omegaInvolution hn) \u22ef`, so the underlying map is precisely the project\u2019s \u03c9-involution, promoted using its bijectivity. The binder `[CommRing R]` matches the fixed commutative coefficient ring, while `[Fintype \u03c3]`, `[DecidableEq \u03c3]`, and `hn : Fintype.card \u03c3 = n` encode a finite set of `n` variables; allowing an arbitrary finite indexing type `\u03c3` is more general than the blueprint\u2019s standard indexing by `[N]`, not a restriction. `DecidableEq \u03c3` is computational structure needed by the polynomial definitions rather than a mathematically substantive added hypothesis."
}