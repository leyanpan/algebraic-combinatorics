## TARGET AlgebraicCombinatorics.FPS.coeff_fpsPow_eq_coeffExpPoly_eval (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] (k : ℕ) (c : K),
  (PowerSeries.coeff k) (PowerSeries.subst (c • AlgebraicCombinatorics.FPS.logSeries) (PowerSeries.exp K)) =
    Polynomial.eval c (Polynomial.map (algebraMap ℚ K) (AlgebraicCombinatorics.FPS.coeffExpPoly✝ k))

Docstring: coeffExpPoly k evaluated at c equals coeff k (fpsExp (c • logSeries)).
...
## PROJECT DEPENDENCY _private.AlgebraicCombinatorics.FPS.NonIntegerPowers.0.AlgebraicCombinatorics.FPS.coeffExpPoly (def)
ℕ → Polynomial ℚ
...
fun k =>
  ∑ d ∈ Finset.range (k + 1),
    Polynomial.C
        ((PowerSeries.coeff d) (PowerSeries.exp ℚ) *
          (PowerSeries.coeff k) (AlgebraicCombinatorics.FPS.logSeriesQ✝ ^ d)) *
      Polynomial.X ^ d

Docstring: The polynomial for coeff k (fpsPow (1+X) c).
This is ∑_{d=0}^k (coeff d exp • coeff k (logSeriesQ^d)) • X^d.
When evaluated at c, this gives coeff k (fpsExp (c • logSeries)). 

## PROJECT DEPENDENCY _private.AlgebraicCombinatorics.FPS.NonIntegerPowers.0.AlgebraicCombinatorics.FPS.logSeriesQ (def)
PowerSeries ℚ

Body:
PowerSeries.mk fun n => if n = 0 then 0 else (-1) ^ (n - 1) / ↑n

Docstring: logSeries over ℚ. This is the base version that maps to logSeries K under algebraMap. 
...
## INFORMAL STATEMENT
lem.fps.coeff-fpsPow-eq-coeffExpPoly-eval

\leanhelper  The coefficient $[x^k](1+x)^c$ equals the evaluation of a polynomial in~ $c$: 
...
{
  "verdict": "faithful",
  "justification": "The formal binders `\u2200 {K} [CommRing K] [Algebra \u211a K] (k : \u2115) (c : K)` exactly match the section\u2019s commutative-\u211a-algebra setting and the parameters `k` and `c`. Its left side, `coeff k (subst (c \u2022 logSeries) (PowerSeries.exp K))`, is `[x^k] Exp(c\u00b7log\u0304)`, which is `[x^k](1+x)^c` by the informal definitions of `f^c`, `Log`, and `logSeries` specialized to `f = 1+x`. The right side evaluates the rational polynomial `coeffExpPoly\u271d k : Polynomial \u211a` at `c` after mapping its coefficients along `algebraMap \u211a K`, exactly expressing `q_k(c)`. The formal theorem is stronger than the existential wording \u201cfor some q_k \u2208 \u211a[x]\u201d because it supplies the explicit, uniform witness `coeffExpPoly\u271d k`."
}