## TARGET AlgebraicCombinatorics.Determinants.prod_Ioi_neg_sub' (theorem) — ELABORATED SIGNATURE
∀ {m : ℕ} {S : Type u_2} [inst : CommRing S] (v : Fin m → S),
  ∏ i, ∏ j ∈ Finset.Ioi i, (v i - v j) = (-1) ^ (m * (m - 1) / 2) * ∏ i, ∏ j ∈ Finset.Ioi i, (v j - v i)

Docstring: The product transformation: swapping subtraction order introduces a sign.
...
## INFORMAL STATEMENT
lem.det.vander.prod-Ioi-neg-sub

\leanhelper  Let $v_1, \ldots , v_m$ be elements of a commutative ring $S$. Then 

\[  \prod _{1 \leq i < j \leq m} (v_i - v_j) = (-1)^{m(m-1)/2} \prod _{1 \leq i < j \leq m} (v_j - v_i).  \]

## VERDICT
{
  "verdict": "faithful",
  "justification": "The informal hypothesis \u201cLet v\u2081, \u2026, v\u2098 be elements of a commutative ring S\u201d is represented by `\u2200 {m : \u2115} {S : Type u_2} [inst : CommRing S] (v : Fin m \u2192 S)`. The indexing `\u220f i, \u220f j \u2208 Finset.Ioi i` ranges over all `i : Fin m` and all `j > i`, exactly corresponding\u2014after the harmless zero-based indexing of `Fin m`\u2014to `1 \u2264 i < j \u2264 m`. The formal equality has the same factors and sign, namely `(v i - v j) = (-1) ^ (m * (m - 1) / 2) * \u2026 (v j - v i)`. Thus it faithfully expresses the blueprint statement."
}