## TARGET AlgebraicCombinatorics.CycleDecomposition.cycleLengths_sum_eq_support_card (theorem) — ELABORATED SIGNATURE
∀ {α : Type u_1} [inst : DecidableEq α] [inst_1 : Fintype α] (σ : Equiv.Perm α),
  (AlgebraicCombinatorics.CycleDecomposition.cycleLengths σ).sum = σ.support.card

Docstring: The sum of cycle lengths equals the size of the support. 

## PROJECT DEPENDENCY AlgebraicCombinatorics.CycleDecomposition.cycleLengths (def)
{α : Type u_1} → [DecidableEq α] → [Fintype α] → Equiv.Perm α → Multiset ℕ

Body:
fun {α} [DecidableEq α] [Fintype α] σ => σ.cycleType

Docstring: The cycle type as a multiset of cycle lengths. 
...
Body:
fun {α} [DecidableEq α] [Fintype α] f => {x | f x ≠ x}

Docstring: The `Finset` of nonfixed points of a permutation. 

Characterization: The finset of points the permutation moves; fixed points are excluded.
...
Body:
fun {α} [Fintype α] [DecidableEq α] σ => Multiset.map (Finset.card ∘ Equiv.Perm.support) σ.cycleFactorsFinset.val

Docstring: The cycle type of a permutation 

Characterization: The multiset of lengths of the non-trivial cycles of a permutation; fixed points contribute nothing and every member is ≥ 2 (`Equiv.Perm.two_le_of_mem_cycleType`).

## INFORMAL STATEMENT
lem.cycleLengths_sum

\leanhelper  The sum of the cycle lengths (of nontrivial cycles) equals the number of non-fixed-point elements of $\sigma $.
...
## VERDICT
{
  "verdict": "faithful",
  "discrepancies": [],
  "justification": "The formal definition unfolds as `cycleLengths \u03c3 = \u03c3.cycleType`, characterized as \u201cthe multiset of lengths of the non-trivial cycles of a permutation; fixed points contribute nothing.\u201d Thus `(cycleLengths \u03c3).sum` is exactly the informal \u201csum of the cycle lengths (of nontrivial cycles).\u201d The right side is `\u03c3.support.card`, where `support` is `{x | \u03c3 x \u2260 x}`, exactly the \u201cnumber of non-fixed-point elements of \u03c3.\u201d The binders `[Fintype \u03b1]` and `[DecidableEq \u03b1]` provide the finite, decidable representation needed for these multisets and finsets and do not alter the mathematical claim."
}