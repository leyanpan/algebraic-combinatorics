## TARGET PermFinset.submatrixDet_of_card_eq (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (A : Matrix (Fin n) (Fin n) R) (P Q : Finset (Fin n))
  (h : P.card = Q.card), PermFinset.submatrixDet A P Q = (A.submatrix ⇑(P.orderEmbOfFin ⋯) ⇑(Q.orderEmbOfFin ⋯)).det

Docstring: When |P| = |Q|, the submatrix determinant equals the actual determinant.
This lemma bridges the "total" definition with the "proof-requiring" style. 

## PROJECT DEPENDENCY PermFinset.submatrixDet (def)
{R : Type u_1} → [CommRing R] → {n : ℕ} → Matrix (Fin n) (Fin n) R → Finset (Fin n) → Finset (Fin n) → R

Body:
fun {R} [CommRing R] {n} A P Q =>
  if h : P.card = Q.card then (A.submatrix ⇑(P.orderEmbOfFin ⋯) ⇑(Q.orderEmbOfFin ⋯)).det else 0
...
## INFORMAL STATEMENT
lem.det.submatrixDet-card-eq

\leanhelper  When $|P| = |Q|$, the submatrix determinant equals the actual determinant of the submatrix $A[P, Q]$.
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.det.submatrixdet
def.det.submatrixDet

\leanhelper  The \emph{submatrix determinant} of an $n \times n$ matrix $A$ with row set $P$ and column set $Q$ (both subsets of $[n]$) is 
...
{
  "verdict": "faithful",
  "justification": "The blueprint says, \u201cWhen |P| = |Q|, the submatrix determinant equals the actual determinant of the submatrix A[P, Q].\u201d The target has exactly the corresponding hypothesis `(h : P.card = Q.card)` and conclusion `PermFinset.submatrixDet A P Q = (A.submatrix \u21d1(P.orderEmbOfFin \u22ef) \u21d1(Q.orderEmbOfFin \u22ef)).det`. By the body of `PermFinset.submatrixDet`, under `h` its value is the determinant branch rather than `0`. Moreover, `Finset.orderEmbOfFin` enumerates each finite set increasingly, so the formal `Matrix.submatrix` is precisely the naturally ordered submatrix `A[P,Q]` specified by the informal definition. The binders `{R} [CommRing R] {n} (A : Matrix (Fin n) (Fin n) R) (P Q : Finset (Fin n))` faithfully encode an `n \u00d7 n` matrix over a commutative ring and row and column subsets of `[n]`; the zero-based `Fin n` indexing is only an equivalent formal encoding of the blueprint\u2019s `[n]`."
}