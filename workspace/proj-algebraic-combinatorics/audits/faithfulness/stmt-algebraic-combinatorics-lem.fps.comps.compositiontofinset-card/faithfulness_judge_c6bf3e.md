## TARGET AlgebraicCombinatorics.MathlibComposition.compositionToFinset_card_add_one (theorem) — ELABORATED SIGNATURE
∀ (n : ℕ),
  0 < n →
    ∀ (c : Composition n), ((AlgebraicCombinatorics.MathlibComposition.compositionToFinset n) c).card + 1 = c.length

Docstring: The cardinality of the associated finset plus 1 equals the composition length. 

## PROJECT DEPENDENCY AlgebraicCombinatorics.MathlibComposition.compositionToFinset (def)
...
## INFORMAL STATEMENT
lem.fps.comps.compositionToFinset-card

\leanhelper  For $n > 0$ and a composition $c$ of $n$, the finset $S(c)$ corresponding to $c$ under the composition-to-finset equivalence satisfies $|S(c)| + 1 = \ell (c)$.
...
{
  "verdict": "faithful",
  "justification": "The blueprint quantifies \u201cFor $n>0$ and a composition $c$ of $n$,\u201d exactly matching the formal binders `\u2200 (n : \u2115), 0 < n \u2192 \u2200 (c : Composition n)`. Its conclusion `$|S(c)| + 1 = \\ell(c)$` matches `((compositionToFinset n) c).card + 1 = c.length`: `Finset.card` is cardinality, `Composition.length` is the number of parts, and `compositionToFinset n` is precisely the stated composition-to-finset equivalence, with body `(compositionEquiv n).trans (compositionAsSetEquiv n)`. No additional mathematically substantive hypothesis or narrowed quantifier is present."
}