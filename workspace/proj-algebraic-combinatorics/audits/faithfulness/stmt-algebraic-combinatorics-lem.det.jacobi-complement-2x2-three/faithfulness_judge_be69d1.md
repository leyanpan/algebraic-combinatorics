## TARGET AlgebraicCombinatorics.Determinants.jacobi_complement_2x2_three (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] (A : Matrix (Fin 3) (Fin 3) R),
  A.adjugate 0 0 * A.adjugate (Fin.last 2) (Fin.last 2) - A.adjugate 0 (Fin.last 2) * A.adjugate (Fin.last 2) 0 =
    A.det * (AlgebraicCombinatorics.Determinants.innerSubmatrix A).det

Docstring: The key lemma for 3×3 matrices (case n = 1):
...
## INFORMAL STATEMENT
lem.det.jacobi-complement-2x2-three

\leanhelper  For a $3 \times 3$ matrix $A$, the $2 \times 2$ determinant of the corner entries of $\operatorname {adj} A$ at positions $\{ 0, 2\}  \times \{ 0, 2\} $ satisfies 
...
{
  "verdict": "faithful",
  "justification": "The formal binder `\u2200 {R : Type u_1} [inst : CommRing R] (A : Matrix (Fin 3) (Fin 3) R)` matches the blueprint\u2019s fixed commutative ring and arbitrary `3 \u00d7 3` matrix. On the left, `0` and `Fin.last 2` are precisely indices `0` and `2`, so the expression is exactly `(adj A)\u2080\u2080 (adj A)\u2082\u2082 - (adj A)\u2080\u2082 (adj A)\u2082\u2080`. On the right, `innerSubmatrix A` has index type `Fin 1`; its sole row and column are mapped by `i.succ.castSucc` to index `1` of `Fin 3`. Hence its determinant is the determinant of the `1 \u00d7 1` middle submatrix, namely `A\u2081\u2081`, making `A.det * (innerSubmatrix A).det` exactly `det A \u00b7 A\u2081\u2081`. The formal theorem therefore states the displayed blueprint identity without additional mathematical hypotheses."
}