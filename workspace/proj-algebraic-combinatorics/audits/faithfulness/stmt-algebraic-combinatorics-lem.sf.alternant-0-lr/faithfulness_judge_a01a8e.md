## TARGET AlgebraicCombinatorics.alternant_eq_zero_of_repeated (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {N : ℕ} {α : Fin N → ℕ},
  (∃ i j, i ≠ j ∧ α i = α j) → AlgebraicCombinatorics.alternant α = 0

Docstring: If α has two equal entries, then a_α = 0. (lem.sf.alternant-0 (a)) 

## TARGET AlgebraicCombinatorics.alternant_swap (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {N : ℕ} {α : Fin N → ℕ} {i j : Fin N},
  i ≠ j → AlgebraicCombinatorics.alternant (α ∘ ⇑(Equiv.swap i j)) = -AlgebraicCombinatorics.alternant α

Docstring: Swapping two entries of α negates the alternant. (lem.sf.alternant-0 (b)) 
...
## INFORMAL STATEMENT
lem.sf.alternant-0-lr

Let $\alpha \in \mathbb {N}^{N}$. 

\medskip 

\textbf{(a)} If the $N$-tuple $\alpha $ has two equal entries, then $a_{\alpha }=0$. 

\medskip 

\textbf{(b)} Let $\beta \in \mathbb {N}^{N}$ be an $N$-tuple obtained from $\alpha $ by swapping two entries. Then, $a_{\beta }=-a_{\alpha }$.
...
{
  "verdict": "faithful",
  "justification": "Both declarations faithfully formalize the two parts. For (a), the blueprint says \u201cIf the N-tuple \u03b1 has two equal entries, then a_\u03b1 = 0,\u201d and the formal hypothesis is exactly \u201c\u2203 i j, i \u2260 j \u2227 \u03b1 i = \u03b1 j,\u201d with conclusion \u201calternant \u03b1 = 0.\u201d For (b), \u201c\u03b2 \u2026 obtained from \u03b1 by swapping two entries\u201d is represented by \u201c\u03b1 \u2218 \u21d1(Equiv.swap i j),\u201d and the binder \u201ci \u2260 j\u201d expresses that two distinct positions are swapped; the conclusion is exactly \u201calternant (\u03b1 \u2218 \u21d1(Equiv.swap i j)) = -alternant \u03b1.\u201d The dependency `alternant` also matches the determinant definition: its summand `sign \u03c3 \u2022 xPow (\u03b1 \u2218 \u03c3)` is the polynomial monomial \u220f\u1d62 x\u1d62^(\u03b1_(\u03c3 i)) occurring in the determinant expansion of `(x_i^(\u03b1_j))`. Using `Fin N` instead of `[N]` is only a zero-based encoding of an N-element index set. Finally, universally quantifying `{R} [CommRing R]` and `{N}` is more general than the chapter\u2019s fixed commutative ring K and fixed N, not a restriction."
}