## TARGET AlgebraicCombinatorics.QBinomialRec.qBinomial_limit_eq_partition_gf (theorem) — ELABORATED SIGNATURE
∀ (k : ℕ),
  ∏ i ∈ Finset.range k, (1 - PowerSeries.X ^ (i + 1))⁻¹ =
    PowerSeries.mk fun n => ↑(AlgebraicCombinatorics.QBinomialRec.partitionCountAtMostKParts n k)

Docstring: The limit of q-binomial coefficients equals the generating function for
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.QBinomialRec.partitionCountAtMostKParts (def)
ℕ → ℕ → ℕ

Body:
fun n k => Nat.card { p // p.parts.card ≤ k }

Docstring: The number of partitions of n with at most k parts. 
...
## INFORMAL STATEMENT
thm.qbinomial-formulas.limit-eq-partition-gf

\leanhelper  The product formula equals the generating function for partitions with at most $k$ parts: 
...
{
  "verdict": "faithful",
  "justification": "The target quantifies exactly over the intended parameter, `\u2200 (k : \u2115)`. Its product `\u220f i \u2208 Finset.range k, (1 - PowerSeries.X ^ (i + 1))\u207b\u00b9` indexes `i = 0,\u2026,k-1`, hence has exponents `1,\u2026,k`, matching `\u220f_{i=1}^{k} 1/(1-q^i)`. The right side `PowerSeries.mk fun n => \u2191(partitionCountAtMostKParts n k)` is the formal power series whose nth coefficient is the partition count, matching `\u2211_{n=0}^\u221e |{p \u22a2 n : at most k parts}| \u00b7 q^n`. In particular, the dependency defines that coefficient as `Nat.card { p // p.parts.card \u2264 k }`; `Nat.Partition` denotes partitions of `n`, and `p.parts.card \u2264 k` is exactly \u201cat most k parts.\u201d The use of formal power series over `\u211a` is an encoding of the coefficient counts and introduces no additional mathematical hypothesis or narrowed combinatorial quantifier."
}