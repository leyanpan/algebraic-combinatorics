## TARGET AlgebraicCombinatorics.FPS.coeff_logSeries_pow_eq_zero_of_gt (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] (k m : ℕ),
  k < m → (PowerSeries.coeff k) (AlgebraicCombinatorics.FPS.logSeries ^ m) = 0

Docstring: Key lemma: coeff k (logSeries^m) = 0 for m > k.
...
## INFORMAL STATEMENT
lem.fps.coeff-logSeries-pow-eq-zero

\leanhelper  For $k < m$, we have $[x^k](\overline{\log }^m) = 0$ (since $\overline{\log }$ has order~ $\geq 1$, so $\overline{\log }^m$ has order~ $\geq m$).
...
{
  "verdict": "faithful",
  "justification": "The target states exactly that for natural numbers `(k m : \u2115)`, under `k < m`, `(PowerSeries.coeff k) (logSeries ^ m) = 0`, matching \u201cFor $k<m$, we have $[x^k](\\overline{\\log}^m)=0$.\u201d The dependency `logSeries` has coefficient `0` at `n = 0` and, otherwise, `(algebraMap \u211a K) ((-1)^(n-1) / \u2191n)`, which matches the informal logarithm series. The binders `[CommRing K] [Algebra \u211a K]` encode the coefficient-ring setting needed for the rational coefficients $(-1)^{n-1}/n$ and do not alter the asserted vanishing property."
}