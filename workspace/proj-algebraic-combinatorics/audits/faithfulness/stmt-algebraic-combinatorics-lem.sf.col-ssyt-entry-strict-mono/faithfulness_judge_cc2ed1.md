## TARGET SymmetricFunctions.colSSYTEntry_strictMono (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} (n : ℕ) (hn : n ≤ N) (T : SymmetricFunctions.SSYT (SymmetricFunctions.NPartition.colPartition N n hn))
  (k₁ k₂ : ℕ) (hk₁ : k₁ < n) (hk₂ : k₂ < n),
  k₁ < k₂ → SymmetricFunctions.colSSYTEntry n hn T k₁ hk₁ < SymmetricFunctions.colSSYTEntry n hn T k₂ hk₂

Docstring: Column entries are strictly increasing - general case 
...
## PROJECT DEPENDENCY SymmetricFunctions.colSSYTEntry (def)
{N : ℕ} →
  (n : ℕ) →
    (hn : n ≤ N) → SymmetricFunctions.SSYT (SymmetricFunctions.NPartition.colPartition N n hn) → (k : ℕ) → k < n → Fin N
...
Docstring: Helper for extracting entry from column SSYT 
...
## INFORMAL STATEMENT
lem.sf.col-ssyt-entry-strict-mono

\leanhelper  The column entries of any SSYT of column partition shape are strictly increasing: if $k_1 < k_2 < n$, then $T(k_1, 0) < T(k_2, 0)$. This follows from the column-strict condition of semistandard tableaux.
...
{
  "verdict": "faithful",
  "justification": "The blueprint states that for any column-shape SSYT, \u201cif $k_1<k_2<n$, then $T(k_1,0)<T(k_2,0)$.\u201d The target quantifies over `T : SymmetricFunctions.SSYT (SymmetricFunctions.NPartition.colPartition N n hn)` and concludes `colSSYTEntry ... k\u2081 hk\u2081 < colSSYTEntry ... k\u2082 hk\u2082` from `k\u2081 < k\u2082`, with `hk\u2081 : k\u2081 < n` and `hk\u2082 : k\u2082 < n`. By the body of `colSSYTEntry`, these are exactly `T.entries \u27e8k, ...\u27e9 \u27e80, ...\u27e9`, i.e. the entries `T(k,0)`. The explicit `hn : n \u2264 N` matches the informal definition of the column partition, which \u201cRequires $n \\leq N$.\u201d Although `hk\u2081` is separately required, it is redundant under the blueprint assumptions `k\u2081 < k\u2082 < n`, so it creates no mathematical restriction. The dependent `Fin N` representation agrees with the stated zero-based Lean convention."
}