## TARGET PowerSeries.summable_iff_coeff_finitely_determined (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_2} (a : I → PowerSeries R),
  (∀ (n : ℕ), {i | (PowerSeries.coeff n) (a i) ≠ 0}.Finite) ↔ ∀ (n : ℕ), PowerSeries.CoeffFinitelyDeterminedInSum a n

Docstring: A family of FPS is summable if and only if each coefficient in its sum is finitely determined.
(Label: prop.fps.summable=fin-det part (a)) 

## PROJECT DEPENDENCY PowerSeries.CoeffFinitelyDeterminedInSum (def)
{R : Type u_1} → [CommRing R] → {I : Type u_2} → (I → PowerSeries R) → ℕ → Prop

Body:
fun {R} [CommRing R] {I} a n => ∃ M, PowerSeries.DeterminesCoeffInSum a M n

Docstring: The `x^n`-coefficient in the sum of `(a_i)_{i ∈ I}` is finitely determined
if there exists a finite subset `M` that determines it.
(Label: def.fps.xn-coeff-fin-determined part (a)) 

## PROJECT DEPENDENCY PowerSeries.DeterminesCoeffInSum (def)
...
Docstring: A finite subset `M` of `I` determines the `x^n`-coefficient in the sum of
...
## INFORMAL STATEMENT
prop.fps.summable=fin-det

Let $\left(\mathbf{a}_{i}\right)_{i\in I}\in K\left[\left[x\right]\right]^{I}$ be a (possibly infinite) family of FPSs. Then: 

\textbf{(a)} The family $\left(\mathbf{a}_{i}\right)_{i\in I}$ is summable if and only if each coefficient in its sum is finitely determined (i.e., for each $n\in \mathbb {N}$, the $x^{n}$-coefficient in the sum of $\left(\mathbf{a}_{i}\right)_{i\in I}$ is finitely determined). 
...
{
  "verdict": "drift",
  "justification": "The logical content of part (a) otherwise matches: the formal left side, `\u2200 (n : \u2115), {i | (PowerSeries.coeff n) (a i) \u2260 0}.Finite`, is exactly the informal definition that for every coefficient \u201call but finitely many\u201d terms vanish, and the right side expands to `\u2200 n, \u2203 M : Finset I, \u2200 J, M \u2286 J \u2192 ...`, matching finite determination. However, the elaborated signature adds the contentful binder `[inst : CommRing R]`. Neither the informal statement nor the supplied informal definitions state that `K` is a commutative ring; in particular, the coordinate operations shown there do not impose commutativity of multiplication. Thus the formal theorem quantifies only over commutative rings and is weaker than the unrestricted blueprint statement. This restriction also enters through the `[CommRing R]` binders of `PowerSeries.CoeffFinitelyDeterminedInSum` and `PowerSeries.DeterminesCoeffInSum`. To make the declaration faithful, either the blueprint must explicitly assume that `K` is a commutative ring, or the target and these dependencies must be generalized to the weakest suitable algebraic structure (for example `Ring R`, if noncommutative coefficient rings are intended). The target is explicitly labeled part (a), so its not formalizing part (b) is not itself a mismatch."
}