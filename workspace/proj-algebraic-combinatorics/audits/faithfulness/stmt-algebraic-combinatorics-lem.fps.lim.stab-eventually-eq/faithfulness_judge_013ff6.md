## TARGET Seq.stabilizesTo_of_eventually_eq (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} {a b : ℕ → K} {lim : K}, Seq.StabilizesTo a lim → (∃ N, ∀ i ≥ N, a i = b i) → Seq.StabilizesTo b lim

Docstring: If a sequence is eventually equal to another, they stabilize to the same limit. 
...
## INFORMAL STATEMENT
lem.fps.lim.stab-eventually-eq

\leanhelper  If a sequence $(a_i)$ stabilizes to $\ell $, and a sequence $(b_i)$ is eventually equal to $(a_i)$ (i.e., there exists $N$ such that $a_i = b_i$ for all $i \geq N$), then $(b_i)$ also stabilizes to $\ell $.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over arbitrary sequences and a limit, `\u2200 {K : Type u_1} {a b : \u2115 \u2192 K} {lim : K}`, and assumes exactly that `a` stabilizes to `lim` and that `a` and `b` are eventually equal: `Seq.StabilizesTo a lim \u2192 (\u2203 N, \u2200 i \u2265 N, a i = b i)`. Its conclusion is exactly `Seq.StabilizesTo b lim`. Moreover, `Seq.StabilizesTo a lim` unfolds to `\u2203 N, \u2200 i \u2265 N, a i = lim`, matching the informal definition. Thus this directly formalizes: \u201cIf `(a_i)` stabilizes to `\u2113`, and `(b_i)` is eventually equal to `(a_i)` ... then `(b_i)` also stabilizes to `\u2113`,\u201d with no added hypotheses or narrowed quantifiers."
}