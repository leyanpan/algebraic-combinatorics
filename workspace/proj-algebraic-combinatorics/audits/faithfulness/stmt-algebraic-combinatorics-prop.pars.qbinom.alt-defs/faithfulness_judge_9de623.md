## TARGET AlgebraicCombinatorics.qBinomial_one_eq_binomial (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommSemiring R] (n k : ℕ), AlgebraicCombinatorics.qBinomial n k 1 = ↑(n.choose k)

Docstring: At q = 1, the q-binomial coefficient equals the ordinary binomial coefficient.

See Proposition 11.2.1(c) in the source. 

## TARGET AlgebraicCombinatorics.qBinomial_eq_sum_increasing_tuples (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommSemiring R] (n k : ℕ),
  k ≤ n →
    ∀ (q : R),
      AlgebraicCombinatorics.qBinomial n k q = ∑ f ∈ AlgebraicCombinatorics.monotoneFunctions k (n - k), q ^ ∑ i, ↑(f i)

Docstring: The q-binomial is defined as a sum over weakly increasing tuples:
...
\[  \binom {n}{k}_{q}=\sum _{0\leq i_{1}\leq i_{2}\leq \cdots \leq i_{k}\leq n-k}q^{i_{1}+i_{2}+\cdots +i_{k}}.  \]
 ...
 Here, the sum ranges over all weakly increasing $k$-tuples $(i_{1},i_{2},\ldots ,i_{k}) \in \{ 0,1,\ldots ,n-k\} ^{k}$. If $k>n$, then this is an empty sum. If $k<0$, then this is also an empty sum. 

\textbf{(b)} Set $\operatorname {sum}S=\sum _{s\in S}s$ for any finite set $S$ of integers. Then, we have 
...
{
  "verdict": "unstatable",
  "justification": "The blueprint quantifies \u201cLet n\u2208\u2115 and k\u2208\u2124\u201d and explicitly requires the negative-index case: \u201cIf k<0, then this is also an empty sum.\u201d Both targets instead bind `(n k : \u2115)`, and the dependency `AlgebraicCombinatorics.qBinomial` itself has type `\u2115 \u2192 \u2115 \u2192 R \u2192 R`. Thus negative integer values of `k` cannot even be supplied to the project\u2019s q-binomial definition. In addition, target (a) assumes `k \u2264 n`, whereas the blueprint also asserts the empty-sum case `k > n`; this restriction enters at the binder `k \u2264 n \u2192`. The existing tuple expression cannot simply be used outside that range, since natural subtraction makes `n - k = 0`, and `monotoneFunctions k 0` is not an empty indexing set. To make the proposition faithful, `qBinomial` (or a project-level integer-indexed wrapper) must accept `k : \u2124`, return zero for `k < 0` and `k > n`, and use the tuple sum in range. Then both targets should quantify `k : \u2124`; target (a) should cover all cases with an explicitly empty/zero right-hand side out of range, and target (c) should use the blueprint\u2019s integer-indexed ordinary binomial, also zero for negative `k`."
}