## TARGET SymmetricFunctions.esymm_recurrence_he (theorem) — ELABORATED SIGNATURE
∀ {σ : Type u_1} [inst : Fintype σ] [inst_1 : DecidableEq σ] {R : Type u_2} [inst_2 : CommRing R] (n : ℕ),
  0 < n →
    MvPolynomial.esymm σ R n =
      ∑ j ∈ Finset.Ico 1 (n + 1), (-1) ^ (j + 1) * MvPolynomial.hsymm σ R j * MvPolynomial.esymm σ R (n - j)

Docstring: Recurrence for e_n from symmetric Newton-Girard: e_n = ∑_{j=1}^n (-1)^{j+1} h_j e_{n-j} for n > 0.
...
## BASE-LIBRARY REF Finset.Ico
{α : Type u_1} → [inst : Preorder α] → [LocallyFiniteOrder α] → α → α → Finset α

Docstring: The finset $[a, b)$ of elements `x` such that `a ≤ x` and `x < b`. Basically `Set.Ico a b` as a
finset. 
...
## INFORMAL STATEMENT
lem.sf.esymm-recurrence

\leanhelper  Recurrence for $e_n$ from the symmetric Newton–Girard: for $n > 0$, 

\[  e_n = \sum _{j=1}^{n} (-1)^{j+1}\,  h_j \,  e_{n-j}.  \]

 This follows from $\sum _{j=0}^n (-1)^j h_j e_{n-j} = 0$ (Theorem~ \ref{thm.sf.newton-girard-he}) by isolating $j = 0$.
...
{
  "verdict": "faithful",
  "justification": "The formal conclusion exactly matches the recurrence: `MvPolynomial.esymm \u03c3 R n = \u2211 j \u2208 Finset.Ico 1 (n + 1), (-1) ^ (j + 1) * MvPolynomial.hsymm \u03c3 R j * MvPolynomial.esymm \u03c3 R (n - j)`. Since `Finset.Ico 1 (n + 1)` consists precisely of natural numbers `j` with `1 \u2264 j \u2264 n`, this is the blueprint\u2019s `e_n = \u2211_{j=1}^{n} (-1)^{j+1} h_j e_{n-j}`. The hypothesis `0 < n` is exactly \u201cfor `n > 0`.\u201d The base-library meanings of `esymm` and `hsymm` agree with the informal definitions as sums of squarefree monomials and all monomials of degree `n`, respectively. Quantification over arbitrary finite `\u03c3` and arbitrary commutative coefficient ring `R` is more general than the stated variables `x\u2081,\u2026,x_N`, rather than a weakening. `[DecidableEq \u03c3]` is an implementation-level requirement for `hsymm`; on a finite variable set it does not impose a mathematically substantive restriction."
}