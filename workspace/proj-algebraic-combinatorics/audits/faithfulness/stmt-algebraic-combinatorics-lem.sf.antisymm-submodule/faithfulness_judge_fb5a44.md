## TARGET AlgebraicCombinatorics.SymmetricPolynomials.isAntisymm_add (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ} {f g : AlgebraicCombinatorics.SymmetricPolynomials.P K N},
  AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm f →
    AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm g →
      AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm (f + g)

Docstring: If f and g are antisymmetric, then f + g is antisymmetric.
Label: exa.sf.PS1 

## TARGET AlgebraicCombinatorics.SymmetricPolynomials.isAntisymm_neg (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ} {f : AlgebraicCombinatorics.SymmetricPolynomials.P K N},
  AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm f → AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm (-f)

Docstring: If f is antisymmetric, then -f is antisymmetric.
Label: exa.sf.PS1 

## TARGET AlgebraicCombinatorics.SymmetricPolynomials.isAntisymm_zero (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ}, AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm 0

Docstring: The zero polynomial is antisymmetric.
Label: exa.sf.PS1 

## TARGET AlgebraicCombinatorics.SymmetricPolynomials.isAntisymm_smul (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ} (c : K) {f : AlgebraicCombinatorics.SymmetricPolynomials.P K N},
  AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm f →
    AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm (c • f)

Docstring: If f is antisymmetric, then c • f is antisymmetric for any scalar c.
Label: exa.sf.PS1 
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.SymmetricPolynomials.IsAntisymm (def)
{K : Type u_1} → [inst : CommRing K] → {N : ℕ} → AlgebraicCombinatorics.SymmetricPolynomials.P K N → Prop

Body:
fun {K} [CommRing K] {N} f => ∀ (σ : Equiv.Perm (Fin N)), (MvPolynomial.rename ⇑σ) f = Equiv.Perm.sign σ • f

Docstring: A polynomial is antisymmetric if σ · f = sign(σ) · f for all permutations σ.
Label: exa.sf.PS1 
...
## INFORMAL STATEMENT
lem.sf.antisymm-submodule

\leanhelper  The antisymmetric polynomials form a $K$-submodule of $\mathcal{P}$: the zero polynomial is antisymmetric, and the sum, scalar multiple, and negation of antisymmetric polynomials are antisymmetric.
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.sf.antisymm
def.sf.antisymm

\leanhelper  A polynomial $f \in \mathcal{P}$ is \emph{antisymmetric} if 
...
{
  "verdict": "faithful",
  "justification": "The four declarations collectively formalize exactly the closure properties in the blueprint: `isAntisymm_zero` proves `IsAntisymm 0`; `isAntisymm_add` proves `IsAntisymm f \u2192 IsAntisymm g \u2192 IsAntisymm (f + g)`; `isAntisymm_smul` proves for every `(c : K)` that `IsAntisymm f \u2192 IsAntisymm (c \u2022 f)`; and `isAntisymm_neg` proves `IsAntisymm f \u2192 IsAntisymm (-f)`. These match \u201cthe zero polynomial is antisymmetric, and the sum, scalar multiple, and negation of antisymmetric polynomials are antisymmetric,\u201d which supplies the stated submodule closure. The setting also matches: each signature quantifies over `[CommRing K]` and `N : \u2115`, while `P K N` is definitionally `MvPolynomial (Fin N) K`, the polynomial ring in N variables. Finally, `IsAntisymm f` is defined as `\u2200 (\u03c3 : Equiv.Perm (Fin N)), rename \u21d1\u03c3 f = Equiv.Perm.sign \u03c3 \u2022 f`, corresponding to the blueprint condition `\u03c3 \u00b7 f = sign(\u03c3) \u00b7 f` for every `\u03c3 \u2208 S_N`. Using `Fin N` rather than `{1,\u2026,N}` is only an indexing representation, not a mathematical restriction."
}