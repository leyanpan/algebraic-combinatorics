## TARGET AlgebraicCombinatorics.SymmetricPolynomials.newtonGirard_hp (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ} [inst_1 : DecidableEq (Fin N)] (n : ℕ),
  0 < n →
    ∑ j ∈ Finset.range n,
        AlgebraicCombinatorics.SymmetricPolynomials.h (n - 1 - j) *
          AlgebraicCombinatorics.SymmetricPolynomials.p (j + 1) =
      ↑n * AlgebraicCombinatorics.SymmetricPolynomials.h n

Docstring: Newton-Girard formula relating h and p: ∑_{j=1}^n h_{n-j} p_j = n · h_n.
...
## TARGET AlgebraicCombinatorics.SymmetricPolynomials.newtonGirard_esymm (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ} (k : ℕ),
  ↑k * AlgebraicCombinatorics.SymmetricPolynomials.e k =
    (-1) ^ (k + 1) *
      ∑ a ∈ Finset.antidiagonal k with a.1 < k,
        (-1) ^ a.1 * AlgebraicCombinatorics.SymmetricPolynomials.e a.1 *
          AlgebraicCombinatorics.SymmetricPolynomials.p a.2

Docstring: Newton-Girard formula: recurrence for elementary symmetric polynomials.
k * e_k = (-1)^{k+1} * ∑_{a ∈ antidiagonal k, a.1 < k} (-1)^{a.1} * e_{a.1} * p_{a.2}
(Theorem thm.sf.NG, equation eq.thm.sf.NG.ep).
Label: thm.sf.NG 

## TARGET AlgebraicCombinatorics.SymmetricPolynomials.newtonGirard_eh (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {N : ℕ} [inst_1 : DecidableEq (Fin N)] (n : ℕ),
  0 < n →
    ∑ j ∈ Finset.range (n + 1),
        (-1) ^ j * AlgebraicCombinatorics.SymmetricPolynomials.e j *
          AlgebraicCombinatorics.SymmetricPolynomials.h (n - j) =
      0

Docstring: Newton-Girard formula relating e and h: ∑_{j=0}^n (-1)^j e_j h_{n-j} = 0 for n > 0.
...
\begin{align}  \sum _{j=0}^{n} (-1)^j e_j h_{n-j} & = 0;  \\ \sum _{j=1}^{n} (-1)^{j-1} e_{n-j} p_j & = n e_n;  \\ \sum _{j=1}^{n} h_{n-j} p_j & = n h_n.  \end{align}
...
{
  "verdict": "faithful",
  "justification": "The three declarations exactly cover the three displayed identities. For `newtonGirard_eh`, the binders `(n : \u2115), 0 < n` match \u201cFor any positive integer n,\u201d and `Finset.range (n + 1)` indexes precisely `j = 0,\u2026,n`, yielding `\u2211 (-1)^j e j * h (n-j) = 0`. For `newtonGirard_hp`, `Finset.range n` has `j = 0,\u2026,n-1`; replacing the formal index by `r = j+1` turns `h (n-1-j) * p (j+1)` into `h (n-r) p_r`, exactly `\u2211_{r=1}^n h_{n-r}p_r = n h_n`. For `newtonGirard_esymm`, an antidiagonal pair `a` satisfies `a.1 + a.2 = k`, and the filter `a.1 < k` is equivalent to `a.2 > 0`. Setting `j = a.2` therefore indexes `j=1,\u2026,k`, with `a.1=k-j`; the sign `(-1)^(k+1) * (-1)^(k-j)` equals `(-1)^(j-1)`, so the equality is the informal `\u2211_{j=1}^k (-1)^(j-1)e_{k-j}p_j = k e_k`, up to symmetry of equality. This theorem quantifies over every `(k : \u2115)` rather than requiring `0 < k`; the additional `k=0` case is a harmless strengthening. The `[DecidableEq (Fin N)]` binders on the declarations involving `h` merely provide the decidability required by the body `MvPolynomial.hsymm (Fin N) K n`; every finite index type `Fin N` has such decidable equality, so this is encoding rather than a mathematical restriction. Finally, the formal `p` may differ from the informal convention at index zero, but all occurrences here have strictly positive power-sum index, where `MvPolynomial.psum` has the required meaning."
}