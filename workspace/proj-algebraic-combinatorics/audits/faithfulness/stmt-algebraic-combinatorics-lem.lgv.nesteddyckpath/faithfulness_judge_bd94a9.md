## TARGET LGV.nestedDyckPath (def) — ELABORATED SIGNATURE
ℕ → LGV.dyckDigraph.Path

Body:
fun n => { vertices := LGV.nestedDyckVertices n, nonempty := ⋯, arcs_valid := ⋯ }

Docstring: The nested Dyck path from (-2n, 0) to (2n, 0) 

## TARGET LGV.nestedDyckPath_start (theorem) — ELABORATED SIGNATURE
∀ (n : ℕ), (LGV.nestedDyckPath n).start = (-(2 * ↑n), 0)

## TARGET LGV.nestedDyckPath_finish (theorem) — ELABORATED SIGNATURE
∀ (n : ℕ), (LGV.nestedDyckPath n).finish = (2 * ↑n, 0)
...
## PROJECT DEPENDENCY LGV.nestedDyckVertices (def)
ℕ → List (ℤ × ℕ)
...
fun n =>
  List.map
    (fun x =>
      match x with
      | ⟨i, isLt⟩ => (↑i - 2 * ↑n, min i (4 * n - i)))
    (List.finRange (4 * n + 1))

Docstring: The vertices of the nested Dyck path from (-2n, 0) to (2n, 0) 

## PROJECT DEPENDENCY LGV.nestedDyckVertices_nonempty (theorem)
∀ (n : ℕ), LGV.nestedDyckVertices n ≠ []

## PROJECT DEPENDENCY LGV.nestedDyckVertices_arcs_valid (theorem)
∀ (n i : ℕ) (hi : i + 1 < (LGV.nestedDyckVertices n).length),
  LGV.dyckDigraph.arc ((LGV.nestedDyckVertices n).get ⟨i, ⋯⟩) ((LGV.nestedDyckVertices n).get ⟨i + 1, hi⟩)
...
## INFORMAL STATEMENT
Nested Dyck path construction

\leanhelper  For each $i \in \mathbb {N}$, define the \emph{nested Dyck path} $\gamma _i$ as the path in the Dyck digraph from $(-2i, 0)$ to $(2i, 0)$ that goes straight up (northeast) from $(-2i, 0)$ to $(0, 2i)$ and then straight down (southeast) from $(0, 2i)$ to $(2i, 0)$. This path has $4i$ arcs and visits the vertices $(j - 2i,\;  \min (j,\;  4i - j))$ for $j = 0, 1, \ldots , 4i$.
...
{
  "verdict": "faithful",
  "justification": "The formal construction matches every component of the blueprint. The binder `\u2115 \u2192 LGV.dyckDigraph.Path` corresponds to \u201cFor each i \u2208 \u2115\u201d and makes `\u03b3_i` a path in the Dyck digraph. Its vertices are exactly `List.finRange (4 * n + 1)` mapped to `(\u2191i - 2 * \u2191n, min i (4 * n - i))`, which is the stated sequence `(j - 2i, min (j, 4i - j))` for `j = 0, \u2026, 4i`. Thus there are `4i + 1` ordered vertices and hence `4i` consecutive arcs; the supplied `arcs_valid` field certifies that each consecutive pair is a Dyck-digraph arc. These coordinates rise northeast to `(0, 2i)` and then descend southeast. Finally, `nestedDyckPath_start` and `nestedDyckPath_finish` state exactly the endpoints `(-2i, 0)` and `(2i, 0)`. There are no additional hypotheses or narrowed quantifiers."
}