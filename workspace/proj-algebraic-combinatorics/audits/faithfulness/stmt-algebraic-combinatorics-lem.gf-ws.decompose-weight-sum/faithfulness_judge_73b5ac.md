## TARGET DominoTilingsZ.decomposeTiling_weight_sum (theorem) — ELABORATED SIGNATURE
∀ (n : ℕ) (T : DominoTilingsZ.Tiling (DominoTilingsZ.Rectangle n 2)),
  match DominoTilingsZ.decomposeTiling n T with
  | ⟨k, ts⟩ => ∑ i, (ts i).fst = n

Docstring: The sum of widths of the faultfree tilings equals the width of the original.
...
## INFORMAL STATEMENT
lem.gf-ws.decompose-weight-sum

\leanhelper  The sum of the widths of the faultfree tilings returned by the decomposition equals the original width $n$. That is, $\sum _{i} m_i = n$.

## VERDICT
{
  "verdict": "faithful",
  "justification": "The target quantifies over exactly the intended data, `\u2200 (n : \u2115) (T : Tiling (Rectangle n 2))`, and concludes that after `decomposeTiling n T = \u27e8k, ts\u27e9`, `\u2211 i, (ts i).fst = n`. By the dependency signature, each `ts i` has type `(m : \u2115) \u00d7 { T' // isFaultfree m T'}`, so `(ts i).fst` is precisely the component width `m_i`; and `i : Fin k` ranges over all returned components. Thus the formal equality is exactly the blueprint\u2019s `\u2211\u1d62 m_i = n`, with no added hypotheses or narrowed quantifiers."
}