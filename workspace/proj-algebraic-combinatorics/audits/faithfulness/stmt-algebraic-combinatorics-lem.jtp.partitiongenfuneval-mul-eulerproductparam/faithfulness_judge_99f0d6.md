## TARGET AlgebraicCombinatorics.partitionGenFunEval_mul_eulerProductParam (theorem) — ELABORATED SIGNATURE
∀ (a : ℤ) (u : ℚ),
  a > 0 → AlgebraicCombinatorics.partitionGenFunEval a u * AlgebraicCombinatorics.eulerProductParam a u = 1

Docstring: Key identity: `partitionGenFunEval * eulerProductParam = 1`.
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.eulerProductParam (def)
ℤ → ℚ → PowerSeries ℚ

Body:
fun a u => ∏' (k : ℕ), (1 - u ^ (2 * (k + 1)) • PowerSeries.X ^ (2 * a * (↑k + 1)).toNat)

Docstring: The Euler product with parameters: ∏_{k>0}(1 - u^{2k}·X^{2ak}).
...
## INFORMAL STATEMENT
lem.jtp.partitionGenFunEval-mul-eulerProductParam

\leanhelper  For $a > 0$, the partition generating function times the Euler product (both evaluated at parameter $a$, $u$) equals $1$: 

\[  \prod _{k>0}\frac{1}{1-u^{2k}x^{2ak}} \cdot \prod _{k>0}(1-u^{2k}x^{2ak}) = 1.  \]
...
{
  "verdict": "faithful",
  "justification": "The elaborated binders exactly encode the stated parameters and hypothesis: `\u2200 (a : \u2124) (u : \u211a), a > 0 \u2192 ... = 1`, matching \u201cFor $a>0$ ... evaluated at parameter $a$, $u$ ... equals $1$.\u201d The dependency `eulerProductParam` is the shifted natural-number product `\u220f' (k : \u2115), (1 - u^(2*(k+1)) \u2022 X^(2*a*(k+1)).toNat)`, which is precisely the product over positive integers. The body of `partitionGenFunEval` sums one term for every partition of `n`, giving coefficient `p(n)u^{2n}` at exponent `2an`, hence the evaluated partition generating function. Under the theorem\u2019s `a > 0` hypothesis, the uses of `.toNat` preserve the intended nonnegative exponents `2an` and `2a(k+1)`. Thus the asserted product is the displayed partition generating function times its Euler product, equal to `1`."
}