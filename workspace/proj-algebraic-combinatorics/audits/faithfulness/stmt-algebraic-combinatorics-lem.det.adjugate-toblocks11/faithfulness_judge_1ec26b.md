## TARGET AlgebraicCombinatorics.Determinants.adjugate_toBlocks₁₁_det_of_invertible (theorem) — ELABORATED SIGNATURE
∀ {m : Type u_3} {n : Type u_4} [inst : Fintype m] [inst_1 : Fintype n] [inst_2 : DecidableEq m]
  [inst_3 : DecidableEq n] {R : Type u_5} [inst_4 : CommRing R] (A : Matrix m m R) (B : Matrix m n R) (C : Matrix n m R)
  (D : Matrix n n R) [hD : Invertible D] [hSchur : Invertible (A - B * ⅟D * C)]
  [hM : Invertible (Matrix.fromBlocks A B C D)],
  Fintype.card m ≥ 1 →
    (Matrix.fromBlocks A B C D).adjugate.toBlocks₁₁.det = (Matrix.fromBlocks A B C D).det ^ (Fintype.card m - 1) * D.det

Docstring: Key identity: For invertible block matrices with invertible bottom-right block,
...
{l : Type u_1} →
  {m : Type u_2} → {n : Type u_3} → {o : Type u_4} → {α : Type u_12} → Matrix (n ⊕ o) (l ⊕ m) α → Matrix n l α
...
## INFORMAL STATEMENT
lem.det.adjugate-toBlocks11

\leanhelper  Let $M = \begin{pmatrix}  A 
...
\end{pmatrix}$ be an invertible block matrix with $D$ invertible and Schur complement $A - BD^{-1}C$ invertible. Let $k = |A|$ (the size of the top-left block) with $k \geq 1$. Then 
...
{
  "verdict": "faithful",
  "justification": "The elaborated signature matches every substantive hypothesis in the blueprint: `[CommRing R]` formalizes the fixed commutative ring; `[hD : Invertible D]`, `[hSchur : Invertible (A - B * \u215fD * C)]`, and `[hM : Invertible (Matrix.fromBlocks A B C D)]` correspond respectively to \u201cwith D invertible and Schur complement A - BD^{-1}C invertible\u201d and \u201cM ... invertible.\u201d The premise `Fintype.card m \u2265 1` is exactly `k \u2265 1` for `k = |A|`, since `A : Matrix m m R`. The conclusion `(Matrix.fromBlocks A B C D).adjugate.toBlocks\u2081\u2081.det = (Matrix.fromBlocks A B C D).det ^ (Fintype.card m - 1) * D.det` is precisely the claimed determinant identity. Quantifying over arbitrary finite index types `m` and `n` is at least as general as using natural-number-sized standard index sets, while the `DecidableEq` instances are representational requirements for matrix identity, determinant, and adjugate rather than additional mathematical restrictions."
}