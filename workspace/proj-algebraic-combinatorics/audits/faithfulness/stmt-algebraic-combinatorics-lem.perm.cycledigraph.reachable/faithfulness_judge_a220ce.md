## TARGET AlgebraicCombinatorics.cycleDigraph_connected_iff (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (σ : AlgebraicCombinatorics.Sn n) (i j : Fin n),
  (AlgebraicCombinatorics.cycleDigraph σ).Reachable i j ↔ ∃ k, (σ ^ k) i = j

Docstring: The connected components of the cycle digraph correspond to the orbits of σ. 
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.cycleDigraph (def)
...
## BASE-LIBRARY REF SimpleGraph.Reachable
{V : Type u} → SimpleGraph V → V → V → Prop

Docstring: Two vertices are *reachable* if there is a walk between them.
...
## INFORMAL STATEMENT
lem.perm.cycleDigraph.reachable

\leanhelper  Two vertices $i$ and $j$ are in the same connected component of the cycle digraph of $\sigma $ if and only if there exists $k \in \mathbb {Z}$ with $\sigma ^k(i) = j$.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies exactly over a permutation and two vertices, `\u2200 {n : \u2115} (\u03c3 : Sn n) (i j : Fin n)`, and states `(cycleDigraph \u03c3).Reachable i j \u2194 \u2203 k, (\u03c3 ^ k) i = j`. Here `Reachable` means that there is a graph walk, matching \u201cin the same connected component.\u201d The occurrence of `Int` together with `DivInvMonoid.toZPow` shows that the inferred witness `k` is an integer, matching \u201cthere exists `k \u2208 \u2124`.\u201d Finally, `cycleDigraph` has adjacency `i \u2260 j \u2227 (\u03c3 i = j \u2228 \u03c3 j = i)`, so its undirected walks precisely encode movement by positive or negative powers of `\u03c3`. The `Fin n` representation is explicitly sanctioned by the informal definition."
}