## TARGET AlgebraicCombinatorics.CauchyBinet.minor_empty (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n m : ℕ} (A : Matrix (Fin n) (Fin m) R),
  AlgebraicCombinatorics.CauchyBinet.minor A ∅ ∅ = 1

Docstring: The minor of the empty set is 1 (determinant of 0×0 matrix). 

## PROJECT DEPENDENCY AlgebraicCombinatorics.CauchyBinet.minor (def)
{R : Type u_1} → [CommRing R] → {n m : ℕ} → Matrix (Fin n) (Fin m) R → Finset (Fin n) → Finset (Fin m) → R

Body:
fun {R} [CommRing R] {n m} A U V =>
  if h : U.card = V.card then (A.submatrix ⇑(U.orderEmbOfFin ⋯) ⇑(V.orderEmbOfFin ⋯)).det else 0

Docstring: A **minor** of a matrix A is the determinant of a square submatrix.
...
\leanhelper  The minor of the empty sets is $1$: $\det (\operatorname {sub}_{\varnothing }^{\varnothing } A) = 1$.
...
{
  "verdict": "faithful",
  "justification": "The blueprint states \u201cThe minor of the empty sets is 1: det (sub_\u2205^\u2205 A) = 1.\u201d The target universally quantifies over a commutative ring, dimensions, and matrix\u2014\u201c\u2200 {R} [CommRing R] {n m} (A : Matrix (Fin n) (Fin m) R)\u201d\u2014and concludes \u201cminor A \u2205 \u2205 = 1.\u201d By the body of `minor`, since the two empty finsets have equal cardinality, `minor A \u2205 \u2205` is exactly the determinant of `A.submatrix` along their canonical increasing embeddings, i.e. the 0\u00d70 matrix `sub_\u2205^\u2205 A`. Thus the formal conclusion is precisely that this determinant equals 1. The `CommRing R` binder matches the algebraic setting required by the blueprint\u2019s determinant definition and adds no drift."
}