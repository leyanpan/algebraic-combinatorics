## TARGET AlgebraicCombinatorics.FPS.fraction_eq_iff (theorem) — ELABORATED SIGNATURE
∀ {L : Type u_1} [inst : CommRing L] (b c : L) (u : Lˣ), AlgebraicCombinatorics.FPS.fraction b u = c ↔ b = c * ↑u

Docstring: Alternative characterization: `b/a = c` iff `b = c * a`.
Label: def.commring.fracs (b) 

## PROJECT DEPENDENCY AlgebraicCombinatorics.FPS.fraction (def)
{L : Type u_1} → [inst : CommRing L] → L → Lˣ → L

Body:
fun {L} [CommRing L] b u => b * ↑u⁻¹

Docstring: The fraction `b/a` is defined as `b * a⁻¹` for a unit `a`.
Label: def.commring.fracs (b)
...
## INFORMAL STATEMENT
lem.commring.fraction-eq-iff

\leanhelper  For an invertible element $a$ and elements $b, c \in L$, we have $b / a = c$ if and only if $b = c \cdot a$.
...
{
  "verdict": "faithful",
  "justification": "The blueprint states: \u201cFor an invertible element $a$ and elements $b,c\\in L$, we have $b/a=c$ iff $b=c\\cdot a$.\u201d The formal binders `\u2200 {L : Type u_1} [CommRing L] (b c : L) (u : L\u02e3)` represent a commutative ring, two elements, and an invertible element bundled as a unit. By the dependency body, `fraction b u = b * \u2191u\u207b\u00b9`, exactly the informal definition of $b/a=b\\cdot a^{-1}$. Thus `fraction b u = c \u2194 b = c * \u2191u` is precisely the claimed equivalence, with `\u2191u` corresponding to $a$. Bundling invertibility as `u : L\u02e3` is an encoding of the stated assumption, not an added mathematical restriction."
}