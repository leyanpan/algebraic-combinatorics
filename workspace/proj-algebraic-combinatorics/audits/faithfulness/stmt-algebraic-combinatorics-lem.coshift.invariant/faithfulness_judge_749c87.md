## TARGET AlgebraicCombinatorics.CauchyBinet.sign_decomposition_leftCoShift_invariant (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (P Q : Finset (Fin n)) (hcard : P.card = Q.card) (σ : Equiv.Perm (Fin n))
  (hσ : AlgebraicCombinatorics.CauchyBinet.imageFinset σ P = Q) (i : Fin n) (hi : ↑i + 1 < n) (hi_notQ : i ∉ Q)
  (hiplus_Q : ⟨↑i + 1, hi⟩ ∈ Q),
  let s := Equiv.swap i ⟨↑i + 1, hi⟩;
  let Q' := AlgebraicCombinatorics.CauchyBinet.imageFinset s Q;
  let σ' := s * σ;
  have hcard' := ⋯;
  have hσ' := ⋯;
  ↑(Equiv.Perm.sign σ') =
...
Docstring: The sign decomposition formula is preserved under left co-shifts.
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.CauchyBinet.imageFinset_leftCoShift_eq (theorem)
    ...
    ∀ (i : Fin n) (hi : ↑i + 1 < n),
      i ∉ Q →
        ⟨↑i + 1, hi⟩ ∈ Q →
          ...
          have Q' := AlgebraicCombinatorics.CauchyBinet.imageFinset s Q;
          have σ' := s * σ;
          AlgebraicCombinatorics.CauchyBinet.imageFinset σ' P = Q'

Docstring: Left co-shift: when σ' = s * σ and Q' = s(Q), we have σ'(P) = Q'. 
...
## INFORMAL STATEMENT
Left co-shift invariance

\leanhelper  If the sign decomposition formula holds for $(P, Q', \sigma ')$ (where $Q' = s_i(Q)$, $\sigma ' = s_i\cdot \sigma $ for $i\notin Q$, $i{+}1\in Q$), then it holds for $(P, Q, \sigma )$.
...
## VERDICT
{
  "verdict": "faithful",
  "discrepancies": [],
  "justification": "The formal implication exactly realizes the stated invariance. It defines `s := Equiv.swap i \u27e8\u2191i + 1, hi\u27e9`, `Q' := imageFinset s Q`, and `\u03c3' := s * \u03c3`, matching \u201c`Q' = s_i(Q)`, `\u03c3' = s_i\u00b7\u03c3`.\u201d Permutation multiplication satisfies `(s * \u03c3) x = s (\u03c3 x)`, agreeing with the informal composition convention. The hypotheses `hi_notQ : i \u2209 Q` and `hiplus_Q : \u27e8\u2191i + 1, hi\u27e9 \u2208 Q` are precisely \u201c`i\u2209Q`, `i+1\u2208Q`\u201d; under these conditions, applying the swap to `Q` is the stated switch/co-shift. Finally, the conclusion is an implication from the sign decomposition equality for `(P,Q',\u03c3')` to the corresponding equality for `(P,Q,\u03c3)`, exactly as claimed. The bound `hi : \u2191i + 1 < n`, image condition `h\u03c3`, and cardinality evidence merely make the adjacent index and extracted permutations well-typed."
}