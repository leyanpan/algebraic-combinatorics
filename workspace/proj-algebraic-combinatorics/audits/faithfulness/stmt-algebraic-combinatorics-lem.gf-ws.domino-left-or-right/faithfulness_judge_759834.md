## TARGET DominoTilingsZ.domino_left_or_right_at_fault (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} {T : DominoTilingsZ.Tiling (DominoTilingsZ.Rectangle n 2)} {k : ℕ},
  DominoTilingsZ.hasFault n T k → ∀ d ∈ T.dominos, d.inLeftPart k ∨ d.inRightPart k

Docstring: At a fault, each domino is either entirely left or entirely right 
...
## INFORMAL STATEMENT
lem.gf-ws.domino-left-or-right

\leanhelper  At a fault position $k$ of a tiling $T$ of $R_{n,2}$, each domino of $T$ lies entirely in the left part (all $x$-coordinates $\le k$) or entirely in the right part (all $x$-coordinates $\ge k+1$).
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over exactly the stated data, `\u2200 {n : \u2115} {T : Tiling (Rectangle n 2)} {k : \u2115}`, assumes `hasFault n T k`, and concludes for every `d \u2208 T.dominos` that `d.inLeftPart k \u2228 d.inRightPart k`. The conclusion definitions exactly match the blueprint\u2019s parentheticals: `inLeftPart` means `\u2200 p \u2208 d.toShape, p.1 \u2264 \u2191k`, while `inRightPart` means `\u2200 p \u2208 d.toShape, p.1 \u2265 \u2191k + 1`. The body of `hasFault` expresses an internal fault (`k > 0 \u2227 k < n`) where a horizontal domino starts strictly left of `k` or at least at `k+1`; vertical dominos cannot straddle a vertical fault line. Thus it formalizes \u201cAt a fault position k ... each domino ... lies entirely in the left part ... or ... the right part,\u201d with no additional hypotheses."
}