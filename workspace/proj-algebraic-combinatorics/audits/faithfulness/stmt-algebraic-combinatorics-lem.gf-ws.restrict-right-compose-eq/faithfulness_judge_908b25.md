## TARGET DominoTilingsZ.restrictTilingRight_composeTilings_eq (theorem) — ELABORATED SIGNATURE
∀ (k : ℕ) (hk : k ≥ 2) (ts : Fin k → (m : ℕ) × { T' // DominoTilingsZ.isFaultfree m T' })
  (hfault :
    DominoTilingsZ.hasFault (DominoTilingsZ.composeTilings k ts).fst (DominoTilingsZ.composeTilings k ts).snd
      (ts ⟨0, ⋯⟩).fst),
  DominoTilingsZ.restrictTilingRight (DominoTilingsZ.composeTilings k ts).fst (DominoTilingsZ.composeTilings k ts).snd
      (ts ⟨0, ⋯⟩).fst hfault =
    ⋯ ▸ (DominoTilingsZ.composeTilings (k - 1) (DominoTilingsZ.faultfreeTilingsTail k ⋯ ts)).snd

Docstring: The right restriction of a composed tiling equals the composition of the tail.
...
## PROJECT DEPENDENCY DominoTilingsZ.partialWidthSum_one (theorem)
∀ (k : ℕ) (hk : k ≥ 2) (ts : Fin k → (m : ℕ) × { T' // DominoTilingsZ.isFaultfree m T' }),
  DominoTilingsZ.partialWidthSum k ts ⟨1, ⋯⟩ = (ts ⟨0, ⋯⟩).fst

Docstring: The partial width sum at index 1 equals the width of the first component 
...
## PROJECT DEPENDENCY DominoTilingsZ.faultfreeTilingsTail (def)
(k : ℕ) →
  k ≥ 1 →
    (Fin k → (m : ℕ) × { T' // DominoTilingsZ.isFaultfree m T' }) →
      Fin (k - 1) → (m : ℕ) × { T' // DominoTilingsZ.isFaultfree m T' }

Body:
fun k hk ts i => ts ⟨↑i + 1, ⋯⟩

Docstring: The tail of a tuple of faultfree tilings 

## PROJECT DEPENDENCY DominoTilingsZ.composeTilings_tail_width (theorem)
∀ (k : ℕ) (hk : k ≥ 2) (ts : Fin k → (m : ℕ) × { T' // DominoTilingsZ.isFaultfree m T' }),
  (DominoTilingsZ.composeTilings (k - 1) (DominoTilingsZ.faultfreeTilingsTail k ⋯ ts)).fst =
    (DominoTilingsZ.composeTilings k ts).fst - (ts ⟨0, ⋯⟩).fst

Docstring: The width of composing the tail equals the total width minus the first component width 
...
## INFORMAL STATEMENT
lem.gf-ws.restrict-right-compose-eq

\leanhelper  For $k \ge 2$, the right restriction of the composed tiling at the first boundary $m_1$, after shifting, equals the composition of the tail $(t_2, \ldots , t_k)$.

## VERDICT
{
  "verdict": "faithful",
  "justification": "The blueprint states: \u201cFor $k \\ge 2$, the right restriction of the composed tiling at the first boundary $m_1$, after shifting, equals the composition of the tail $(t_2,\\ldots,t_k)$.\u201d The formal binders match this setting: `\u2200 (k : \u2115) (hk : k \u2265 2) (ts : Fin k \u2192 (m : \u2115) \u00d7 { T' // isFaultfree m T' })`, and its conclusion identifies `restrictTilingRight ... (ts \u27e80, \u22ef\u27e9).fst` with `composeTilings (k - 1) (faultfreeTilingsTail k \u22ef ts)`, up to the necessary transport `\u22ef \u25b8` arising from equality of rectangle widths. The `hfault : hasFault ... (ts \u27e80, \u22ef\u27e9).fst` binder supplies the dependent proof required by `restrictTilingRight`; it encodes that the stated first boundary is a valid fault position rather than changing the mathematical equality. This is carried by the setting: each component is faultfree and has positive width by `isFaultfree`, and `composeTilings` concatenates the components, making their boundary a fault. Thus the declaration faithfully formalizes the stated right restriction and tail composition."
}