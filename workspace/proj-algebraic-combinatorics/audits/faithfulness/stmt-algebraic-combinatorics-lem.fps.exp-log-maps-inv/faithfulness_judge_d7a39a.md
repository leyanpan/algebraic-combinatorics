## TARGET PowerSeries.Exp_Log_inverse (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K],
  Function.LeftInverse PowerSeries.Log PowerSeries.Exp ∧ Function.RightInverse PowerSeries.Log PowerSeries.Exp

Docstring: `Exp` and `Log` are mutually inverse. This is Lemma 7.8.8 (lem.fps.Exp-Log-maps-inv). 

## TARGET PowerSeries.Log_Exp (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] (g : ↑PowerSeries.PowerSeries₀),
  PowerSeries.Log (PowerSeries.Exp g) = g

Docstring: `Log (Exp g) = g` for any `g ∈ K⟦X⟧₀`. This is part of Lemma 7.8.8 (lem.fps.Exp-Log-maps-inv). 

## TARGET PowerSeries.Exp_Log (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] (f : ↑PowerSeries.PowerSeries₁),
  PowerSeries.Exp (PowerSeries.Log f) = f
...
## BASE-LIBRARY REF Function.LeftInverse
{α : Sort u_1} → {β : Sort u_2} → (β → α) → (α → β) → Prop

Docstring: `LeftInverse g f` means that `g` is a left inverse to `f`. That is, `g ∘ f = id`. 
...
## BASE-LIBRARY REF Function.RightInverse
{α : Sort u_1} → {β : Sort u_2} → (β → α) → (α → β) → Prop

Docstring: `RightInverse g f` means that `g` is a right inverse to `f`. That is, `f ∘ g = id`. 
...
{
  "verdict": "faithful",
  "justification": "The blueprint assumes throughout that \u201cK is \u2026 a commutative \u211a-algebra,\u201d exactly represented by the binders `\u2200 {K} [CommRing K] [Algebra \u211a K]`. Its spaces are also matched definitionally: `PowerSeries\u2080 = {f | constantCoeff f = 0}` and `PowerSeries\u2081 = {f | constantCoeff f = 1}`. The target conjunction `Function.LeftInverse PowerSeries.Log PowerSeries.Exp \u2227 Function.RightInverse PowerSeries.Log PowerSeries.Exp` expands respectively to `\u2200 g : PowerSeries\u2080, Log (Exp g) = g` and `\u2200 f : PowerSeries\u2081, Exp (Log f) = f`, precisely saying that the two maps are mutually inverse. The component targets `PowerSeries.Log_Exp` and `PowerSeries.Exp_Log` state these same two identities separately. Since mutually inverse maps are bijections between their stated domains and codomains, this faithfully formalizes \u201cmutually inverse bijections between K[[x]]\u2080 and K[[x]]\u2081.\u201d"
}