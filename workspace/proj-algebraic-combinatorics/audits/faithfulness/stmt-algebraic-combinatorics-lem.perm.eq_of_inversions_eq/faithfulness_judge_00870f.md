## TARGET Equiv.Perm.eq_of_inversions_eq (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (σ τ : Equiv.Perm (Fin n)), (∀ (i j : Fin n), i < j → (σ j < σ i ↔ τ j < τ i)) → σ = τ

Docstring: Two permutations with the same inversions are equal.
...
## INFORMAL STATEMENT
lem.perm.eq_of_inversions_eq

\leanhelper  Two permutations $\sigma , \tau \in S_n$ with the same inversions are equal: if for all $i < j$ we have $\sigma (j) < \sigma (i) \iff \tau (j) < \tau (i)$, then $\sigma = \tau $.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over exactly the intended finite symmetric group: `\u2200 {n : \u2115} (\u03c3 \u03c4 : Equiv.Perm (Fin n))`, matching \u201ctwo permutations `\u03c3, \u03c4 \u2208 S_n`\u201d under the supplied convention that Lean uses `Fin n` in place of `[n]` and represents permutations by `Equiv.Perm`. Its hypothesis `(\u2200 (i j : Fin n), i < j \u2192 (\u03c3 j < \u03c3 i \u2194 \u03c4 j < \u03c4 i))` exactly matches \u201cfor all `i < j`, `\u03c3(j) < \u03c3(i) \u2194 \u03c4(j) < \u03c4(i)`,\u201d and its conclusion is precisely `\u03c3 = \u03c4`. There are no added hypotheses or restricted quantifiers beyond the definition-fixed Lean encoding."
}