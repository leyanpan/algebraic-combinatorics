## TARGET AlgebraicCombinatorics.MathlibComposition.card_compositions_of_length (theorem) — ELABORATED SIGNATURE
∀ (n k : ℕ), 0 < n → 0 < k → {c | c.length = k}.card = (n - 1).choose (k - 1)

Docstring: **Theorem thm.fps.comps.num-comps-n-k** (using Mathlib's Composition):
...
## INFORMAL STATEMENT
lem.fps.comps.card-compositions-of-length

\leanhelper  For $n > 0$ and $k > 0$, the number of compositions of $n$ into $k$ parts (using Mathlib’s composition type) equals $\binom {n-1}{k-1}$. The proof establishes a bijection between compositions of length $k$ and $(k-1)$-element subsets of $\{ 1, 2, \ldots , n-1\} $ via Lemma~ \ref{lem.fps.comps.compositionToFinset-card}, then uses the formula for counting subsets of a given size.
...
{
  "verdict": "faithful",
  "justification": "The binders `\u2200 (n k : \u2115), 0 < n \u2192 0 < k` exactly match \u201cFor $n>0$ and $k>0$,\u201d with both parameters natural as required by the definition of compositions of $n$ into $k$ parts. The filtered universal finset `{c | c.length = k}` ranges over `Composition n`; by the library definitions, `Composition n` consists of positive-integer lists summing to `n`, and `Composition.length` is their number of parts. Thus its `.card` is precisely \u201cthe number of compositions of $n$ into $k$ parts.\u201d Finally, `(n - 1).choose (k - 1)` is the natural-number binomial coefficient $\\binom{n-1}{k-1}$. Hence the formal equality states exactly the blueprint claim."
}