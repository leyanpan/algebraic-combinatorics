## TARGET AlgebraicCombinatorics.Determinants.det_laplace_multi_row (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {m : ℕ} (A : Matrix (Fin m) (Fin m) R) (P : Finset (Fin m)),
  A.det =
    ∑ Q ∈ AlgebraicCombinatorics.Determinants.sameCardSubsets m P,
      (-1) ^ (P.sum Fin.val + Q.sum Fin.val) * AlgebraicCombinatorics.Determinants.submatrixDet A P Q *
        AlgebraicCombinatorics.Determinants.submatrixDet A Pᶜ Qᶜ

Docstring: Theorem thm.det.laplace-multi (a):
...
## TARGET AlgebraicCombinatorics.Determinants.det_laplace_multi_col (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {m : ℕ} (A : Matrix (Fin m) (Fin m) R) (Q : Finset (Fin m)),
  A.det =
    ∑ P ∈ AlgebraicCombinatorics.Determinants.sameCardSubsets m Q,
      (-1) ^ (P.sum Fin.val + Q.sum Fin.val) * AlgebraicCombinatorics.Determinants.submatrixDet A P Q *
        AlgebraicCombinatorics.Determinants.submatrixDet A Pᶜ Qᶜ

Docstring: Theorem thm.det.laplace-multi (b):
For any subset Q of [n], det A = ∑_{P : |P|=|Q|} (-1)^(sum P + sum Q) det(sub_P^Q A) det(sub_{~P}^{~Q} A)
Label: thm.det.laplace-multi 

## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.sameCardSubsets (def)
(m : ℕ) → Finset (Fin m) → Finset (Finset (Fin m))

Body:
fun m P => {Q ∈ Finset.univ.powerset | Q.card = P.card}

Docstring: The set of column subsets Q with |Q| = |P|. 
...
{
  "verdict": "faithful",
  "justification": "Both declarations match parts (a) and (b). The row theorem quantifies over \u201c(P : Finset (Fin m))\u201d and sums over \u201cQ \u2208 sameCardSubsets m P\u201d; the dependency body \u201c{Q \u2208 Finset.univ.powerset | Q.card = P.card}\u201d is exactly the informal range \u201cQ \u2286 [n], |Q| = |P|\u201d. The column theorem symmetrically quantifies over Q and sums over equal-cardinality P. The factors use \u201csubmatrixDet A P Q\u201d and \u201csubmatrixDet A P\u1d9c Q\u1d9c\u201d; through \u201cfinsetToFin\u201d and \u201corderIsoOfFin\u201d, rows and columns are selected in increasing order, matching the informal definition of submatrices. Although Lean represents [n] by zero-based \u201cFin m\u201d while the blueprint uses {1,\u2026,n}, the exponent is unaffected: translating both equal-cardinality sets to one-based indices adds |P|+|Q|=2|P|, an even number, so the value of (-1)^(sum P + sum Q) is unchanged. The binders \u201c{R : Type u_1} [CommRing R] {m : \u2115}\u201d faithfully encode the fixed commutative ring K and natural n, and in fact quantify uniformly over all such rings. No additional mathematically substantive hypothesis or narrowed range is present."
}