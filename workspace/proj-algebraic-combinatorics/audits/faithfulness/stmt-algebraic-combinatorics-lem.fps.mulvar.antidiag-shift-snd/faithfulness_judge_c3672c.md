## TARGET AlgebraicCombinatorics.antidiag_shift_snd (theorem) — ELABORATED SIGNATURE
∀ {σ : Type u_2} [inst : DecidableEq σ] (i : σ) (m : σ →₀ ℕ),
  Finset.map { toFun := fun p => (p.1, p.2 + fun₀ | i => 1), inj' := ⋯ } (Finset.antidiagonal m) =
    {p ∈ Finset.antidiagonal (m + fun₀ | i => 1) | (fun₀ | i => 1) ≤ p.2}

Docstring: The map `(a, b) ↦ (a, b + single i 1)` gives a bijection from `antidiagonal m` to
...
## INFORMAL STATEMENT
lem.fps.mulvar.antidiag-shift-snd

\leanhelper  The map $(\mathbf{a}, \mathbf{b}) \mapsto (\mathbf{a}, \mathbf{b} + \mathbf{e}_i)$ gives a bijection from $\operatorname {antidiagonal}(\mathbf{m})$ to the subset of $\operatorname {antidiagonal}(\mathbf{m} + \mathbf{e}_i)$ consisting of pairs $(\mathbf{a}, \mathbf{b})$ with $\mathbf{e}_i \leq \mathbf{b}$.
...
{
  "verdict": "faithful",
  "justification": "The formal map is exactly `(p.1, p.2) \u21a6 (p.1, p.2 + single i 1)`, matching `(\u001b[1m\u001b[0ma, b) \u21a6 (a, b + e_i)`, with `fun\u2080 | i => 1` representing the unit finitely supported function `e_i`. The equality `Finset.map ... (Finset.antidiagonal m) = {p \u2208 Finset.antidiagonal (m + fun\u2080 | i => 1) | (fun\u2080 | i => 1) \u2264 p.2}` says that its image is precisely the stated subset. Because `Finset.map` is supplied an embedding, the map is injective, so this image equality expresses the claimed bijection. The binder `\u2200 {\u03c3 : Type u_2} [DecidableEq \u03c3] (i : \u03c3) (m : \u03c3 \u2192\u2080 \u2115)` is a more general finitely supported-index setting; `DecidableEq \u03c3` is an encoding requirement for the finite antidiagonal/single construction rather than a substantive added mathematical hypothesis."
}