## TARGET AlgebraicCombinatorics.DoublyInfinitePowerSeries.isLaurentSeries_ofLaurentSeries (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : Semiring K] (f : LaurentSeries K),
  (AlgebraicCombinatorics.DoublyInfinitePowerSeries.ofLaurentSeries f).IsLaurentSeries

Docstring: A Mathlib LaurentSeries satisfies our IsLaurentSeries predicate.
...
## TARGET AlgebraicCombinatorics.DoublyInfinitePowerSeries.ofLaurentSeries_toLaurentSeries (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : Semiring K] (f : AlgebraicCombinatorics.DoublyInfinitePowerSeries K) (hf : f.IsLaurentSeries),
  AlgebraicCombinatorics.DoublyInfinitePowerSeries.ofLaurentSeries (f.toLaurentSeries hf) = f

Docstring: The round-trip from DoublyInfinitePowerSeries to LaurentSeries and back gives the same function 

## TARGET AlgebraicCombinatorics.DoublyInfinitePowerSeries.toLaurentSeries_ofLaurentSeries (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : Semiring K] (f : LaurentSeries K),
  (AlgebraicCombinatorics.DoublyInfinitePowerSeries.ofLaurentSeries f).toLaurentSeries ⋯ = f

Docstring: The round-trip from LaurentSeries to DoublyInfinitePowerSeries and back gives the same series 

## PROJECT DEPENDENCY AlgebraicCombinatorics.DoublyInfinitePowerSeries.IsLaurentSeries (def)
{K : Type u_1} → [inst : Semiring K] → AlgebraicCombinatorics.DoublyInfinitePowerSeries K → Prop

Body:
fun {K} [Semiring K] f => ∃ N, ∀ n < N, f.coeff n = 0

Docstring: A doubly infinite power series is a Laurent series if the sequence of negative
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.DoublyInfinitePowerSeries.ofLaurentSeries (def)
{K : Type u_1} → [inst : Semiring K] → LaurentSeries K → AlgebraicCombinatorics.DoublyInfinitePowerSeries K

Body:
fun {K} [Semiring K] f n => f.coeff n

Docstring: Convert a Mathlib LaurentSeries to a DoublyInfinitePowerSeries 
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.DoublyInfinitePowerSeries.toLaurentSeries (def)
{K : Type u_1} →
  [inst : Semiring K] → (f : AlgebraicCombinatorics.DoublyInfinitePowerSeries K) → f.IsLaurentSeries → LaurentSeries K

Body:
fun {K} [Semiring K] f hf => { coeff := fun n => f.coeff n, isPWO_support' := ⋯ }

Docstring: Convert a DoublyInfinitePowerSeries satisfying IsLaurentSeries to a Mathlib LaurentSeries 
...
\leanhelper  Mathlib’s Laurent series satisfy the textbook definition (the sequence of negative-indexed coefficients is essentially finite). Moreover, the conversions between doubly infinite power series satisfying this condition and Mathlib’s Laurent series are inverse to each other.
...
{
  "verdict": "faithful",
  "justification": "The three declarations jointly match both clauses of the blueprint. First, `\u2200 ... (f : LaurentSeries K), (ofLaurentSeries f).IsLaurentSeries` says every Mathlib Laurent series satisfies the project predicate, whose body is exactly `\u2203 N, \u2200 n < N, f.coeff n = 0`, i.e. all sufficiently low integer-indexed coefficients vanish. Second, `\u2200 ... (f : DoublyInfinitePowerSeries K) (hf : f.IsLaurentSeries), ofLaurentSeries (f.toLaurentSeries hf) = f` gives one inverse direction on the series satisfying that condition. Third, `\u2200 ... (f : LaurentSeries K), (ofLaurentSeries f).toLaurentSeries \u22ef = f` gives the other inverse direction. The formal binders assume only `[Semiring K]`, whereas the textbook setting defines `K` as a commutative ring; this is a weaker hypothesis and hence a strictly more general, not weaker, theorem. No mathematically contentful extra hypothesis or narrowed quantifier is introduced."
}