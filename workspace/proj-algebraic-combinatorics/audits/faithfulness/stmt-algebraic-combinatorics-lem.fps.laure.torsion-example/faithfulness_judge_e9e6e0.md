## TARGET AlgebraicCombinatorics.torsion_example (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [Nontrivial K],
  ∃ p f, p ≠ 0 ∧ f ≠ 0 ∧ AlgebraicCombinatorics.laurentPolynomialSmul p f = 0

Docstring: The multiplication (1-x) · (∑_{n ∈ ℤ} x^n) = 0 shows that K[[x^±]] has torsion
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.DoublyInfinitePowerSeries.instAddCommMonoid (def)
{K : Type u_1} → [inst : Semiring K] → AddCommMonoid (AlgebraicCombinatorics.DoublyInfinitePowerSeries K)

Body:
fun {K} [Semiring K] => Pi.addCommMonoid

## PROJECT DEPENDENCY AlgebraicCombinatorics.laurentPolynomialSmul (def)
{K : Type u_1} →
  [inst : CommRing K] →
    LaurentPolynomial K →
      AlgebraicCombinatorics.DoublyInfinitePowerSeries K → AlgebraicCombinatorics.DoublyInfinitePowerSeries K

Body:
fun {K} [CommRing K] p f n => Finsupp.sum p fun i b => b * f (n - i)

Docstring: The action of a Laurent polynomial on a doubly infinite power series.
...
## BASE-LIBRARY REF Nontrivial
Type u_3 → Prop

Docstring: Predicate typeclass for expressing that a type is not reduced to a single element. In rings,
...
\leanhelper  The constant series $\mathbf{1} = (1, 1, 1, \ldots ) \in K[\! [x^{\pm }]\! ]$ satisfies $(1 - x) \cdot \mathbf{1} = 0$. This shows that the $K[x^{\pm }]$-module $K[\! [x^{\pm }]\! ]$ has torsion: a nonzero module element is annihilated by a nonzero ring element.
...
{
  "verdict": "drift",
  "justification": "The blueprint asserts the specific calculation: \u201cThe constant series \ud835\udfd9 = (1,1,1,\u2026) \u2026 satisfies (1 \u2212 x) \u00b7 \ud835\udfd9 = 0,\u201d and then its nonzero consequences. The target concludes only `\u2203 p f, p \u2260 0 \u2227 f \u2260 0 \u2227 laurentPolynomialSmul p f = 0`; it neither fixes `p` to `1 - x` nor `f` to the constant-one family. Thus it formalizes existence of some torsion witnesses, not the stated example. The difference enters in the target\u2019s existential binders `\u2203 p f`; these should be replaced or supplemented by the equation for the explicit Laurent polynomial `1-x` and series `fun _ : \u2124 => 1`, together with their nonzeroness. There is also an added binder `[Nontrivial K]`. The section assumption says that `K` is a commutative \u211a-algebra, while the supplied definitions of commutative ring and algebra do not require `0 \u2260 1`; hence `[Nontrivial K]` is a mathematically contentful restriction not licensed by those definitions. Although replacing the \u211a-algebra setting by the more general `[CommRing K]` would otherwise be acceptable, restricting it further to `[Nontrivial K]` excludes the trivial algebra. Faithfulness would require either carrying nontriviality in the blueprint\u2019s algebra/ring convention or stating the blueprint under its actual \u211a-algebra setting without this extra restriction."
}