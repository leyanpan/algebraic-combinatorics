## TARGET AlgebraicCombinatorics.Perm.longestElement_inv (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ}, (AlgebraicCombinatorics.Perm.longestElement n)⁻¹ = AlgebraicCombinatorics.Perm.longestElement n

Docstring: The inverse of the longest element is itself: w₀⁻¹ = w₀.

This follows from `longestElement_mul_self`.


## TARGET AlgebraicCombinatorics.Perm.longestElement_mul_self (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ}, AlgebraicCombinatorics.Perm.longestElement n * AlgebraicCombinatorics.Perm.longestElement n = 1

Docstring: The longest element w₀ is an involution: w₀ * w₀ = 1.

This follows from the fact that w₀ = Fin.revPerm and revPerm is an involution.


## TARGET AlgebraicCombinatorics.Perm.longestElement (def) — ELABORATED SIGNATURE
...
## TARGET AlgebraicCombinatorics.Perm.longestElement_apply (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (i : Fin n), (AlgebraicCombinatorics.Perm.longestElement n) i = ⟨n - 1 - ↑i, ⋯⟩

Docstring: Explicit computation of the longest element: w₀(i) = n - 1 - i.
...
## INFORMAL STATEMENT
lem.perm.longestElement

\leanhelper  The longest element $w_0 \in S_n$ is the reversal permutation $w_0(i) = n+1-i$. It is an involution: $w_0^2 = \operatorname {id}$ and $w_0^{-1} = w_0$.
...
{
  "verdict": "faithful",
  "justification": "The formal declarations collectively match all parts of the blueprint. The binder `\u2200 {n : \u2115}` agrees with the relevant informal setting \u201cLet n \u2208 \u2115\u201d from the inversion/length definition. The apparent formula difference is only the indexing convention: the blueprint uses `[n] = {1,\u2026,n}` and states `w\u2080(i) = n+1-i`, whereas `Fin n` consists of zero-based indices `0,\u2026,n-1`, so `longestElement_apply` correctly gives `n - 1 - \u2191i`. The body of `longestElement` defines precisely this reversal permutation. Finally, `longestElement_mul_self` states `w\u2080 * w\u2080 = 1`, corresponding to `w\u2080\u00b2 = id`, and `longestElement_inv` states `(w\u2080)\u207b\u00b9 = w\u2080`. There are no added mathematical hypotheses or narrowed index ranges beyond the faithful zero-based encoding of `S_n` as `Equiv.Perm (Fin n)`."
}