## TARGET AlgebraicCombinatorics.FPS.Laurent.kBoundedToFinsupp_sum (theorem) — ELABORATED SIGNATURE
∀ (k : ℕ) (f : Fin (k + 1) → ℤ),
  ∑ i ∈ (AlgebraicCombinatorics.FPS.Laurent.kBoundedToFinsupp k f).support,
      (AlgebraicCombinatorics.FPS.Laurent.kBoundedToFinsupp k f) i * 3 ^ i =
    AlgebraicCombinatorics.FPS.Laurent.kBoundedBTValue k f

Docstring: The sum over kBoundedToFinsupp equals kBoundedBTValue. 

## TARGET AlgebraicCombinatorics.FPS.Laurent.kBoundedToFinsupp (def) — ELABORATED SIGNATURE
(k : ℕ) → (Fin (k + 1) → ℤ) → ℕ →₀ ℤ

Body:
fun k f => Finsupp.onFinset (Finset.range (k + 1)) (fun i => if h : i < k + 1 then f ⟨i, h⟩ else 0) ⋯

Docstring: Helper to convert a k-bounded representation to a Finsupp. 
...
## BASE-LIBRARY REF Finsupp.onFinset
{α : Type u_1} → {M : Type u_4} → [inst : Zero M] → (s : Finset α) → (f : α → M) → (∀ (a : α), f a ≠ 0 → a ∈ s) → α →₀ M

Docstring: `Finsupp.onFinset s f hf` is the finsupp function representing `f` restricted to the finset `s`.
...
## INFORMAL STATEMENT
Conversion between bounded and finitely supported representations

\leanhelper  A $k$-bounded representation $f : \{ 0, 1, \ldots , k\}  \to \mathbb {Z}$ can be converted to a finitely supported function $\mathbb {N} \to \mathbb {Z}$ by extending with zeros. The weighted sum of the resulting function equals the $k$-bounded value.
...
{
  "verdict": "faithful",
  "justification": "The blueprint quantifies over \u201cA k-bounded representation f : {0,1,\u2026,k} \u2192 \u2124,\u201d matched exactly by the binders `\u2200 (k : \u2115) (f : Fin (k + 1) \u2192 \u2124)`. The conversion is extension by zero: `kBoundedToFinsupp` uses `Finset.range (k + 1)` and returns `f \u27e8i,h\u27e9` when `i < k + 1`, and `0` otherwise. The theorem\u2019s left side is the weighted sum of that finitely supported function over its support, while `kBoundedBTValue k f` is defined as `\u2211 i, f i * 3 ^ \u2191i`; thus the asserted equality is precisely that the weighted sum after zero-extension equals the k-bounded value. There are no added hypotheses or narrowed quantifiers."
}