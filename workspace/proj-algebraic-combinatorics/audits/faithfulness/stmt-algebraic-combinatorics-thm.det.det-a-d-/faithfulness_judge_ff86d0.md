## TARGET AlgebraicCombinatorics.CauchyBinet.det_add_diagonal (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (A : Matrix (Fin n) (Fin n) R) (d : Fin n → R),
  (A + Matrix.diagonal d).det = ∑ P, (A.submatrix ⇑(P.orderEmbOfFin ⋯) ⇑(P.orderEmbOfFin ⋯)).det * ∏ i ∈ Pᶜ, d i
...
{
  "verdict": "faithful",
  "justification": "The formal identity matches the blueprint term-for-term. The binders `\u2200 {R : Type u_1} [CommRing R] {n : \u2115} (A : Matrix (Fin n) (Fin n) R) (d : Fin n \u2192 R)` encode an `n \u00d7 n` matrix and the diagonal entries of `D`; by the documented body of `Matrix.diagonal`, `Matrix.diagonal d` is exactly the diagonal matrix with entries `d i`. The conclusion `(A + Matrix.diagonal d).det = \u2211 P, ... * \u220f i \u2208 P\u1d9c, d i` corresponds to `det(A+D) = \u2211_{P\u2286[n]} det(sub_P^P A) \u00b7 \u220f_{i\u2208[n]\\P} d_i`: the unbounded `\u2211 P` ranges over all `Finset (Fin n)`, `P\u1d9c` is complement in the finite universe, and `P.orderEmbOfFin` lists the elements of `P` increasingly, so the two identical reindexings in `A.submatrix` give the naturally ordered principal submatrix specified by the informal definition. Using `Fin n` rather than the one-based set `[n]` is only a canonical reindexing. The `[CommRing R]` binder supplies the algebraic setting required for the signed Leibniz determinant from the informal definition; it is an encoding requirement rather than an additional mathematical restriction. If the blueprint's conventional `K` denotes a field, the Lean theorem is moreover strictly more general, since every field is a commutative ring."
}