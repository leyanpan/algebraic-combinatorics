## TARGET SymmetricFunctions.mem_verticalNStripPartitions_of_isVerticalNStrip (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} (mu : SymmetricFunctions.NPartition N) (n : ℕ) (lam : SymmetricFunctions.NPartition N)
  (hcontained : mu ≤ lam),
  { outer := lam, inner := mu, contained := hcontained }.isVerticalNStrip n →
    lam ∈ SymmetricFunctions.verticalNStripPartitions mu n

Docstring: Simplified membership criterion for vertical strip partitions using SkewPartition API.
...
## PROJECT DEPENDENCY SymmetricFunctions.SkewPartition.isVerticalNStrip (def)
{N : ℕ} → SymmetricFunctions.SkewPartition N → ℕ → Prop

Body:
fun {N} s n => s.isVerticalStrip ∧ s.size = n

Docstring: A skew partition is a vertical n-strip if it is a vertical strip
with |Y(λ/μ)| = n.
...
## INFORMAL STATEMENT
lem.sf.mem-vertical-of-isVerticalNStrip

\leanhelper  If $\mu \subseteq \lambda $ and $\lambda /\mu $ is a vertical $n$-strip, then $\lambda $ belongs to the finite set of vertical $n$-strip partitions of $\mu $.
...
{
  "verdict": "faithful",
  "justification": "The target directly formalizes the implication: its binder `(hcontained : mu \u2264 lam)` matches \u201cIf \u03bc \u2286 \u03bb,\u201d since `NPartition.instLE` uses `partLE`, defined as `\u2200 i, mu.parts i \u2264 lam.parts i`. Its premise `{ outer := lam, inner := mu, contained := hcontained }.isVerticalNStrip n` matches \u201c\u03bb/\u03bc is a vertical n-strip\u201d: `isVerticalNStrip` is `isVerticalStrip \u2227 size = n`, where `isVerticalStrip` requires `lam.parts i \u2264 mu.parts i + 1` and the bundled skew partition already carries containment. Finally, the conclusion `lam \u2208 verticalNStripPartitions mu n` is exactly membership in the finite set described by the informal definition. The finset body enumerates componentwise values between `mu_i` and `mu_i + 1`, filters for a weakly decreasing partition and size difference `n`, so its meaning agrees with vertical n-strip partitions. There are no additional mathematically substantive hypotheses."
}