## TARGET PowerSeries.loder_eq_derivative_Log (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_2} [inst : Field R] [inst_1 : Algebra ℚ R] {f : PowerSeries R},
  PowerSeries.constantCoeff f = 1 →
    f.loder = (PowerSeries.derivative R) (PowerSeries.subst (f - 1) (PowerSeries.logbar R))

Docstring: The logarithmic derivative equals the derivative of the logarithm over ℚ-algebras.
This is Proposition 7.8.13 (prop.fps.loder.log). 

## PROJECT DEPENDENCY PowerSeries.loder (def)
{R : Type u_2} → [Field R] → PowerSeries R → PowerSeries R

Body:
fun {R} [Field R] f => (PowerSeries.derivative R) f * f⁻¹

Docstring: **Definition 7.8.12 (def.fps.loder.1)**: The logarithmic derivative.
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.fps.deriv
def.fps.deriv

Let $f\in K\left[ \left[ x\right] \right] $ be an FPS. Then, the \emph{derivative} $f^{\prime }$ of $f$ is an FPS defined as follows: Write $f$ as $f=\sum _{n\in \mathbb {N}}f_{n}x^{n}$ (with $f_{0},f_{1},f_{2},\ldots \in K$), and set

\[  f^{\prime }:=\sum _{n>0}nf_{n}x^{n-1}.  \]
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.fps.loder.1
def.fps.loder.1

In this definition, we do \textbf{not} use Convention \ref{conv.fps.exp.K-Q-alg}; thus, $K$ can be an arbitrary commutative ring. However, we set $K\left[\left[x\right]\right]_{1}=\left\{ f\in K\left[\left[x\right]\right]\  \mid \  \left[x^{0}\right]f=1\right\}  $. 

For any FPS $f\in K\left[\left[x\right]\right]_{1}$, we define the \emph{logarithmic derivative} $\operatorname {loder}f\in K\left[\left[ x\right]\right]$ to be the FPS $\frac{f^{\prime }}{f}$. (This is well-defined, since $f$ is easily seen to be invertible.)
...
{
  "verdict": "unstatable",
  "justification": "The blueprint quantifies over every commutative \u211a-algebra: \u201cLet K be a commutative \u211a-algebra.\u201d The target instead has `\u2200 {R : Type u_2} [Field R] [Algebra \u211a R]`, restricting the result to fields, which excludes general commutative \u211a-algebras such as polynomial rings. More decisively, this restriction enters through the dependency `PowerSeries.loder`, whose binder is `{R : Type u_2} \u2192 [Field R] \u2192 PowerSeries R \u2192 PowerSeries R`. Thus the blueprint\u2019s theorem cannot be stated for an arbitrary commutative \u211a-algebra using the supplied `loder` definition. To make it statable and faithful, `PowerSeries.loder` must be generalized to `[CommRing R]` on series with constant coefficient `1`\u2014for example, by taking `f` in the corresponding subtype or taking a proof `constantCoeff f = 1` and constructing its unit inverse. The target could then quantify over `[CommRing R] [Algebra \u211a R]` rather than `[Field R] [Algebra \u211a R]`."
}