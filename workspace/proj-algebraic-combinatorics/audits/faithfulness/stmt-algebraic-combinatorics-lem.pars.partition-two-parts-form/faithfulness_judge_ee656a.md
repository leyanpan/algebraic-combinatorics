## TARGET Nat.Partition.partition_two_parts_form (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (p : n.Partition), p.parts.card = 2 → ∃ b, 1 ≤ b ∧ 2 * b ≤ n ∧ p.parts = {n - b, b}

Docstring: A partition into 2 parts has the form {n-b, b} for some 1 ≤ b ≤ n/2. 
...
## BASE-LIBRARY REF Multiset.instInsert
{α : Type u_1} → Insert α (Multiset α)
...
## BASE-LIBRARY REF Multiset.instSingleton
{α : Type u_1} → Singleton α (Multiset α)

## INFORMAL STATEMENT
lem.pars.partition-two-parts-form

\leanhelper  Every partition of $n$ into $2$ parts has the form $\{ n-b,\, b\} $ for some $1\le b \le \lfloor n/2\rfloor $.
...
{
  "verdict": "faithful",
  "justification": "The blueprint says every partition of n \u201cinto 2 parts\u201d has the form `{n-b, b}` for some `1 \u2264 b \u2264 \u230an/2\u230b`. The formal hypothesis `p.parts.card = 2` expresses two parts, while the conclusion `\u2203 b, 1 \u2264 b \u2227 2 * b \u2264 n \u2227 p.parts = {n - b, b}` gives exactly that multiset form. For natural numbers, `2 * b \u2264 n` is equivalent to `b \u2264 \u230an/2\u230b`. Using `Nat.Partition` is faithful: its documented meaning is a multiset of positive integers summing to `n`; although the informal definition allows an integer index `n`, any integer admitting a partition is necessarily nonnegative, so negative indices add only vacuous cases."
}