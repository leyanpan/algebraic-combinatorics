## TARGET AlgebraicCombinatorics.Determinants.vandermonde_det_poly' (theorem) — ELABORATED SIGNATURE
∀ (m : ℕ),
  (AlgebraicCombinatorics.Determinants.vandermondePolyMat m).det =
    AlgebraicCombinatorics.Determinants.vandermondePolyProd m

Docstring: **Lemma lem.det.vander.a.pol**: The Vandermonde determinant in the polynomial ring.
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.vandermondePolyMat (def)
(m : ℕ) → Matrix (Fin m) (Fin m) (MvPolynomial (Fin m) ℤ)

Body:
fun m => Matrix.of fun i j => MvPolynomial.X i ^ (m - 1 - ↑j)

Docstring: The Vandermonde matrix in the polynomial ring ℤ[x₁,...,xₙ] with entries xᵢⁿ⁻ʲ.
This is the matrix from Lemma lem.det.vander.a.pol.
Entry (i,j) = xᵢ^{n-1-j} (using 0-indexed notation).
Label: lem.det.vander.a.pol 

## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.vandermondePolyProd (def)
(m : ℕ) → MvPolynomial (Fin m) ℤ

Body:
fun m => ∏ i, ∏ j ∈ Finset.Ioi i, (MvPolynomial.X i - MvPolynomial.X j)

Docstring: The product of differences ∏_{1≤i<j≤n} (xᵢ - xⱼ) in the polynomial ring.
Label: lem.det.vander.a.pol 
...
\begin{equation}  \det \left( \left( x_i^{n-j} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = \prod _{1 \leq i < j \leq n} (x_i - x_j).  \end{equation}
...
{
  "verdict": "faithful",
  "justification": "The blueprint quantifies \u201cLet n \u2208 \u2115,\u201d matching the target binder `\u2200 (m : \u2115)` with no added hypotheses. The dependency `vandermondePolyMat m` has entry `MvPolynomial.X i ^ (m - 1 - \u2191j)` for `i j : Fin m`; under the standard shift from zero-based indices `i,j` to the blueprint\u2019s one-based indices `i+1,j+1`, this is exactly `x_i^(n-j)`. Likewise, `vandermondePolyProd m` is `\u220f i, \u220f j \u2208 Finset.Ioi i, (X i - X j)`, and `Finset.Ioi i` selects precisely the indices `j` with `i < j`, matching `\u220f_{1\u2264i<j\u2264n} (x_i-x_j)`. The coefficient ring and variables are exactly `MvPolynomial (Fin m) \u2124`, representing `\u2124[x\u2081,\u2026,x\u2099]`, and `Matrix.det` is the determinant by the Leibniz formula. Thus the target equality states the same Vandermonde determinant identity, including the case `n = 0`."
}