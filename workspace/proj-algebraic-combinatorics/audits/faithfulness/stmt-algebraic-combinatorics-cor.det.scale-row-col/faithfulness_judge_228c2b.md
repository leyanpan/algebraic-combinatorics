## TARGET AlgebraicCombinatorics.Det.det_scale_rows (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {n : ℕ} (A : Matrix (Fin n) (Fin n) K) (d : Fin n → K),
  (Matrix.of fun i j => d i * A i j).det = (∏ i, d i) * A.det

Docstring: Scaling row i by d_i multiplies determinant by ∏ d_i.
Label: eq.cor.det.scale-row-col.row 

## TARGET AlgebraicCombinatorics.Det.det_scale_cols (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {n : ℕ} (A : Matrix (Fin n) (Fin n) K) (d : Fin n → K),
  (Matrix.of fun i j => d j * A i j).det = (∏ j, d j) * A.det

Docstring: Scaling column j by d_j multiplies determinant by ∏ d_j.
...
## INFORMAL STATEMENT
cor.det.scale-row-col

Let $n \in \mathbb {N}$. Let $A \in K^{n \times n}$ and $d_1, d_2, \ldots , d_n \in K$. Then, 

\begin{equation}  \det \left( \left( d_i A_{i,j} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = d_1 d_2 \cdots d_n \cdot \det A  \end{equation}

 and 

\begin{equation}  \det \left( \left( d_j A_{i,j} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = d_1 d_2 \cdots d_n \cdot \det A.  \end{equation}
...
{
  "verdict": "faithful",
  "justification": "The two declarations exactly formalize the two displayed equations. For rows, `det_scale_rows` states `(Matrix.of fun i j => d i * A i j).det = (\u220f i, d i) * A.det`, matching `det ((d_i A_{i,j})) = d_1 d_2 \u22ef d_n \u00b7 det A`. For columns, `det_scale_cols` states `(Matrix.of fun i j => d j * A i j).det = (\u220f j, d j) * A.det`, matching the analogous equation with `d_j A_{i,j}`. The binders `A : Matrix (Fin n) (Fin n) K` and `d : Fin n \u2192 K` encode an `n \u00d7 n` matrix and the family `d_1,\u2026,d_n`; `Fin n` is merely the zero-based formal indexing of the blueprint\u2019s `[n]`, and `\u220f i` ranges over all its elements. The hypothesis `[CommRing K]` agrees with the algebraic setting fixed through the supplied matrix/module and commutative-ring definitions. `Matrix.det` is explicitly the Leibniz-formula determinant, matching `def.det.det`."
}