## TARGET PowerSeries.derivative_expbar_comp (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] {g : PowerSeries K},
  PowerSeries.constantCoeff g = 0 →
    (PowerSeries.derivative K) (PowerSeries.subst g (PowerSeries.expbar K)) =
      PowerSeries.subst g (PowerSeries.exp K) * (PowerSeries.derivative K) g

Docstring: Chain rule for composition with `expbar`: `(expbar ∘ g)' = (exp ∘ g) · g'`.
This is Proposition 7.8.3(a) (prop.fps.exp-log-der).

Note that `expbar = exp - 1`, so `expbar' = exp' = exp`, and the chain rule gives
`(expbar ∘ g)' = (exp ∘ g) · g'`. 

## TARGET PowerSeries.derivative_logbar_comp (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : Field K] [inst_1 : Algebra ℚ K] {g : PowerSeries K},
  PowerSeries.constantCoeff g = 0 →
    (PowerSeries.derivative K) (PowerSeries.subst g (PowerSeries.logbar K)) = (1 + g)⁻¹ * (PowerSeries.derivative K) g

Docstring: Chain rule for composition with `logbar`: `(logbar ∘ g)' = (1+g)⁻¹ · g'`.
This is Proposition 7.8.3(b) (prop.fps.exp-log-der).
Note: Requires Field K for the inverse to exist. 

## TARGET PowerSeries.derivative_exp_comp (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] {g : PowerSeries K},
  PowerSeries.constantCoeff g = 0 →
    (PowerSeries.derivative K) (PowerSeries.subst g (PowerSeries.exp K)) =
      PowerSeries.subst g (PowerSeries.exp K) * (PowerSeries.derivative K) g

Docstring: Chain rule for composition with `exp`: `(exp ∘ g)' = (exp ∘ g) · g'`.
This is Proposition 7.8.3(a) (prop.fps.exp-log-der). 
...
Let $g\in K\left[\left[x\right]\right]$ with $\left[x^{0}\right]g=0$. Then: \medskip 
...
{
  "verdict": "drift",
  "justification": "Part (a) is faithfully covered by the two commutative-Q-algebra theorems. Both quantify over `\u2200 {K} [CommRing K] [Algebra \u211a K] {g : PowerSeries K}` with `constantCoeff g = 0`; `derivative_expbar_comp` proves `(expbar \u2218 g)' = (exp \u2218 g) \u00b7 g'`, while `derivative_exp_comp` proves `(exp \u2218 g)' = (exp \u2218 g) \u00b7 g'`. Together, and with `PowerSeries.expbar` defined as `PowerSeries.exp K - 1`, these give the chain of equalities in informal part (a).\n\nPart (b), however, adds a mathematically substantive restriction. The blueprint setting says, \u201cThroughout this section ... `K` is ... a commutative \u211a-algebra,\u201d whereas `derivative_logbar_comp` quantifies over `\u2200 {K} [Field K] [Algebra \u211a K]`. Requiring `[Field K]` excludes general commutative \u211a-algebras, even though `1 + g` has constant coefficient `1` and hence is invertible as a formal power series in the intended setting. The restriction enters through the target binder `[inst : Field K]`, driven by the available `PowerSeries.instInv : [Field k] \u2192 Inv (PowerSeries k)`. To be faithful, the logarithm theorem should instead use `[CommRing K] [Algebra \u211a K]` and a construction of the inverse of the unit `1 + g` valid over any commutative ring, or equivalently formulate the conclusion using a series explicitly characterized as that inverse rather than the field-dependent global `PowerSeries` inverse."
}