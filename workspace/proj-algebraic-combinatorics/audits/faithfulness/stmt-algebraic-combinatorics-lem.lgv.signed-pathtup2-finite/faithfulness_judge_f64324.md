## TARGET LGV1.signedPathTuples2_finite (theorem) — ELABORATED SIGNATURE
∀ (A A' B B' : LGV1.LatticePoint), (LGV1.signedPathTuples2 A A' B B').Finite

Docstring: The set of signed path tuples is finite. 

## TARGET LGV1.signedIpats2_finite (theorem) — ELABORATED SIGNATURE
∀ (A A' B B' : LGV1.LatticePoint), (LGV1.signedIpats2 A A' B B').Finite

Docstring: The set of intersecting signed path tuples is finite. 

## TARGET LGV1.signedNipats2_finite (theorem) — ELABORATED SIGNATURE
∀ (A A' B B' : LGV1.LatticePoint), (LGV1.signedNipats2 A A' B B').Finite

Docstring: The set of non-intersecting signed path tuples is finite. 
...
## PROJECT DEPENDENCY LGV1.signedPathTuples2 (def)
(A A' B B' : LGV1.LatticePoint) → Set (LGV1.SignedPathTuple2 A A' B B')

Body:
fun A A' B B' => Set.univ

Docstring: The set of all signed path tuples from (A,A') to (B,B') or (B',B). 

## PROJECT DEPENDENCY LGV1.signedIpats2 (def)
(A A' B B' : LGV1.LatticePoint) → Set (LGV1.SignedPathTuple2 A A' B B')

Body:
fun A A' B B' => {spt | spt.isIntersecting}

Docstring: The set of intersecting signed path tuples. 

## PROJECT DEPENDENCY LGV1.signedNipats2 (def)
(A A' B B' : LGV1.LatticePoint) → Set (LGV1.SignedPathTuple2 A A' B B')

Body:
fun A A' B B' => {spt | ¬spt.isIntersecting}

Docstring: The set of non-intersecting signed path tuples. 
...
## INFORMAL STATEMENT
lem.lgv.signed-pathtup2-finite

\leanhelper  The sets of signed path tuples (all, non-intersecting, and intersecting) are finite.
...
{
  "verdict": "faithful",
  "justification": "The three declarations exactly cover the blueprint\u2019s \u201csets of signed path tuples (all, non-intersecting, and intersecting)\u201d: `\u2200 (A A' B B' : LGV1.LatticePoint), (LGV1.signedPathTuples2 A A' B B').Finite`, together with the analogous statements for `signedNipats2` and `signedIpats2`. The quantification matches \u201cGiven four lattice points A, A', B, B'\u201d without added hypotheses. The dependency bodies also preserve the intended meanings: `signedPathTuples2 ... = Set.univ`, `signedIpats2 ... = {spt | spt.isIntersecting}`, and `signedNipats2 ... = {spt | \u00acspt.isIntersecting}`. A `SignedPathTuple2` consists of two east/north lattice paths with destinations `(B,B')` or `(B',B)` selected by a Boolean sign flag, matching the informal signed-path-tuple definition, while `isIntersecting` asserts that the two paths share a vertex. Thus all three intended sets are asserted finite with no mathematically contentful restriction or extra hypothesis."
}