## TARGET LGV1.LatticePath.coordSum_endpoint (theorem) — ELABORATED SIGNATURE
∀ (path : LGV1.LatticePath) (start : LGV1.LatticePoint),
  (path.endpoint start).coordSum = start.coordSum + ↑path.stepCount

Docstring: The coordinate sum increases by the step count along any path. 
...
## PROJECT DEPENDENCY LGV1.LatticePath.stepCount (def)
LGV1.LatticePath → ℕ

Body:
fun path => List.length path

Docstring: The number of steps in a path. 
...
## INFORMAL STATEMENT
lem.lgv.coordsum-endpoint

\leanhelper  For any lattice path $p$ starting at $A$, the coordinate sum of the endpoint satisfies 
...
{
  "verdict": "faithful",
  "justification": "The target quantifies without extra hypotheses over exactly \u201cany lattice path p starting at A\u201d: `\u2200 (path : LGV1.LatticePath) (start : LGV1.LatticePoint)`. Its conclusion, `(path.endpoint start).coordSum = start.coordSum + \u2191path.stepCount`, matches `cs(endpoint(p,A)) = cs(A) + \u2113(p)`. The dependency bodies realize the informal meanings: `LatticePoint = \u2124 \u00d7 \u2124`, `coordSum p = p.1 + p.2`, `stepCount path = List.length path`, and `endpoint` folds the path\u2019s east/north steps from the starting point. The cast `\u2191path.stepCount` merely embeds the natural-number length into \u2124 so it can be added to the integer coordinate sum; it adds no mathematical restriction."
}