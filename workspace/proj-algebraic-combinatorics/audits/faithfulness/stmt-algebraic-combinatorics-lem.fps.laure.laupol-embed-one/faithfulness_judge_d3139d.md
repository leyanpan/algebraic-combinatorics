## TARGET AlgebraicCombinatorics.FPS.Laurent.laurentPolynomialToSeries_zero (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K], AlgebraicCombinatorics.FPS.Laurent.laurentPolynomialToSeries 0 = 0

Docstring: The embedding sends 0 to 0. 

## TARGET AlgebraicCombinatorics.FPS.Laurent.laurentPolynomialToSeries_one (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K], AlgebraicCombinatorics.FPS.Laurent.laurentPolynomialToSeries 1 = 1

Docstring: The embedding sends 1 to 1. 
...
## BASE-LIBRARY REF HahnSeries.instOne
{Γ : Type u_1} →
  {R : Type u_3} → [Zero Γ] → [inst : PartialOrder Γ] → [inst_1 : Zero R] → [One R] → One (HahnSeries Γ R)
...
## INFORMAL STATEMENT
Embedding preserves 0 and 1

\leanhelper  The embedding sends $0 \in K[x^{\pm }]$ to $0 \in K((x))$ and $1 \in K[x^{\pm }]$ to $1 \in K((x))$.
...
{
  "verdict": "faithful",
  "justification": "The two signatures assert exactly the two parts of the blueprint: for every `{K : Type u_1} [inst : CommRing K]`, `laurentPolynomialToSeries 0 = 0` and `laurentPolynomialToSeries 1 = 1`. This matches \u201cThe embedding sends 0 \u2208 K[x^{\u00b1}] to 0 \u2208 K((x)) and 1 \u2208 K[x^{\u00b1}] to 1 \u2208 K((x)).\u201d The dependency has type `LaurentPolynomial K \u2192 LaurentSeries K` and body `p \u21a6 { coeff := \u21d1p, ... }`, so it is precisely the coefficientwise inclusion of Laurent polynomials into Laurent series. The `CommRing K` binder agrees with the blueprint\u2019s commutative-ring setting, and the informal definition explicitly identifies Mathlib Laurent series with the Hahn-series implementation used formally."
}