## TARGET AlgebraicCombinatorics.CauchyBinet.extractAlpha_spec (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (P Q : Finset (Fin n)) (hcard : P.card = Q.card) (σ : Equiv.Perm (Fin n))
  (hσ : AlgebraicCombinatorics.CauchyBinet.imageFinset σ P = Q) (j : Fin P.card),
  σ ((P.orderEmbOfFin ⋯) j) = (Q.orderEmbOfFin ⋯) ((AlgebraicCombinatorics.CauchyBinet.extractAlpha P Q hcard σ hσ) j)

Docstring: The key property of extractAlpha: σ(pⱼ) = q_{α(j)} where pⱼ and qⱼ are the j-th elements
of P and Q in sorted order. 

## TARGET AlgebraicCombinatorics.CauchyBinet.extractBeta_spec (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (P Q : Finset (Fin n)) (hcard : P.card = Q.card) (σ : Equiv.Perm (Fin n))
  (hσ : AlgebraicCombinatorics.CauchyBinet.imageFinset σ P = Q) (j : Fin Pᶜ.card),
  σ ((Pᶜ.orderEmbOfFin ⋯) j) = (Qᶜ.orderEmbOfFin ⋯) ((AlgebraicCombinatorics.CauchyBinet.extractBeta P Q hcard σ hσ) j)

Docstring: The key property of extractBeta: σ(pᶜⱼ) = qᶜ_{β(j)} where pᶜⱼ and qᶜⱼ are the j-th elements
...
\begin{enumerate} \item $\sigma (p_j) = q_{\alpha _\sigma (j)}$ for all $j$, 

\item $\sigma (p'_j) = q'_{\beta _\sigma (j)}$ for all $j$. 
...
{
  "verdict": "faithful",
  "justification": "The two declarations exactly formalize the two enumerated claims. Under the blueprint hypothesis \u201cFor \u03c3 with \u03c3(P) = Q\u201d and the extraction context \u201cP,Q \u2286 [n] with |P|=|Q|,\u201d both signatures bind `hcard : P.card = Q.card`, `\u03c3 : Equiv.Perm (Fin n)`, and `h\u03c3 : imageFinset \u03c3 P = Q`. For every `j : Fin P.card`, `extractAlpha_spec` states `\u03c3 ((P.orderEmbOfFin \u22ef) j) = (Q.orderEmbOfFin \u22ef) ((extractAlpha ... ) j)`, which is precisely `\u03c3(p_j) = q_{\u03b1_\u03c3(j)}` with increasing enumerations. Likewise, for every `j : Fin P\u1d9c.card`, `extractBeta_spec` states `\u03c3 ((P\u1d9c.orderEmbOfFin \u22ef) j) = (Q\u1d9c.orderEmbOfFin \u22ef) ((extractBeta ... ) j)`, precisely `\u03c3(p'_j) = q'_{\u03b2_\u03c3(j)}`. The use of `Fin n` and zero-based indices is explicitly licensed by the informal convention, while `orderEmbOfFin` supplies the required increasing order. No mathematically substantive extra hypothesis or restricted quantifier is introduced."
}