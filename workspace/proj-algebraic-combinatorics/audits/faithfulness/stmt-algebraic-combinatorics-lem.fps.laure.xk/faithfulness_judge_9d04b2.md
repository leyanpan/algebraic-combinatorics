## TARGET AlgebraicCombinatorics.FPS.Laurent.T_one_zpow (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] (k : ℤ), ↑(⋯.unit ^ k) = LaurentPolynomial.T k

Docstring: **The k-th power of x equals T(k) for all integers k**.
...
## TARGET AlgebraicCombinatorics.FPS.Laurent.T_coeff_eq (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] (k i : ℤ), (LaurentPolynomial.T k) i = if i = k then 1 else 0

Docstring: **The k-th power of T is the unit sequence at k**.
(Proposition prop.fps.laure.xk, label: prop.fps.laure.xk)
...
## BASE-LIBRARY REF LaurentPolynomial.isUnit_T
∀ {R : Type u_1} [inst : Semiring R] (n : ℤ), IsUnit (LaurentPolynomial.T n)
...
\leanhelper  For each $k \in \mathbb {Z}$, the Laurent polynomial $T(k)$ has coefficient $1$ at position $k$ and $0$ elsewhere: $T(k)_i = \delta _{i,k}$. Moreover, $T(1)^k = T(k)$ for all $k \in \mathbb {Z}$ (using integer powers via the unit structure).
...
{
  "verdict": "faithful",
  "justification": "The two declarations exactly cover the two clauses of the blueprint. `T_coeff_eq` quantifies `\u2200 {K} [CommRing K] (k i : \u2124)` and concludes `(LaurentPolynomial.T k) i = if i = k then 1 else 0`, which is precisely \u201cfor each k \u2208 \u2124 \u2026 T(k)\u1d62 = \u03b4\u1d62,\u2096.\u201d `T_one_zpow` quantifies `\u2200 {K} [CommRing K] (k : \u2124)` and concludes `\u2191(\u22ef.unit ^ k) = LaurentPolynomial.T k`; the left side uses the bundled unit associated to `T 1`, so `^ k` is an integer power via `Units.instDivInvMonoid`, exactly matching \u201cT(1)^k = T(k) \u2026 using integer powers via the unit structure.\u201d The `[CommRing K]` binder is the coefficient-ring setting fixed by the informal definitions, not an added hypothesis."
}