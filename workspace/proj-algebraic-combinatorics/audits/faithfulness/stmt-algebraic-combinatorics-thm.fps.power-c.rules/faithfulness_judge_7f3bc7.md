## TARGET AlgebraicCombinatorics.FPS.fpsPow_add (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] {f : PowerSeries K},
  AlgebraicCombinatorics.FPS.HasConstantTermOne f →
    ∀ (a b : K),
      AlgebraicCombinatorics.FPS.fpsPow f (a + b) =
        AlgebraicCombinatorics.FPS.fpsPow f a * AlgebraicCombinatorics.FPS.fpsPow f b

Docstring: Rule of exponents: f^{a+b} = f^a * f^b
...
## TARGET AlgebraicCombinatorics.FPS.fpsPow_mul (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] {f g : PowerSeries K},
  AlgebraicCombinatorics.FPS.HasConstantTermOne f →
    AlgebraicCombinatorics.FPS.HasConstantTermOne g →
      ∀ (a : K),
        AlgebraicCombinatorics.FPS.fpsPow (f * g) a =
          AlgebraicCombinatorics.FPS.fpsPow f a * AlgebraicCombinatorics.FPS.fpsPow g a

Docstring: Rule of exponents: (fg)^a = f^a * g^a
Label: eq.sec.gf.nips.rules-of-exps (second rule) 

## TARGET AlgebraicCombinatorics.FPS.fpsPow_pow (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] {f : PowerSeries K},
  AlgebraicCombinatorics.FPS.HasConstantTermOne f →
    ∀ (a b : K),
      AlgebraicCombinatorics.FPS.fpsPow (AlgebraicCombinatorics.FPS.fpsPow f a) b =
        AlgebraicCombinatorics.FPS.fpsPow f (a * b)

Docstring: Rule of exponents: (f^a)^b = f^{ab}
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.FPS.fpsPow (def)
{K : Type u_1} → [inst : CommRing K] → [Algebra ℚ K] → PowerSeries K → K → PowerSeries K

Body:
fun {K} [CommRing K] [Algebra ℚ K] f c => AlgebraicCombinatorics.FPS.fpsExp (c • AlgebraicCombinatorics.FPS.fpsLog f)

Docstring: The c-th power of an FPS f with constant term 1.
f^c := Exp(c * Log(f)) = exp.subst (c * log_series.subst (f-1))
Label: def.fps.power-c 

## PROJECT DEPENDENCY AlgebraicCombinatorics.FPS.fpsExp (def)
{K : Type u_1} → [inst : CommRing K] → [Algebra ℚ K] → PowerSeries K → PowerSeries K

Body:
fun {K} [CommRing K] [Algebra ℚ K] g => PowerSeries.subst g (PowerSeries.exp K)

Docstring: The Exp map: Exp(g) = exp(g) = (exp K).subst g
This is well-defined when g has constant term 0.
Label: def.fps.Exp-Log-maps 

## PROJECT DEPENDENCY AlgebraicCombinatorics.FPS.fpsLog (def)
{K : Type u_1} → [inst : CommRing K] → [Algebra ℚ K] → PowerSeries K → PowerSeries K

Body:
fun {K} [CommRing K] [Algebra ℚ K] f => PowerSeries.subst (f - 1) AlgebraicCombinatorics.FPS.logSeries

Docstring: The Log map: Log(f) = log_series(f-1) = logSeries.subst (f-1)
This is well-defined when f has constant term 1.
Label: def.fps.Exp-Log-maps 

## PROJECT DEPENDENCY AlgebraicCombinatorics.FPS.logSeries (def)
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.fps.power-c
def.fps.power-c
...
{
  "verdict": "faithful",
  "justification": "The three declarations exactly formalize the three asserted identities. The blueprint assumes \u201cK is a commutative \u211a-algebra,\u201d matched by `{K : Type u_1} [CommRing K] [Algebra \u211a K]`. Its conditions `f,g \u2208 K[[x]]\u2081` are matched by `HasConstantTermOne f` and `HasConstantTermOne g`, whose body is `PowerSeries.constantCoeff ... = 1`. Finally, `fpsPow f c` is defined as `fpsExp (c \u2022 fpsLog f)`, matching `f^c := Exp(c Log f)`. The conclusions respectively state `fpsPow f (a + b) = fpsPow f a * fpsPow f b`, `fpsPow (f * g) a = fpsPow f a * fpsPow g a`, and `fpsPow (fpsPow f a) b = fpsPow f (a * b)`, exactly corresponding to the three blueprint formulas. The second theorem does not quantify `b`, but `b` does not occur in that formula; collectively the declarations cover every `a,b,f,g` required by the blueprint."
}