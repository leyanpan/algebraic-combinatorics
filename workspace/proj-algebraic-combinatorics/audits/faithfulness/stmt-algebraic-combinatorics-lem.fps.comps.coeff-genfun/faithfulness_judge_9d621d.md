## TARGET AlgebraicCombinatorics.BoundedWeakComposition.coeff_genFun (theorem) — ELABORATED SIGNATURE
∀ (n k p : ℕ),
  (PowerSeries.coeff n) (AlgebraicCombinatorics.BoundedWeakComposition.genFun k p) =
    ↑(Fintype.card (AlgebraicCombinatorics.BoundedWeakComposition n k p))

Docstring: Key lemma: the coefficient of `x^n` in the generating function equals the count.
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.BoundedWeakComposition (def)
ℕ → ℕ → ℕ → Type

Body:
fun n k p => { α // ∑ i, ↑(α i) = n }

Docstring: A weak composition of `n` into `k` parts with entries bounded by `p`
(i.e., each entry is in `{0, 1, ..., p-1}`).

This is used in Theorem thm.fps.comps.num-wpcomps-n-k.


## PROJECT DEPENDENCY AlgebraicCombinatorics.BoundedWeakComposition.fintype (def)
(n k p : ℕ) → Fintype (AlgebraicCombinatorics.BoundedWeakComposition n k p)

Body:
fun n k p => Fintype.subtype {α | ∑ i, ↑(α i) = n} ⋯

Docstring: The set of bounded weak compositions is finite.
...
## INFORMAL STATEMENT
lem.fps.comps.coeff-genFun

\leanhelper  The coefficient of $x^n$ in the generating function $W_{k,p}$ equals the number of bounded weak compositions of $n$ into $k$ parts with entries in $\{ 0, 1, \ldots , p-1\} $.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over exactly the unrestricted natural parameters, `\u2200 (n k p : \u2115)`, and asserts `(PowerSeries.coeff n) (genFun k p) = \u2191(Fintype.card (BoundedWeakComposition n k p))`. The dependency `genFun` has body `(\u2211 i \u2208 Finset.range p, PowerSeries.X ^ i) ^ k`, i.e. the stated generating function whose allowed exponents are precisely `0, \u2026, p-1`. The type `BoundedWeakComposition n k p` is the subtype satisfying `\u2211 i, \u2191(\u03b1 i) = n`, and its docstring fixes the underlying objects as weak compositions into `k` parts with entries in `{0, 1, ..., p-1}`. Thus its `Fintype.card` is exactly the number described in the blueprint, cast into the integer coefficient ring. There are no added hypotheses or narrowed quantifiers."
}