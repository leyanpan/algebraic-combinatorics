## TARGET AlgebraicCombinatorics.fps_geometric_subst_fibonacci (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : Field K],
  PowerSeries.subst (PowerSeries.X + PowerSeries.X ^ 2) AlgebraicCombinatorics.geometricSeries =
    (1 - PowerSeries.X - PowerSeries.X ^ 2)⁻¹

Docstring: Substituting (x + x²) into the geometric series yields 1/(1 - x - x²).
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.geometricSeries (def)
...
## INFORMAL STATEMENT
lem.fps.geometric-subst-fibonacci

\leanhelper  Substituting $x + x^2$ into the geometric series $\frac{1}{1-x}$ yields the generating function for shifted Fibonacci numbers: 
...
{
  "verdict": "faithful",
  "justification": "The target asserts exactly the displayed blueprint identity. Its left side, `PowerSeries.subst (PowerSeries.X + PowerSeries.X ^ 2) AlgebraicCombinatorics.geometricSeries`, is substitution of `x + x\u00b2` into the geometric series, and the dependency defines `geometricSeries` as `(1 - PowerSeries.X)\u207b\u00b9`. Its right side is `(1 - PowerSeries.X - PowerSeries.X ^ 2)\u207b\u00b9`, matching `1/(1-x-x\u00b2)`. The binder `\u2200 {K : Type u_2} [Field K]` agrees with the informal geometric-series definition\u2019s setting \u201cwhere K is a field\u201d; it adds no unsupported restriction. The reference to shifted Fibonacci numbers is descriptive of the displayed generating function rather than an additional quantified coefficient assertion."
}