## TARGET AlgebraicCombinatorics.Perm.inv_one (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ}, AlgebraicCombinatorics.Perm.inv 1 = ∅

Docstring: The identity permutation has no inversions.

The identity preserves order, so there cannot be a pair (i, j) with i < j and 1(i) > 1(j).


## TARGET AlgebraicCombinatorics.Perm.invCount_one (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ}, AlgebraicCombinatorics.Perm.invCount 1 = 0

Docstring: The identity permutation has length 0.
...
\leanhelper  The identity permutation $\operatorname {id} \in S_n$ has no inversions: $\operatorname {inv}(\operatorname {id}) = \emptyset $ and $\ell (\operatorname {id}) = 0$.
...
{
  "verdict": "faithful",
  "justification": "The two declarations exactly formalize the two conjuncts of the blueprint. For every natural `n`, `AlgebraicCombinatorics.Perm.inv 1 = \u2205` states that the identity permutation has no pairs satisfying the definition `p.1 < p.2 \u2227 1 p.1 > 1 p.2`, matching \u201c`inv(id) = \u2205`.\u201d Likewise, `AlgebraicCombinatorics.Perm.invCount 1 = 0`, together with `invCount \u03c3 := (inv \u03c3).card`, matches \u201c`\u2113(id) = 0`.\u201d The formal realization of `[n]` as `Fin n` uses labels `0,\u2026,n\u22121` rather than `1,\u2026,n`, but this is an order-isomorphic encoding and does not alter the identity\u2019s inversion set or length. There are no added hypotheses or narrowed quantifiers."
}