## TARGET AlgebraicCombinatorics.InclusionExclusion.card_functions_avoiding_set (theorem) — ELABORATED SIGNATURE
∀ (m n : ℕ) (I : Finset (Fin n)), Fintype.card { f // ∀ i ∈ I, i ∉ Set.range f } = (n - I.card) ^ m

Docstring: Count functions from `Fin m` to `Fin n` whose range avoids a given set `I`. 
...
## INFORMAL STATEMENT
Cardinality of functions avoiding a set

\leanhelper  Let $m, n \in \mathbb {N}$ and let $I$ be a subset of $[n]$. The number of functions $f\colon [m] \to [n]$ such that $i \notin \operatorname {im} f$ for all $i \in I$ is $(n - |I|)^m$.
...
{
  "verdict": "faithful",
  "justification": "The formal binders `\u2200 (m n : \u2115) (I : Finset (Fin n))` match \u201cLet m, n \u2208 \u2115 and let I be a subset of [n],\u201d using the stipulated replacement of `[n]` by `Fin n`. The subtype `{ f // \u2200 i \u2208 I, i \u2209 Set.range f }` consists exactly of functions `f : Fin m \u2192 Fin n` whose image omits every `i \u2208 I`, and `Fintype.card` counts those functions. The equality to `(n - I.card) ^ m` is precisely the claimed `(n - |I|)^m`. No additional mathematically substantive hypotheses or restricted quantifiers are present."
}