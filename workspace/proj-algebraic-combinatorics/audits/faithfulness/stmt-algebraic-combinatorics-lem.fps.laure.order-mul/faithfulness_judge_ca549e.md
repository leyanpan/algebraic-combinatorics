## TARGET AlgebraicCombinatorics.FPS.Laurent.coeff_mul_eq_zero_of_lt_order (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] (f g : LaurentSeries K),
  ∀ n < HahnSeries.order f + HahnSeries.order g, f ≠ 0 → g ≠ 0 → (f * g).coeff n = 0

Docstring: The product of two Laurent series has coefficients zero below the sum of their orders.
...
## TARGET AlgebraicCombinatorics.FPS.Laurent.order_mul_ge (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] (f g : LaurentSeries K),
  HahnSeries.orderTop f + HahnSeries.orderTop g ≤ HahnSeries.orderTop (f * g)

Docstring: If `f` and `g` are nonzero Laurent series with orders `p` and `q` respectively,
...
## BASE-LIBRARY REF HahnSeries.order
{Γ : Type u_1} → {R : Type u_3} → [inst : PartialOrder Γ] → [inst_1 : Zero R] → [Zero Γ] → HahnSeries Γ R → Γ

Docstring: The order of a nonzero Hahn series `x` is a minimal element of `Γ` where `x` has a
...
## BASE-LIBRARY REF WithTop
Type u_2 → Type u_2

Docstring: Attach `⊤` to a type. 

## BASE-LIBRARY REF Preorder.toLE
{α : Type u_2} → [self : Preorder α] → LE α

## BASE-LIBRARY REF WithTop.instPreorder
{α : Type u_1} → [Preorder α] → Preorder (WithTop α)

## BASE-LIBRARY REF PartialOrder.toPreorder
{α : Type u_2} → [self : PartialOrder α] → Preorder α

## BASE-LIBRARY REF WithTop.add
{α : Type u} → [Add α] → Add (WithTop α)

## BASE-LIBRARY REF HahnSeries.orderTop
{Γ : Type u_1} → {R : Type u_3} → [inst : PartialOrder Γ] → [inst_1 : Zero R] → HahnSeries Γ R → WithTop Γ

Docstring: The orderTop of a Hahn series `x` is a minimal element of `WithTop Γ` where `x` has a nonzero
coefficient if `x ≠ 0`, and is `⊤` when `x = 0`. 

## INFORMAL STATEMENT
Order of product

\leanhelper  If $f$ and $g$ are Laurent series, then $\operatorname {ord}(f) + \operatorname {ord}(g) \le \operatorname {ord}(f \cdot g)$. In particular, the coefficients of $f \cdot g$ vanish below $\operatorname {ord}(f) + \operatorname {ord}(g)$.
...
{
  "verdict": "drift",
  "justification": "`order_mul_ge` faithfully states the main inequality for all Laurent series: its binders are `\u2200 ... (f g : LaurentSeries K), orderTop f + orderTop g \u2264 orderTop (f * g)`. Using `HahnSeries.orderTop`, which assigns `\u22a4` to zero, appropriately covers the blueprint\u2019s unrestricted \u201cIf f and g are Laurent series\u201d. However, `coeff_mul_eq_zero_of_lt_order` narrows the blueprint\u2019s unrestricted consequence by adding the binders `f \u2260 0 \u2192 g \u2260 0 \u2192`. The informal statement simply says \u201cthe coefficients of f \u00b7 g vanish below ord(f) + ord(g)\u201d and imposes no nonzeroness hypotheses. It also uses `HahnSeries.order`, whose zero convention is `order 0 = 0`, rather than the `orderTop` convention used by the main inequality. To make this declaration faithful to the stated consequence, remove the two nonzero binders and formulate the threshold consistently using `orderTop`, e.g. require `(n : WithTop \u2124) < orderTop f + orderTop g`; then the zero cases are included automatically."
}