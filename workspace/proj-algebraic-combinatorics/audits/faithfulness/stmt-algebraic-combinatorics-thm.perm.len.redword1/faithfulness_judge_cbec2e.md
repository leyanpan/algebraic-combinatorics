## TARGET Equiv.Perm.exists_reduced_word (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (σ : Equiv.Perm (Fin n)), ∃ w, Equiv.Perm.IsReduced w ∧ Equiv.Perm.wordProd w = σ

Docstring: **Theorem thm.perm.len.redword1 (a)**: Every permutation can be written as a
...
## TARGET Equiv.Perm.length_le_word_length (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (w : Equiv.Perm.Word n), (Equiv.Perm.wordProd w).length ≤ List.length w

Docstring: **Theorem thm.perm.len.redword1 (b)**: The length ℓ(σ) is the minimum number
...
## INFORMAL STATEMENT
First reduced word theorem for the symmetric group

Let $n \in \mathbb {N}$ and $\sigma \in S_n$. Then: \medskip 

\textbf{(a)} We can write $\sigma $ as a composition (i.e., product) of $\ell (\sigma )$ simples. \medskip 

\textbf{(b)} The number $\ell (\sigma )$ is the smallest $p \in \mathbb {N}$ such that we can write $\sigma $ as a composition of $p$ simples. \medskip 

\textbf{Keep in mind:} The composition of $0$ simples is $\mathrm{id}$, since $\mathrm{id}$ is the neutral element of the group $S_n$.
...
{
  "verdict": "faithful",
  "justification": "Part (a) is expressed by `\u2200 {n : \u2115} (\u03c3 : Equiv.Perm (Fin n)), \u2203 w, Equiv.Perm.IsReduced w \u2227 Equiv.Perm.wordProd w = \u03c3`. Expanding `IsReduced`, the witness satisfies `List.length w = (wordProd w).length`; using `wordProd w = \u03c3`, it is a product of exactly `\u03c3.length` simple transpositions. Here `length \u03c3 = \u03c3.inversions.card`, matching the blueprint\u2019s \u2113(\u03c3), and `Word n = List (Fin (n - 1))` indexes exactly the adjacent transpositions (with the harmless zero-based relabeling of `[n]` as `Fin n`). Part (b), `\u2200 {n : \u2115} (w : Equiv.Perm.Word n), (Equiv.Perm.wordProd w).length \u2264 List.length w`, is a more general formulation of the required lower bound: whenever a word `w` of length `p` represents a fixed `\u03c3`, it yields `\u2113(\u03c3) \u2264 p`. Combined with part (a), which supplies a representation with `p = \u2113(\u03c3)`, this says precisely that \u2113(\u03c3) is the smallest possible number of simples. The empty list has product `1`, so the stated zero-simple identity case is also represented."
}