## TARGET AlgebraicCombinatorics.CycleDecomposition.cycleOf_mem_cycles (theorem) — ELABORATED SIGNATURE
∀ {α : Type u_1} [inst : DecidableEq α] [inst_1 : Fintype α] (σ : Equiv.Perm α),
  ∀ x ∈ σ.support, σ.cycleOf x ∈ AlgebraicCombinatorics.CycleDecomposition.cycles σ

Docstring: The cycle of σ containing x (when x is not a fixed point) is in `cycles σ`. 
...
## BASE-LIBRARY REF Equiv.Perm.cycleOf
{α : Type u_2} → (f : Equiv.Perm α) → [DecidableRel f.SameCycle] → α → Equiv.Perm α

Docstring: `f.cycleOf x` is the cycle of the permutation `f` to which `x` belongs. 

## BASE-LIBRARY REF Equiv.Perm.instDecidableRelSameCycle
{α : Type u_2} → [DecidableEq α] → [Fintype α] → (f : Equiv.Perm α) → DecidableRel f.SameCycle
...
## INFORMAL STATEMENT
lem.cycleOf_mem_cycles

\leanhelper  If $x$ is not a fixed point of $\sigma $ (i.e., $\sigma (x) \neq x$), then the cycle of $\sigma $ containing $x$ is among the cycles of $\sigma $.
...
{
  "verdict": "faithful",
  "justification": "The binder `\u2200 x \u2208 \u03c3.support` exactly encodes the informal hypothesis \u201c\u03c3(x) \u2260 x,\u201d since `Equiv.Perm.support` is documented as \u201cThe Finset of nonfixed points of a permutation.\u201d The conclusion `\u03c3.cycleOf x \u2208 ...cycles \u03c3` likewise states that the cycle containing `x` is among the cycles, with `cycles \u03c3` defined as `\u03c3.cycleFactorsFinset`. Although the informal definition says the cycles of \u03c3 include 1-cycles while the project definition notes that `cycles` includes only cycles of length \u2265 2, this causes no drift here: under `x \u2208 \u03c3.support`, the cycle containing `x` is nontrivial, so it belongs to both notions. The `[Fintype \u03b1]` binder matches the informal finite-set setting, and `[DecidableEq \u03b1]` is computational encoding needed by the finset-based definitions rather than an additional mathematical hypothesis."
}