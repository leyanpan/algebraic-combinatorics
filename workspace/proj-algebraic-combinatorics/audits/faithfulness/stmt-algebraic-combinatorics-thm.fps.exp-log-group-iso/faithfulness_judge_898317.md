## TARGET PowerSeries.Exp_Log_groupIso (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K],
  (Function.LeftInverse PowerSeries.Log PowerSeries.Exp ∧ Function.RightInverse PowerSeries.Log PowerSeries.Exp) ∧
    (∀ (f g : ↑PowerSeries.PowerSeries₀),
        PowerSeries.Exp ⟨↑f + ↑g, ⋯⟩ = ⟨↑(PowerSeries.Exp f) * ↑(PowerSeries.Exp g), ⋯⟩) ∧
      PowerSeries.Exp ⟨0, ⋯⟩ = ⟨1, ⋯⟩

Docstring: **Theorem 7.8.11** (thm.fps.Exp-Log-group-iso): The maps `Exp` and `Log` are mutually inverse
...
## PROJECT DEPENDENCY PowerSeries.PowerSeries₁.mul_mem (theorem)
∀ {R : Type u_2} [inst : CommRing R] {f g : PowerSeries R},
  f ∈ PowerSeries.PowerSeries₁ → g ∈ PowerSeries.PowerSeries₁ → f * g ∈ PowerSeries.PowerSeries₁
...
## PROJECT DEPENDENCY PowerSeries.PowerSeries₁.one_mem (theorem)
∀ {R : Type u_2} [inst : CommRing R], 1 ∈ PowerSeries.PowerSeries₁

Docstring: `R⟦X⟧₁` is closed under multiplication and division and contains 1.
...
{
  "verdict": "faithful",
  "justification": "The blueprint says that `Exp` and `Log` \u201care mutually inverse group isomorphisms\u201d between `(K[[x]]\u2080,+,0)` and `(K[[x]]\u2081,\u00b7,1)`. The formal conjunction `Function.LeftInverse PowerSeries.Log PowerSeries.Exp \u2227 Function.RightInverse PowerSeries.Log PowerSeries.Exp` states both `Log (Exp f) = f` and `Exp (Log g) = g`. It also states the homomorphism laws `\u2200 (f g : \u2191PowerSeries.PowerSeries\u2080), Exp \u27e8\u2191f + \u2191g, \u22ef\u27e9 = \u27e8\u2191(Exp f) * \u2191(Exp g), \u22ef\u27e9` and `Exp \u27e80, \u22ef\u27e9 = \u27e81, \u22ef\u27e9`. Thus `Exp` is a bijective group homomorphism with inverse `Log`; the corresponding homomorphism and identity laws for `Log` follow from mutual inverseness. The binders `[CommRing K] [Algebra \u211a K]` match the stated section convention that `K` is a commutative \u211a-algebra, and the bodies of `PowerSeries\u2080`, `PowerSeries\u2081`, `Exp`, and `Log` match the informal definitions."
}