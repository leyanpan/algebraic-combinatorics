## TARGET AlgebraicCombinatorics.SignedCounting.switch_switch (theorem) — ELABORATED SIGNATURE
∀ (i : ℕ) (S : Finset ℕ),
  AlgebraicCombinatorics.SignedCounting.switch i (AlgebraicCombinatorics.SignedCounting.switch i S) = S

Docstring: The switch operation is an involution. 

## PROJECT DEPENDENCY AlgebraicCombinatorics.SignedCounting.switch (def)
ℕ → Finset ℕ → Finset ℕ
...
## INFORMAL STATEMENT
lem.switch_involution

\leanhelper  The switch operation $\operatorname {switch}_i$ is an involution, i.e., $\operatorname {switch}_i(\operatorname {switch}_i(S)) = S$ for all finite sets $S$.
...
{
  "verdict": "faithful",
  "justification": "The informal statement asserts that for every natural index i and every finite subset S of \u2115, \u201cswitch_i(switch_i(S)) = S.\u201d The elaborated target has exactly these binders and conclusion: \u201c\u2200 (i : \u2115) (S : Finset \u2115), switch i (switch i S) = S.\u201d The dependency `switch` also matches the informal definition exactly: it returns `symmDiff S {i, i + 1}` when `(S \u2229 {i, i + 1}).card = 1`, and otherwise returns `S`. There are no added hypotheses or narrowed quantifiers."
}