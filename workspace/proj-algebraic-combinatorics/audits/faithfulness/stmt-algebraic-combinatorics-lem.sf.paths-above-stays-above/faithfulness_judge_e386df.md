## TARGET SymmetricFunctions.paths_above_at_x_stays_above (theorem) — ELABORATED SIGNATURE
∀ (p p' : LGV.integerLattice.Path),
  (∀ v ∈ p.vertices, v ∉ p'.vertices) →
    ∀ (x₀ : ℤ),
      (∃ idx, ∃ (hidx : idx < p.vertices.length), (p.vertices.get ⟨idx, hidx⟩).1 = x₀) →
        (∃ idx, ∃ (hidx : idx < p'.vertices.length), (p'.vertices.get ⟨idx, hidx⟩).1 = x₀) →
          (∀ (idx_p : ℕ) (hidx_p : idx_p < p.vertices.length) (idx_p' : ℕ) (hidx_p' : idx_p' < p'.vertices.length),
            ...
            ∀ x₁ ≥ x₀,
              (∃ idx, ∃ (hidx : idx < p.vertices.length), (p.vertices.get ⟨idx, hidx⟩).1 = x₁) →
                (∃ idx, ∃ (hidx : idx < p'.vertices.length), (p'.vertices.get ⟨idx, hidx⟩).1 = x₁) →
                  ∀ (idx_p : ℕ) (hidx_p : idx_p < p.vertices.length) (idx_p' : ℕ)
                    (hidx_p' : idx_p' < p'.vertices.length),
                    (p.vertices.get ⟨idx_p, hidx_p⟩).1 = x₁ →
                      (p'.vertices.get ⟨idx_p', hidx_p'⟩).1 = x₁ →
                        (p'.vertices.get ⟨idx_p', hidx_p'⟩).2 > (p.vertices.get ⟨idx_p, hidx_p⟩).2

Docstring: Key lemma: for two non-intersecting paths, if one is above the other at some x-coordinate,
...
## INFORMAL STATEMENT
lem.sf.paths-above-stays-above

\leanhelper  If two non-intersecting lattice paths $p$ and $p'$ have starting $y$-coordinates satisfying $y_{\mathrm{start}}(p) < y_{\mathrm{start}}(p')$, and at some column $x_0$ the path $p$ is weakly above $p'$ (in $y$-coordinate), then $p$ stays weakly above $p'$ at all subsequent columns. 
...
{
  "verdict": "drift",
  "justification": "The vertical order is reversed. The blueprint assumes that at `x\u2080`, \u201cthe path `p` is weakly above `p'`\u201d and concludes that \u201c`p` stays weakly above `p'`.\u201d In contrast, the target assumes `(p'.vertices.get \u2026).2 > (p.vertices.get \u2026).2` at `x\u2080` and concludes the same strict inequality at every `x\u2081 \u2265 x\u2080`; thus it says that `p'` stays strictly above `p`. The omitted hypothesis `y_start(p) < y_start(p')` would by itself be a permissible generalization, but it does not repair this reversed order. To match the blueprint, the inequalities in both the `x\u2080` hypothesis and the `x\u2081` conclusion should compare `p` above `p'`, e.g. `(p.vertices.get \u2026).2 \u2265 (p'.vertices.get \u2026).2` (with strictness derivable from nonintersection when the compared vertices have the same x-coordinate). The starting-y hypothesis may also be added for an exact transcription."
}