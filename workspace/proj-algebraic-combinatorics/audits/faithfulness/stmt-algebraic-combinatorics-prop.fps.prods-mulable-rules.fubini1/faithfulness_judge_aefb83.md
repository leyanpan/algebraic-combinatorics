## TARGET PowerSeries.tprod_fubini_full (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_3} {J : Type u_4} {a : I × J → PowerSeries R}
  (ha : PowerSeries.Multipliable a) (ha_I : ∀ (i : I), PowerSeries.Multipliable fun j => a (i, j))
  (ha_J : ∀ (j : J), PowerSeries.Multipliable fun i => a (i, j))
  (ha_outer_I : optParam (PowerSeries.Multipliable fun i => PowerSeries.tprod (fun j => a (i, j)) ⋯) ⋯)
  (ha_outer_J : optParam (PowerSeries.Multipliable fun j => PowerSeries.tprod (fun i => a (i, j)) ⋯) ⋯),
  PowerSeries.tprod (fun i => PowerSeries.tprod (fun j => a (i, j)) ⋯) ha_outer_I = PowerSeries.tprod a ha ∧
    PowerSeries.tprod a ha = PowerSeries.tprod (fun j => PowerSeries.tprod (fun i => a (i, j)) ⋯) ha_outer_J

Docstring: Full Fubini rule: products over `I × J` can be computed as iterated products in either order.
...
## TARGET PowerSeries.tprod_fubini_J (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_3} {J : Type u_4} {a : I × J → PowerSeries R}
  (ha : PowerSeries.Multipliable a) (ha_J : ∀ (j : J), PowerSeries.Multipliable fun i => a (i, j))
  (ha_outer_J : optParam (PowerSeries.Multipliable fun j => PowerSeries.tprod (fun i => a (i, j)) ⋯) ⋯),
  PowerSeries.tprod a ha = PowerSeries.tprod (fun j => PowerSeries.tprod (fun i => a (i, j)) ⋯) ha_outer_J

Docstring: Fubini rule (J-first version): products over `I × J` can be computed as iterated products
over `J` then `I`.
(Label: prop.fps.prods-mulable-rules.fubini1) 

## TARGET PowerSeries.multipliable_tprod_fubini_J (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_3} {J : Type u_4} {a : I × J → PowerSeries R},
  PowerSeries.Multipliable a →
    ∀ (ha_J : ∀ (j : J), PowerSeries.Multipliable fun i => a (i, j)),
      PowerSeries.Multipliable fun j => PowerSeries.tprod (fun i => a (i, j)) ⋯

Docstring: Fubini rule (J-first version): the iterated product over `J` then `I` is multipliable.
(Label: prop.fps.prods-mulable-rules.fubini1) 
...
## PROJECT DEPENDENCY PowerSeries.multipliable_tprod_fubini (theorem)
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_3} {J : Type u_4} {a : I × J → PowerSeries R},
  PowerSeries.Multipliable a →
    ∀ (ha_I : ∀ (i : I), PowerSeries.Multipliable fun j => a (i, j)),
      PowerSeries.Multipliable fun i => PowerSeries.tprod (fun j => a (i, j)) ⋯

Docstring: Fubini rule: the iterated product over `I` then `J` is multipliable.
(Label: prop.fps.prods-mulable-rules.fubini1) 
...
{
  "verdict": "faithful",
  "justification": "The full theorem matches the blueprint\u2019s hypotheses and equality chain. It quantifies over arbitrary index types `I` and `J` and a family `a : I \u00d7 J \u2192 PowerSeries R`, assuming `ha : PowerSeries.Multipliable a`, `ha_I : \u2200 i, PowerSeries.Multipliable (fun j => a (i,j))`, and `ha_J : \u2200 j, PowerSeries.Multipliable (fun i => a (i,j))`, exactly answering the informal assumptions that the total family and every row and column are multipliable. Its conclusion `tprod_i (tprod_j a(i,j)) = tprod a \u2227 tprod a = tprod_j (tprod_i a(i,j))` is precisely the displayed three-term equality. The binders `ha_outer_I` and `ha_outer_J` are `optParam` proof arguments whose defaults witness that the iterated outer families are multipliable; they encode the well-definedness needed by the dependent definition `PowerSeries.tprod` rather than impose an additional mathematical restriction. This is also explicitly supported by `PowerSeries.multipliable_tprod_fubini` and `PowerSeries.multipliable_tprod_fubini_J`. The latter two J-oriented target declarations state individual consequences of the same rule and are more general where they omit the irrelevant row hypothesis `ha_I`; weaker hypotheses are acceptable. Finally, `PowerSeries.Multipliable`, `DeterminesCoeffInProd`, and `tprod` implement the informal coefficientwise finite-determination definitions directly."
}