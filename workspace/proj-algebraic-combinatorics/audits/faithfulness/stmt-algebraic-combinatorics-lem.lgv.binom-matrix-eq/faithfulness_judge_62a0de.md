## TARGET LGV.binom_matrix_eq_pathWeightMatrix (theorem) — ELABORATED SIGNATURE
∀ {k : ℕ} (a b : Fin k → ℕ),
  (Matrix.of fun i j => ↑((a i).choose (b j))) =
    LGV.pathWeightMatrix LGV.integerLattice_pathFinite LGV.unitArcWeight (LGV.binomLatticeA a) (LGV.binomLatticeB b)

Docstring: The binomial matrix equals pathWeightMatrix with unit weight 
...
## PROJECT DEPENDENCY LGV.unitArcWeight (def)
LGV.ArcWeight LGV.integerLattice ℤ

Body:
fun x x_1 x_2 => 1

Docstring: The unit arc weight function assigns 1 to every arc 

## PROJECT DEPENDENCY LGV.binomLatticeA (def)
{k : ℕ} → (Fin k → ℕ) → LGV.kVertex (ℤ × ℤ) k

Body:
fun {k} a i => (0, -↑(a i))

Docstring: The source lattice points for the binomial coefficient determinant:
A_i = (0, -a_i) for the LGV lemma application. 

## PROJECT DEPENDENCY LGV.binomLatticeB (def)
{k : ℕ} → (Fin k → ℕ) → LGV.kVertex (ℤ × ℤ) k

Body:
fun {k} b i => (↑(b i), -↑(b i))

Docstring: The target lattice points for the binomial coefficient determinant:
...
## INFORMAL STATEMENT
Binomial matrix equals path weight matrix

\leanhelper  With $A_i = (0, -a_i)$, $B_j = (b_j, -b_j)$, and unit arc weights, the path weight matrix equals the matrix $\bigl(\binom {a_i}{b_j}\bigr)$.
...
{
  "verdict": "faithful",
  "justification": "The formal theorem quantifies over arbitrary sizes and natural-indexed data, `\u2200 {k : \u2115} (a b : Fin k \u2192 \u2115)`, and identifies `Matrix.of fun i j => \u2191((a i).choose (b j))` with the stated path-weight matrix. Its dependencies instantiate exactly the blueprint data: `binomLatticeA a i = (0, -\u2191(a i))`, `binomLatticeB b i = (\u2191(b i), -\u2191(b i))`, and `unitArcWeight ... = 1`, on the integer lattice whose arcs are east or north steps. Moreover, `pathWeightMatrix` has entries `pathWeightSum ... (A i) (B j)`, so the right-hand side is precisely the matrix of unit-weight path sums. The use of `Fin k` merely encodes the blueprint\u2019s finite index set, and the equality being written in the reverse order from \u201cthe path weight matrix equals the matrix\u201d is immaterial by symmetry of equality."
}