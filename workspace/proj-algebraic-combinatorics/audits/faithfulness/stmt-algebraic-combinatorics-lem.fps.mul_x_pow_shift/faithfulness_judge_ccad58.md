## TARGET AlgebraicCombinatorics.FPS.coeff_mul_X_pow (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] (k : ℕ) (f : PowerSeries R) (n : ℕ),
  (PowerSeries.coeff n) (f * PowerSeries.X ^ k) = if n < k then 0 else (PowerSeries.coeff (n - k)) f

Docstring: f * X^k shifts f by k positions (generalization of Lemma lem.fps.xa) 
...
## INFORMAL STATEMENT
lem.fps.mul_X_pow_shift

\leanhelper  For any $k\in \mathbb {N}$ and any FPS $\mathbf{a}$, $[x^n](\mathbf{a}\cdot x^k) = \begin{cases}  0 &  \text{if }n<k, \\[x^{n-k}]\mathbf{a} &  \text{if }n\geq k.\end{cases}$

## VERDICT
{
  "verdict": "faithful",
  "discrepancies": [],
  "justification": "The formal theorem states, for every `k : \u2115`, `f : PowerSeries R`, and `n : \u2115`, that `(PowerSeries.coeff n) (f * PowerSeries.X ^ k) = if n < k then 0 else (PowerSeries.coeff (n - k)) f`. This exactly matches the blueprint\u2019s `[x^n](a\u00b7x^k) = 0` when `n<k`, and `[x^{n-k}]a` when `n\u2265k`. Here `PowerSeries.coeff` is explicitly the nth coefficient and `PowerSeries.X` is the formal-power-series variable. The `CommRing R` binder merely fixes the coefficient setting and introduces no conflict with the unspecified coefficient ring of the informal FPS."
}