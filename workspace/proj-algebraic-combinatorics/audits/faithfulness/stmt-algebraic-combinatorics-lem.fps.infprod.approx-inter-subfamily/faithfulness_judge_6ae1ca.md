## TARGET PowerSeries.isXnApproximator_inter_subfamily (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {I : Type u_2} {a : I → PowerSeries R} {U : Finset I} {n : ℕ},
  PowerSeries.IsXnApproximator a U n →
    (∀ (i : I), IsUnit ((PowerSeries.coeff 0) (a i))) →
      ∀ (J : Set I), PowerSeries.IsXnApproximator (fun i => a ↑i) (U.preimage Subtype.val ⋯) n

Docstring: Key lemma: If U is an x^n-approximator for the full family, and all FPS are invertible,
...
## BASE-LIBRARY REF Finset.preimage
{α : Type u} → {β : Type v} → (s : Finset β) → (f : α → β) → Set.InjOn f (f ⁻¹' ↑s) → Finset α

Docstring: Preimage of `s : Finset β` under a map `f` injective on `f ⁻¹' s` as a `Finset`. 

## BASE-LIBRARY REF Function.Injective.injOn
∀ {α : Type u_1} {β : Type u_2} {f : α → β}, Function.Injective f → ∀ {s : Set α}, Set.InjOn f s

Docstring: **Alias** of `Set.injOn_of_injective`.
...
## BASE-LIBRARY REF Subtype.val_injective
∀ {α : Sort u_1} {p : α → Prop}, Function.Injective Subtype.val

## BASE-LIBRARY REF Set.preimage
{α : Type u} → {β : Type v} → (α → β) → Set β → Set α

Docstring: The preimage of `s : Set β` by `f : α → β`, written `f ⁻¹' s`,
...
## INFORMAL STATEMENT
lem.fps.infprod.approx-inter-subfamily

\leanhelper  If $U$ is an $x^n$-approximator for a family $(\mathbf{a}_i)_{i \in I}$ of invertible FPSs and $J \subseteq I$, then $U \cap J$ (as a preimage under the subtype embedding) is an $x^n$-approximator for the subfamily $(\mathbf{a}_i)_{i \in J}$. This is the Lean formalization of Lemma~ \ref{lem.fps.prods-mulable-subfams-appr}, using the invertibility hypothesis to cancel factors outside $J$.
...
{
  "verdict": "faithful",
  "justification": "The hypothesis `PowerSeries.IsXnApproximator a U n` exactly says, via its definition, that `U` determines every coefficient `m \u2264 n`, matching \u201cU is an x^n-approximator.\u201d The binder `(\u2200 (i : I), IsUnit ((PowerSeries.coeff 0) (a i)))` expresses invertibility of each formal power series through the standard criterion that an FPS over a commutative ring is invertible iff its constant coefficient is a unit. For every `J : Set I`, the conclusion applies `IsXnApproximator` to the restricted family `fun i => a \u2191i` indexed by the subtype `J`, and to `U.preimage Subtype.val \u22ef`, which is precisely `U \u2229 J` represented in that subtype. Thus it matches the blueprint\u2019s subfamily conclusion. The formal coefficient ring assumption `[CommRing R]` is at least as general as the usual coefficient-field setting and does not weaken the result."
}