## TARGET AlgebraicCombinatorics.Determinants.cauchy_det_of_poly (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : Field K] {m : ℕ} (x y : Fin m → K) (h : ∀ (i j : Fin m), x i + y j ≠ 0),
  (AlgebraicCombinatorics.Determinants.cauchyMat x y h).det =
    AlgebraicCombinatorics.Determinants.cauchyNumerator x y / AlgebraicCombinatorics.Determinants.cauchyDenominator x y

Docstring: Cauchy determinant follows from the polynomial version.
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.cauchyNumerator (def)
{K : Type u_2} → [Field K] → {m : ℕ} → (Fin m → K) → (Fin m → K) → K

Body:
fun {K} [Field K] {m} x y => ∏ i, ∏ j ∈ Finset.Ioi i, (x i - x j) * (y i - y j)

Docstring: The numerator of the Cauchy determinant formula:
∏_{i<j}((xᵢ - xⱼ)(yᵢ - yⱼ)) 

## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.cauchyDenominator (def)
{K : Type u_2} → [Field K] → {m : ℕ} → (Fin m → K) → (Fin m → K) → K

Body:
fun {K} [Field K] {m} x y => ∏ i, ∏ j, (x i + y j)

Docstring: The denominator of the Cauchy determinant formula:
∏_{i,j}(xᵢ + yⱼ) 
...
## INFORMAL STATEMENT
lem.det.cauchy-det-of-poly

\leanhelper  Let $K$ be a field. Let $x_1, \ldots , x_m, y_1, \ldots , y_m \in K$ with $x_i + y_j \neq 0$ for all $i, j$. Then 

\[  \det \! \left(\frac{1}{x_i + y_j}\right)_{i,j} = \frac{\prod _{i < j}(x_i - x_j)(y_i - y_j)}{\prod _{i,j}(x_i + y_j)}.  \]
...
{
  "verdict": "faithful",
  "justification": "The binders exactly match the blueprint: `\u2200 {K} [Field K] {m} (x y : Fin m \u2192 K) (h : \u2200 i j, x i + y j \u2260 0)`. The body of `cauchyMat` gives entries `(x i + y j)\u207b\u00b9`, i.e. `1 / (x_i + y_j)` in a field. The conclusion uses `Matrix.det` and expands through the project definitions to `\u220f i, \u220f j \u2208 Finset.Ioi i, (x i - x j) * (y i - y j)` divided by `\u220f i, \u220f j, (x i + y j)`, exactly the stated products over `i < j` and all `i,j`. Using `Fin m` merely encodes the finite index set with zero-based rather than one-based labels and does not change the mathematical assertion."
}