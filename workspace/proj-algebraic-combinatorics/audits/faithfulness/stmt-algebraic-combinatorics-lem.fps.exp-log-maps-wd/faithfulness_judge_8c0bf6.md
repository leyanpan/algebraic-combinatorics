## TARGET PowerSeries.PowerSeries₁.subst_mem (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] {f g : PowerSeries K},
  f ∈ PowerSeries.PowerSeries₁ → g ∈ PowerSeries.PowerSeries₀ → PowerSeries.subst g f ∈ PowerSeries.PowerSeries₁

Docstring: Composition of an FPS with constant term 1 and one with constant term 0 has constant term 1.
This is Lemma 7.8.7(b) (lem.fps.Exp-Log-maps-wd). 

## TARGET PowerSeries.exp_subst_mem_PowerSeries₁ (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] {g : PowerSeries K},
  g ∈ PowerSeries.PowerSeries₀ → PowerSeries.subst g (PowerSeries.exp K) ∈ PowerSeries.PowerSeries₁

Docstring: `exp ∘ g` has constant term 1 when `g` has constant term 0.
...
Docstring: Composition of two FPS with constant term 0 has constant term 0.
This is Lemma 7.8.7(a) (lem.fps.Exp-Log-maps-wd). 

## TARGET PowerSeries.logbar_subst_sub_one_mem_PowerSeries₀ (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] {f : PowerSeries K},
  f ∈ PowerSeries.PowerSeries₁ → PowerSeries.subst (f - 1) (PowerSeries.logbar K) ∈ PowerSeries.PowerSeries₀
...
## INFORMAL STATEMENT
lem.fps.Exp-Log-maps-wd

\textbf{(a)} For any $f,g\in K\left[\left[ x\right]\right]_{0}$, we have $f\circ g\in K\left[\left[x\right] \right]_{0}$. \medskip 

\textbf{(b)} For any $f\in K\left[\left[x\right]\right]_{1}$ and $g\in K\left[\left[x\right]\right]_{0}$, we have $f\circ g\in K\left[ \left[x\right]\right]_{1}$. \medskip 

\textbf{(c)} For any $g\in K\left[\left[x\right]\right]_{0}$, we have $\exp \circ g\in K\left[\left[x\right]\right]_{1}$. \medskip 

\textbf{(d)} For any $f\in K\left[\left[x\right]\right]_{1}$, we have $f-1\in K\left[\left[x\right]\right]_{0}$ and $\overline{\log } \circ \left(f-1\right)\in K\left[\left[x\right]\right]_{0}$.
...
{
  "verdict": "faithful",
  "justification": "The four declarations match parts (a)\u2013(d), with `PowerSeries.subst g f` representing the blueprint\u2019s `f \u2218 g`. The set definitions agree exactly with the informal ones: `PowerSeries\u2080` has body `{f | PowerSeries.constantCoeff f = 0}` and `PowerSeries\u2081` has body `{f | PowerSeries.constantCoeff f = 1}`. Thus (a) is formalized by `f \u2208 PowerSeries\u2080 \u2192 g \u2208 PowerSeries\u2080 \u2192 subst g f \u2208 PowerSeries\u2080`, and (b) by `f \u2208 PowerSeries\u2081 \u2192 g \u2208 PowerSeries\u2080 \u2192 subst g f \u2208 PowerSeries\u2081`. Their binder `[CommRing K]` is weaker than the section-wide informal assumption that `K` is a commutative \u211a-algebra, so these two are strictly more general and hence faithful. Part (c) has the appropriate binders `[CommRing K] [Algebra \u211a K]` and states `g \u2208 PowerSeries\u2080 \u2192 subst g (exp K) \u2208 PowerSeries\u2081`. Part (d) similarly states the logarithmic conclusion `f \u2208 PowerSeries\u2081 \u2192 subst (f - 1) (logbar K) \u2208 PowerSeries\u2080`, and `logbar` has the prescribed Mercator-series coefficients. Although the target does not explicitly include the additional conjunct `f-1 \u2208 K[[x]]\u2080`, that conjunct follows directly from the premise and the definitions: constant coefficient 1 implies the constant coefficient of `f-1` is `1-1=0`. Hence omitting this redundant consequence does not change the mathematical assertion."
}