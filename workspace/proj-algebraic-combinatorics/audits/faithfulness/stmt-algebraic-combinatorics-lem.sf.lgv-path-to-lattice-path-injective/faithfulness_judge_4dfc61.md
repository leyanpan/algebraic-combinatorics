## TARGET SymmetricFunctions.lgvPathToLatticePath_injective (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} (a c : ℤ) (p₁ p₂ : LGV.integerLattice.Path) (hstart₁ : p₁.start = (a, 1)) (hfinish₁ : p₁.finish = (c, ↑N))
  (hstart₂ : p₂.start = (a, 1)) (hfinish₂ : p₂.finish = (c, ↑N)),
  SymmetricFunctions.lgvPathToLatticePath a c p₁ hstart₁ hfinish₁ =
      SymmetricFunctions.lgvPathToLatticePath a c p₂ hstart₂ hfinish₂ →
    p₁ = p₂

Docstring: Corollary: lgvPathToLatticePath is injective on paths with the same start and end.
...
## PROJECT DEPENDENCY SymmetricFunctions.lgvPathToLatticePath (def)
{N : ℕ} →
  (a c : ℤ) → (p : LGV.integerLattice.Path) → p.start = (a, 1) → p.finish = (c, ↑N) → SymmetricFunctions.LatticePath a c

Body:
fun {N} a c p hstart hfinish =>
  {
    eastStepHeights :=
      let ycoords := SymmetricFunctions.lgvPathEastStepYCoords p.vertices;
      have h := ⋯;
      SymmetricFunctions.lgvYCoordsToFinN ycoords h,
    weaklyIncreasing := ⋯, length_eq := ⋯ }

Docstring: Convert an LGV path from (a, 1) to (c, N) to a LatticePath.
...
## PROJECT DEPENDENCY SymmetricFunctions.lgvPathEastStepYCoords (def)
List (ℤ × ℤ) → List ℤ

Body:
fun vertices =>
  List.brecOn vertices fun vertices f =>
    (match (motive := (vertices : List (ℤ × ℤ)) → List.below vertices → List ℤ) vertices with
      | [] => fun x => []
      | [head] => fun x => []
      | v₀ :: v₁ :: rest => fun x => if v₁.1 = v₀.1 + 1 ∧ v₁.2 = v₀.2 then v₀.2 :: x.1 else x.1)
      f

Docstring: Extract the y-coordinates of east-steps from an LGV path vertex list.
For a path from (a, 1) to (c, N), this returns the y-coordinates at which
each east-step occurs. 

## PROJECT DEPENDENCY SymmetricFunctions.lgvYCoordsToFinN (def)
{N : ℕ} → (ycoords : List ℤ) → (∀ y ∈ ycoords, 1 ≤ y ∧ y ≤ ↑N) → List (Fin N)

Body:
fun {N} ycoords h => List.pmap (fun y hy => ⟨(y - 1).toNat, ⋯⟩) ycoords ⋯

Docstring: Convert east-step y-coordinates (in ℤ, range [1, N]) to Fin N elements.
This is a helper for the LGV path to LatticePath bijection. 

## PROJECT DEPENDENCY SymmetricFunctions.extractEastStepYCoords (def)
List (ℤ × ℤ) → List ℤ
...
fun vertices =>
  List.brecOn vertices fun vertices f =>
    (match (motive := (vertices : List (ℤ × ℤ)) → List.below vertices → List ℤ) vertices with
      | [] => fun x => []
      | [head] => fun x => []
      | u :: v :: rest => fun x => if v.1 = u.1 + 1 then u.2 :: x.1 else x.1)
      f

Docstring: Extract y-coordinates at east-step positions from a path vertex list.
...
## INFORMAL STATEMENT
lem.sf.lgv-path-to-lattice-path-injective

\leanhelper  The conversion from LGV path to lattice path is injective: two LGV paths with the same east-step $y$-coordinates have the same vertex sequence. This is because a path in the integer lattice is uniquely determined by its starting point and the sequence of east-step positions.
...
{
  "verdict": "faithful",
  "justification": "The informal conversion is explicitly for an LGV path \u201cfrom `(a, 1)` to `(c, N)`,\u201d and the target quantifies paths with exactly those endpoint conditions: `hstart\u2081 : p\u2081.start = (a, 1)`, `hfinish\u2081 : p\u2081.finish = (c, \u2191N)` and likewise for `p\u2082`. Thus these are domain-encoding hypotheses, not added mathematical restrictions. Equality of `lgvPathToLatticePath ...` equates the converted east-step-height lists; by the dependency body, these are obtained from `lgvPathEastStepYCoords p.vertices` via `lgvYCoordsToFinN`, exactly implementing the informal definition of extracting east-step y-coordinates and subtracting 1. The conclusion `p\u2081 = p\u2082` implies that the paths have the same vertex sequence, as required by \u201ctwo LGV paths with the same east-step y-coordinates have the same vertex sequence,\u201d and is an acceptable stronger formulation of that conclusion."
}