## TARGET PowerSeries.tprod_eq_tprod_fibers (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {S : Type u_3} {W : Type u_4} {f : S → W} {a : S → PowerSeries R}
  (ha : PowerSeries.Multipliable a) (ha_fibers : ∀ (w : W), PowerSeries.Multipliable fun s => a ↑s)
...
Docstring: A multipliable product can be broken into subproducts indexed by a map.
...
## PROJECT DEPENDENCY PowerSeries.multipliable_prod_fibers (theorem)
∀ {R : Type u_1} [inst : CommRing R] {S : Type u_3} {W : Type u_4} {f : S → W} {a : S → PowerSeries R},
  PowerSeries.Multipliable a →
    ∀ (ha_fibers : ∀ (w : W), PowerSeries.Multipliable fun s => a ↑s),
      PowerSeries.Multipliable fun w => PowerSeries.tprod (fun s => a ↑s) ⋯

Docstring: The family of subproducts indexed by fibers is multipliable.
(Label: prop.fps.prods-mulable-rules.SW1)
...
## INFORMAL STATEMENT
thm.fps.prods-mulable-fibers-eq.lean

\leanhelper  Under the hypotheses of Theorem~ \ref{thm.fps.prods-mulable-fibers.lean}, 

\[  \prod _{s \in S} \mathbf{a}_s = \prod _{w \in W} \Bigl(\prod _{s \in f^{-1}(w)} \mathbf{a}_s\Bigr).  \]

 This is the Lean formalization of Proposition~ \ref{prop.fps.prods-mulable-rules.SW1} (parts \textbf{(a)} and \textbf{(b)}).
...
{
  "verdict": "faithful",
  "justification": "The target states exactly the fiberwise product identity from the blueprint: `PowerSeries.tprod a ha = PowerSeries.tprod (fun w => PowerSeries.tprod (fun s => a \u2191s) \u22ef) ha_outer`, corresponding to `\u220f_{s \u2208 S} a_s = \u220f_{w \u2208 W} (\u220f_{s \u2208 f\u207b\u00b9(w)} a_s)`. Its hypotheses `ha : PowerSeries.Multipliable a` and `ha_fibers : \u2200 w, PowerSeries.Multipliable fun s => a \u2191s` are precisely the hypotheses of the referenced fiber theorem, as shown by `PowerSeries.multipliable_prod_fibers`. The subtype variable `s` in each inner product indexes the fiber of `f` over `w`. The additional-looking binder `ha_outer : optParam (PowerSeries.Multipliable ...) \u22ef` is not a mathematical restriction: its default is supplied by `PowerSeries.multipliable_prod_fibers ha ha_fibers`, so it merely provides the proof required by the definition of `PowerSeries.tprod`. The project definitions of `Multipliable`, `DeterminesCoeffInProd`, and `tprod` agree with the corresponding informal definitions."
}