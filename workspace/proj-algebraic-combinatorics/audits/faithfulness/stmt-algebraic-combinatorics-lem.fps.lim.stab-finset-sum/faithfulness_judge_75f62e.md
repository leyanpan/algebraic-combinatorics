## TARGET Seq.stabilizesTo_finset_sum (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} {ι : Type u_2} [inst : AddCommMonoid K] [DecidableEq ι] (s : Finset ι) {a : ι → ℕ → K} {la : ι → K},
  (∀ i ∈ s, Seq.StabilizesTo (a i) (la i)) → Seq.StabilizesTo (fun n => ∑ i ∈ s, a i n) (∑ i ∈ s, la i)

Docstring: A finite sum of stabilizing sequences stabilizes. 
...
## INFORMAL STATEMENT
lem.fps.lim.stab-finset-sum

\leanhelper  Let $S$ be a finite set, and for each $s \in S$, let $(a_{s,i})_{i \in \mathbb {N}}$ be a sequence stabilizing to $\ell _s$. Then the sequence $\left(\sum _{s \in S} a_{s,i}\right)_{i \in \mathbb {N}}$ stabilizes to $\sum _{s \in S} \ell _s$.
...
{
  "verdict": "faithful",
  "justification": "The formal hypothesis `\u2200 i \u2208 s, Seq.StabilizesTo (a i) (la i)` exactly matches \u201cfor each s \u2208 S ... a sequence stabilizing to \u2113_s,\u201d since `Seq.StabilizesTo` is defined as `\u2203 N, \u2200 i \u2265 N, a i = lim`. The conclusion `Seq.StabilizesTo (fun n => \u2211 i \u2208 s, a i n) (\u2211 i \u2208 s, la i)` exactly states that the sequence of finite sums stabilizes to the sum of the eventual values. `s : Finset \u03b9` represents the finite set. `[AddCommMonoid K]` supplies the algebraic structure needed for an unordered finite sum, and `[DecidableEq \u03b9]` is an implementation-level requirement for finsets rather than an additional mathematical restriction."
}