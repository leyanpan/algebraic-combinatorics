## TARGET AlgebraicCombinatorics.transposeMonotone_involutive (theorem) — ELABORATED SIGNATURE
∀ (k m : ℕ) (f : Fin k → Fin (m + 1)),
  Monotone f → AlgebraicCombinatorics.transposeMonotone✝ m k (AlgebraicCombinatorics.transposeMonotone✝¹ k m f) = f
...
## INFORMAL STATEMENT
lem.pars.qbinom.transpose-monotone-involutive

\leanhelper  Let $(f_0, f_1, \ldots , f_{k-1})$ be a weakly increasing tuple in $\{ 0,1,\ldots ,m\} ^k$. Define the transpose $(g_0, g_1, \ldots , g_{m-1})$ with $g_j = \# \{ i : f_i > m{-}1{-}j\} $. Then applying the transpose operation twice recovers the original tuple.
...
{
  "verdict": "faithful",
  "justification": "The blueprint assumes a \u201cweakly increasing tuple in {0,1,\u2026,m}^k\u201d; the formal binders encode exactly this as `f : Fin k \u2192 Fin (m + 1)` with `Monotone f`. The dependency defines the transpose by `fun k m f j => \u27e8{i | \u2191(f i) > m - 1 - \u2191j}.card, \u22ef\u27e9`, matching `g_j = # { i : f_i > m\u22121\u2212j }`. Finally, the conclusion `transposeMonotone m k (transposeMonotone k m f) = f` states that applying this operation twice recovers the original tuple. Function equality is precisely entrywise recovery. No mathematically substantive extra hypothesis or narrowed quantifier is present."
}