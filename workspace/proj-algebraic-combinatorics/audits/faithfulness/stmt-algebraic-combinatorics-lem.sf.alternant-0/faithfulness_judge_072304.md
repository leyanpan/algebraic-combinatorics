## TARGET alternant_zero_of_eq (theorem) — ELABORATED SIGNATURE
∀ (N : ℕ) {R : Type u_2} [inst : CommRing R] {α : Fin N → ℕ} (i j : Fin N), i ≠ j → α i = α j → alternant N α = 0

Docstring: An alternant is zero if two entries of α are equal.
Lemma \ref{lem.sf.alternant-0}(a) in the source.

This delegates to `AlgebraicCombinatorics.alternant_eq_zero_of_repeated`. 

## TARGET alternant_swap (theorem) — ELABORATED SIGNATURE
∀ (N : ℕ) {R : Type u_2} [inst : CommRing R] {α : Fin N → ℕ} (i j : Fin N),
  i ≠ j → alternant N (α ∘ ⇑(Equiv.swap i j)) = -alternant N α

Docstring: Swapping columns of an alternant multiplies it by -1.
Lemma \ref{lem.sf.alternant-0}(b) in the source.

This delegates to `AlgebraicCombinatorics.alternant_swap`. 

## PROJECT DEPENDENCY alternant (def)
{R : Type u_1} → [inst : CommRing R] → (N : ℕ) → (Fin N → ℕ) → MvPolynomial (Fin N) R

Body:
fun {R} [CommRing R] N α => AlgebraicCombinatorics.alternant α

Docstring: The alternant a_α for an N-tuple α.
...
\textbf{(a)} If the $N$-tuple $\alpha $ has two equal entries, then $a_{\alpha }=0$. \medskip 
...
{
  "verdict": "faithful",
  "justification": "Both declarations faithfully formalize the corresponding parts. For (a), the blueprint says \u201chas two equal entries,\u201d and the target quantifies distinct positions via `(i j : Fin N), i \u2260 j \u2192 \u03b1 i = \u03b1 j \u2192 alternant N \u03b1 = 0`. For (b), the blueprint\u2019s tuple obtained by swapping two entries is represented by `\u03b1 \u2218 \u21d1(Equiv.swap i j)`, and the target concludes exactly `alternant N (...) = -alternant N \u03b1`; `i \u2260 j` expresses that two distinct entries are swapped. The dependency `alternant` denotes the polynomial `\u2211 \u03c3, sign \u03c3 \u2022 xPow (\u03b1 \u2218 \u21d1\u03c3)`, which matches the determinant expansion of the informal matrix `(x_i^{\u03b1_j})`, as its monomial has exponents `\u03b1 (\u03c3 i)`. The binder `[CommRing R]` is precisely the chapter\u2019s fixed commutative coefficient ring, while quantifying over all such `R` is harmless generality."
}