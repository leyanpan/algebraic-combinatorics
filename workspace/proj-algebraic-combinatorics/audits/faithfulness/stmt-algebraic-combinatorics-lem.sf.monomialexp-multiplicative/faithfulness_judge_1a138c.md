## TARGET AlgebraicCombinatorics.SymmetricFunctions.monomialExp_add (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommSemiring R] {N : ℕ} (a b : Fin N → ℕ),
  AlgebraicCombinatorics.SymmetricFunctions.monomialExp (a + b) =
    AlgebraicCombinatorics.SymmetricFunctions.monomialExp a * AlgebraicCombinatorics.SymmetricFunctions.monomialExp b

Docstring: monomialExp is multiplicative in the exponent. 

## TARGET AlgebraicCombinatorics.SymmetricFunctions.monomialExp_zero (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommSemiring R] {N : ℕ}, AlgebraicCombinatorics.SymmetricFunctions.monomialExp 0 = 1

Docstring: monomialExp 0 is 1. 
...
## INFORMAL STATEMENT
lem.sf.monomialExp-multiplicative

\leanhelper  The monomial map is multiplicative: $x^{a+b} = x^a \cdot x^b$, and $x^0 = 1$.
...
{
  "verdict": "faithful",
  "justification": "The two declarations exactly formalize the two asserted identities. `monomialExp_add` quantifies over exponent tuples `(a b : Fin N \u2192 \u2115)` and concludes `monomialExp (a + b) = monomialExp a * monomialExp b`, matching \u201c$x^{a+b} = x^a \\cdot x^b$.\u201d `monomialExp_zero` concludes `monomialExp 0 = 1`, matching \u201c$x^0 = 1$.\u201d The dependency defines `monomialExp a` as `\u220f i, MvPolynomial.X i ^ a i`, which is precisely the monomial $x_1^{a_1}\\cdots x_N^{a_N}$ from `def.sf.monomial`. Although the signatures assume only `[CommSemiring R]` rather than the blueprint\u2019s commutative-ring setting, this is a weaker hypothesis and therefore a valid strict generalization, not drift."
}