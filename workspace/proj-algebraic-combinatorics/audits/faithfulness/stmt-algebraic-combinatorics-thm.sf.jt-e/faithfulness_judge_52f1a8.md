...
{
  "verdict": "unstatable",
  "justification": "The blueprint states simply \u201cLet \u03bb and \u03bc be two partitions\u201d and its definitions make s_{\u03bb/\u03bc} meaningful even without \u03bc \u2286 \u03bb: `def.sf.skew-tab` says, \u201cIf we don\u2019t have \u03bc \u2286 \u03bb, then we agree that there are no Young tableaux of shape \u03bb/\u03bc,\u201d while `def.sf.skew-schur` defines the sum for \u201ctwo N-partitions.\u201d The project dependency cannot express that full domain: `SymmetricFunctions.skewSchur` takes a `SymmetricFunctions.SkewPartition N`, whose only constructor is `SkewPartition.mk ... inner \u2264 outer`. Consequently the target must add `(hcontained : \u2200 i, mu i \u2264 lam i)`, so it cannot state the blueprint\u2019s non-contained cases. To make the blueprint statable, `skewSchur` (or an extension of it) must accept arbitrary pairs of `NPartition`s and return the empty-tableau sum, hence zero, when containment fails. There is also an independent quantifier restriction: the blueprint chooses an arbitrary determinant size `M` bounding the lengths of `\u03bb\u1d57` and `\u03bc\u1d57`, whereas the target uses one index size everywhere, `(lam mu lamt muT : Fin N \u2192 \u2115)`. A faithful signature would use separate sizes\u2014for example `lam mu : Fin N \u2192 \u2115` and `lamt muT : Fin M \u2192 \u2115`\u2014and take the determinant over `Fin M`; `IsTranspose` already supports different tuple lengths via `{N M : \u2115}`."
}