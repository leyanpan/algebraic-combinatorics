## TARGET AlgebraicCombinatorics.benderKnuthPrefixMatching_semistandard_stembridge (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} {lam mu : Fin N → ℕ},
          ...
          ∀ (T : AlgebraicCombinatorics.Tableau lam mu) (hT : AlgebraicCombinatorics.IsSemistandard T),
            AlgebraicCombinatorics.IsNPartition (nu + AlgebraicCombinatorics.contentColGeq T (j + 1)) →
              AlgebraicCombinatorics.isMisstep (nu + AlgebraicCombinatorics.contentColGeq T j) k →
                AlgebraicCombinatorics.IsSemistandard (AlgebraicCombinatorics.benderKnuthPrefixMatching k hk j T hT)

Docstring: benderKnuthPrefixMatching preserves semistandardness in the Stembridge involution context.
...
## INFORMAL STATEMENT
Prefix BK preserves semistandardness (Stembridge context)

\leanhelper  Under the Stembridge context (where $\beta = \nu + \operatorname {cont}(\operatorname {col}_{\geq j+1} T)$ is a partition and $k$ is a misstep for $\nu + \operatorname {cont}(\operatorname {col}_{\geq j} T)$), the prefix-restricted BK involution $\beta _k^{<j}(T)$ preserves semistandardness.
...
{
  "verdict": "faithful",
  "justification": "The target matches the stated Stembridge context: it assumes `j > 0`, a semistandard tableau via `(T : Tableau lam mu) (hT : IsSemistandard T)`, the partition condition `IsNPartition (nu + contentColGeq T (j + 1))`, and the misstep condition `isMisstep (nu + contentColGeq T j) k`, and concludes `IsSemistandard (benderKnuthPrefixMatching k hk j T hT)`. These correspond directly to \u201c\u03b2 = \u03bd + cont(col_{\u2265 j+1} T) is a partition,\u201d \u201ck is a misstep for \u03bd + cont(col_{\u2265 j} T),\u201d and \u201c\u03b2_k^{<j}(T) preserves semistandardness.\u201d The separate binder `(hk : \u2191k + 1 < N)` adds no mathematical restriction because `isMisstep \u03b1 k` is defined as `\u2203 (h : \u2191k + 1 < N), \u03b1 k < \u03b1 \u27e8\u2191k + 1,h\u27e9`; it merely supplies the bound proof needed to construct `k+1` in `Fin N`. The assumptions that `lam` and `mu` are N-partitions agree with the tableau setting. Although the formal `Tableau` type remains inhabited as a function type even when `mu` is not componentwise below `lam`, this only makes the theorem more general than the blueprint\u2019s valid-skew-shape setting, not weaker."
}