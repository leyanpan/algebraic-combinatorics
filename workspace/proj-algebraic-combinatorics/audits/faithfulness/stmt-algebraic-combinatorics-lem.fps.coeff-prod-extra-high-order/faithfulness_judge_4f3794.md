## TARGET coeff_prod_eq_of_extra_high_order' (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {I : Type u_2} [inst_1 : DecidableEq I] (g : I → PowerSeries K) (s t : Finset I),
  s ⊆ t →
    ∀ (n : ℕ),
      (∀ i ∈ t \ s, ∀ m ≤ n, (PowerSeries.coeff m) (g i) = 0) →
        (PowerSeries.coeff n) (∏ i ∈ t, (1 + g i)) = (PowerSeries.coeff n) (∏ i ∈ s, (1 + g i))

Docstring: For a finite product where extra factors have high order, the coeff n is unchanged.
...
## INFORMAL STATEMENT
lem.fps.coeff-prod-extra-high-order

\leanhelper  \textbf{(Finite irrelevance for extra high-order factors.)} Let $s\subseteq t$ be finite sets of indices, and let $g : I\to K[[x]]$. Assume that for each $i\in t\setminus s$, we have $[x^m](g_i) = 0$ for all $m\le n$. Then $[x^n]\! \left(\prod _{i\in t}(1+g_i)\right) = [x^n]\! \left(\prod _{i\in s}(1+g_i)\right)$.
...
{
  "verdict": "faithful",
  "justification": "The formal conclusion exactly matches the blueprint: `(PowerSeries.coeff n) (\u220f i \u2208 t, (1 + g i)) = (PowerSeries.coeff n) (\u220f i \u2208 s, (1 + g i))`, under `s \u2286 t` and `\u2200 i \u2208 t \\ s, \u2200 m \u2264 n, (PowerSeries.coeff m) (g i) = 0`. The formal `\u2200 (n : \u2115)` makes explicit the arbitrary coefficient index used in the informal statement. Although the section convention assumes a commutative \u211a-algebra, the formal binder only requires `[CommRing K]`; this is a weaker hypothesis and hence makes the theorem strictly more general, which is faithful. The binder `[DecidableEq I]` is only an implementation requirement for `Finset` difference, not a mathematically contentful restriction."
}