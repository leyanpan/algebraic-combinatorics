## TARGET PowerSeries.Exp_add (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] (f g : ↑PowerSeries.PowerSeries₀),
  PowerSeries.Exp ⟨↑f + ↑g, ⋯⟩ = ⟨↑(PowerSeries.Exp f) * ↑(PowerSeries.Exp g), ⋯⟩

Docstring: `Exp(f + g) = Exp(f) · Exp(g)`. This is Lemma 7.8.9(a) (lem.fps.Exp-Log-additive). 

## TARGET PowerSeries.Log_mul (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] (f g : ↑PowerSeries.PowerSeries₁),
  PowerSeries.Log ⟨↑f * ↑g, ⋯⟩ = ⟨↑(PowerSeries.Log f) + ↑(PowerSeries.Log g), ⋯⟩

Docstring: `Log(fg) = Log(f) + Log(g)`. This is Lemma 7.8.9(b) (lem.fps.Exp-Log-additive). 
...
## PROJECT DEPENDENCY PowerSeries.Exp (def)
{K : Type u_2} → [inst : CommRing K] → [Algebra ℚ K] → ↑PowerSeries.PowerSeries₀ → ↑PowerSeries.PowerSeries₁

Body:
fun {K} [CommRing K] [Algebra ℚ K] g => ⟨PowerSeries.subst (↑g) (PowerSeries.exp K), ⋯⟩

Docstring: The exponential map `Exp : K⟦X⟧₀ → K⟦X⟧₁` defined by `g ↦ exp ∘ g`.
This is Definition 7.8.6(c) (def.fps.Exp-Log-maps). 

## PROJECT DEPENDENCY PowerSeries.mem_PowerSeries₀_iff (theorem)
∀ {R : Type u_2} [inst : CommRing R] {f : PowerSeries R}, f ∈ PowerSeries.PowerSeries₀ ↔ PowerSeries.constantCoeff f = 0

## PROJECT DEPENDENCY PowerSeries.mem_PowerSeries₁_iff (theorem)
∀ {R : Type u_2} [inst : CommRing R] {f : PowerSeries R}, f ∈ PowerSeries.PowerSeries₁ ↔ PowerSeries.constantCoeff f = 1

## PROJECT DEPENDENCY PowerSeries.Log (def)
{K : Type u_2} → [inst : CommRing K] → [Algebra ℚ K] → ↑PowerSeries.PowerSeries₁ → ↑PowerSeries.PowerSeries₀

Body:
fun {K} [CommRing K] [Algebra ℚ K] f => ⟨PowerSeries.subst (↑f - 1) (PowerSeries.logbar K), ⋯⟩

Docstring: The logarithm map `Log : K⟦X⟧₁ → K⟦X⟧₀` defined by `f ↦ logbar ∘ (f - 1)`.
This is Definition 7.8.6(c) (def.fps.Exp-Log-maps). 

## PROJECT DEPENDENCY PowerSeries.logbar (def)
(K : Type u_1) → [inst : CommRing K] → [Algebra ℚ K] → PowerSeries K

Body:
fun K [CommRing K] [Algebra ℚ K] => PowerSeries.mk fun n => if n = 0 then 0 else (algebraMap ℚ K) ((-1) ^ (n - 1) / ↑n)

Docstring: The logarithm series `logbar = ∑_{n≥1} (-1)^{n-1}/n · x^n`, which is the Mercator series
...
## BASE-LIBRARY REF propext
∀ {a b : Prop}, (a ↔ b) → a = b

Docstring: The [axiom](https://lean-lang.org/doc/reference/4.28.0/find/?domain=Verso.Genre.Manual.section&name=axioms) of **propositional extensionality**. It asserts that if
...
## BASE-LIBRARY REF RingHomClass.toAddMonoidHomClass
...
## BASE-LIBRARY REF RingHom.instRingHomClass
∀ {α : Type u_2} {β : Type u_3} {x : NonAssocSemiring α} {x_1 : NonAssocSemiring β}, RingHomClass (α →+* β) α β

## BASE-LIBRARY REF Subtype.property
∀ {α : Sort u} {p : α → Prop} (self : Subtype p), p ↑self

Docstring: The proof that `val` satisfies the predicate `p`.
...
## BASE-LIBRARY REF map_mul
∀ {M : Type u_4} {N : Type u_5} {F : Type u_9} [inst : Mul M] [inst_1 : Mul N] [inst_2 : FunLike F M N]
  [MulHomClass F M N] (f : F) (x y : M), f (x * y) = f x * f y

Docstring: See note [hom simp lemma priority] 

## BASE-LIBRARY REF NonUnitalRingHomClass.toMulHomClass
...
## BASE-LIBRARY REF RingHomClass.toNonUnitalRingHomClass
∀ {F : Type u_1} {α : Type u_2} {β : Type u_3} [inst : FunLike F α β] {x : NonAssocSemiring α}
  {x_1 : NonAssocSemiring β} [RingHomClass F α β], NonUnitalRingHomClass F α β
...
## INFORMAL STATEMENT
lem.fps.Exp-Log-additive

\textbf{(a)} For any $f,g\in K\left[\left[ x\right]\right]_{0}$, we have 

\[  \operatorname {Exp}\left(f+g\right)=\operatorname {Exp}f\cdot \operatorname {Exp}g.  \]

\textbf{(b)} For any $f,g\in K\left[\left[x\right]\right]_{1}$, we have 
...
{
  "verdict": "faithful",
  "justification": "Both declarations match the two blueprint clauses. For (a), the binders `(f g : \u2191PowerSeries.PowerSeries\u2080)` quantify exactly over series whose constant coefficient is zero, since `PowerSeries\u2080` has body `{f | PowerSeries.constantCoeff f = 0}`, matching \u201cfor any f,g \u2208 K[[x]]\u2080\u201d; the conclusion `PowerSeries.Exp \u27e8\u2191f + \u2191g, \u22ef\u27e9 = \u27e8\u2191(PowerSeries.Exp f) * \u2191(PowerSeries.Exp g), \u22ef\u27e9` is precisely `Exp(f+g)=Exp f\u00b7Exp g`. For (b), `(f g : \u2191PowerSeries.PowerSeries\u2081)` uses the body `{f | PowerSeries.constantCoeff f = 1}`, matching \u201cfor any f,g \u2208 K[[x]]\u2081\u201d, and `PowerSeries.Log \u27e8\u2191f * \u2191g, \u22ef\u27e9 = \u27e8\u2191(PowerSeries.Log f) + \u2191(PowerSeries.Log g), \u22ef\u27e9` is precisely `Log(fg)=Log f+Log g`. The definitions of `Exp` and `Log` are respectively substitution into `PowerSeries.exp K` and substitution of `f-1` into `PowerSeries.logbar K`, matching the informal definitions. Finally, `[CommRing K] [Algebra \u211a K]` exactly reflects the section convention that `K` is a commutative \u211a-algebra; universally quantifying `K` merely makes that context explicit."
}