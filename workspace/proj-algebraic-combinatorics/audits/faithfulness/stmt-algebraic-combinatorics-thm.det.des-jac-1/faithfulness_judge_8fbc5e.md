## TARGET AlgebraicCombinatorics.Determinants.desnanot_jacobi_det2 (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {m : ℕ} (A : Matrix (Fin (m + 2)) (Fin (m + 2)) R),
  A.det * (AlgebraicCombinatorics.Determinants.innerSubmatrix A).det =
    !![(AlgebraicCombinatorics.Determinants.submatrixRemove A 0 0).det,
          (AlgebraicCombinatorics.Determinants.submatrixRemove A 0 (Fin.last (m + 1))).det;
        (AlgebraicCombinatorics.Determinants.submatrixRemove A (Fin.last (m + 1)) 0).det,
          (AlgebraicCombinatorics.Determinants.submatrixRemove A (Fin.last (m + 1)) (Fin.last (m + 1))).det].det

Docstring: Alternative formulation of Desnanot-Jacobi using a 2×2 determinant:
det(A) · det(A') = det [[det(A_{~1,~1}), det(A_{~1,~n})], [det(A_{~n,~1}), det(A_{~n,~n})]]
Label: thm.det.des-jac-1 

## TARGET AlgebraicCombinatorics.Determinants.desnanot_jacobi (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {m : ℕ} (A : Matrix (Fin (m + 2)) (Fin (m + 2)) R),
  A.det * (AlgebraicCombinatorics.Determinants.innerSubmatrix A).det =
    (AlgebraicCombinatorics.Determinants.submatrixRemove A 0 0).det *
        (AlgebraicCombinatorics.Determinants.submatrixRemove A (Fin.last (m + 1)) (Fin.last (m + 1))).det -
      (AlgebraicCombinatorics.Determinants.submatrixRemove A 0 (Fin.last (m + 1))).det *
        (AlgebraicCombinatorics.Determinants.submatrixRemove A (Fin.last (m + 1)) 0).det

Docstring: Desnanot-Jacobi identity (Theorem thm.det.des-jac-1):
...
\begin{align*}  \det A \cdot \det (A’) & = \det (A_{\sim 1, \sim 1}) \cdot \det (A_{\sim n, \sim n}) - \det (A_{\sim 1, \sim n}) \cdot \det (A_{\sim n, \sim 1}) \\ & = \det \begin{pmatrix}  \det (A_{\sim 1, \sim 1}) 
...
{
  "verdict": "faithful",
  "justification": "The blueprint quantifies over \u201cn \u2208 \u2115 \u2026 n \u2265 2\u201d and an \u201cn \u00d7 n-matrix\u201d over the commutative coefficient ring K. Both targets quantify as `\u2200 {R : Type u_1} [CommRing R] {m : \u2115} (A : Matrix (Fin (m + 2)) (Fin (m + 2)) R)`, which represents exactly all natural sizes n \u2265 2 by n = m + 2, with no contentful added hypothesis. The definition `innerSubmatrix A := A.submatrix (fun i => i.succ.castSucc) (fun j => j.succ.castSucc)` selects indices 1,\u2026,m in zero-based `Fin (m+2)`, exactly removing the first and last rows and columns as required for A\u2032. Likewise, `submatrixRemove A i j := A.submatrix i.succAbove j.succAbove` implements the blueprint\u2019s \u201cremoving its i-th row and its j-th column\u201d; `0` and `Fin.last (m + 1)` are respectively the first and last indices. The theorem `desnanot_jacobi` states precisely `det A \u00b7 det A\u2032 = det(A_{~1,~1})\u00b7det(A_{~n,~n}) - det(A_{~1,~n})\u00b7det(A_{~n,~1})`, while `desnanot_jacobi_det2` states that the same left-hand side equals the determinant of the displayed 2\u00d72 matrix with those four minors in the correct positions. Taken together, they exactly formalize the two equalities in the informal statement."
}