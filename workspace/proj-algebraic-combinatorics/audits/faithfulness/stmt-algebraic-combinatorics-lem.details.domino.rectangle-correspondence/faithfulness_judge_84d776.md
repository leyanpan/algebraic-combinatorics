## TARGET DominoBridge.rectangle_correspondence (theorem) — ELABORATED SIGNATURE
∀ (n m : ℕ), DominoBridge.cellToZ '' ↑(DominoTilings.Rectangle n m) = DominoTilingsZ.Rectangle n m

Docstring: The image of DominoTilings.Rectangle under cellToZ equals DominoTilingsZ.Rectangle 
as sets of ℤ × ℤ. 

## PROJECT DEPENDENCY DominoBridge.cellToZ (def)
ℕ × ℕ → ℤ × ℤ

Body:
fun c => (↑c.1, ↑c.2)

Docstring: Cast a cell from ℕ × ℕ to ℤ × ℤ. 
...
## INFORMAL STATEMENT
lem.details.domino.rectangle-correspondence

\leanhelper  The $\mathbb {N}$-based rectangle $\{ (i,j) \mid 1 \le i \le n, 1 \le j \le m\} $ and the $\mathbb {Z}$-based rectangle $\{ (i,j) \in \mathbb {Z}^2 \mid 1 \le i \le n, 1 \le j \le m\} $ are in bijection via the coordinate casting map.
...
{
  "verdict": "faithful",
  "justification": "The blueprint says the two rectangles \u201care in bijection via the coordinate casting map.\u201d The target states for all natural numbers, `\u2200 (n m : \u2115), DominoBridge.cellToZ '' \u2191(DominoTilings.Rectangle n m) = DominoTilingsZ.Rectangle n m`. By the dependency bodies, `DominoTilings.Rectangle n m` is exactly the 1-indexed natural-coordinate rectangle, `DominoTilingsZ.Rectangle n m` is exactly `{p | 1 \u2264 p.1 \u2227 p.1 \u2264 \u2191n \u2227 1 \u2264 p.2 \u2227 p.2 \u2264 \u2191m}`, and `cellToZ` casts both coordinates. The image equality gives surjectivity onto the integer rectangle, while coordinatewise casting `\u2115 \u00d7 \u2115 \u2192 \u2124 \u00d7 \u2124` is injective, so its restriction is a bijection. There are no added hypotheses or narrowed quantifiers."
}