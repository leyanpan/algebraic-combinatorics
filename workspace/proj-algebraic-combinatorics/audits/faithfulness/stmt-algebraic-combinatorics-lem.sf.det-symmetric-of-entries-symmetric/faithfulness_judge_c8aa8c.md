## TARGET SymmetricFunctions.det_isSymmetric_of_entries_symmetric (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} {R : Type u_1} [inst : CommRing R] (M : Matrix (Fin N) (Fin N) (MvPolynomial (Fin N) R)),
  (∀ (i j : Fin N), (M i j).IsSymmetric) → M.det.IsSymmetric

Docstring: If all entries of a matrix are symmetric polynomials, then the determinant is symmetric.
...
## INFORMAL STATEMENT
lem.sf.det-symmetric-of-entries-symmetric

\leanhelper  If all entries of a matrix $M$ over $R[x_1, \ldots , x_N]$ are symmetric polynomials, then $\det (M)$ is a symmetric polynomial. This is because for any permutation $\sigma $, $\sigma (\det (M)) = \det (\sigma (M)) = \det (M)$, where the first equality uses that $\sigma $ is a ring homomorphism and the second uses that $\sigma $ fixes each entry.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies over exactly the relevant setting: `\u2200 {N : \u2115} {R : Type u_1} [CommRing R] (M : Matrix (Fin N) (Fin N) (MvPolynomial (Fin N) R))`, matching a square matrix over the polynomial ring `R[x\u2081, \u2026, x_N]` with a commutative coefficient ring. Its hypothesis `(\u2200 (i j : Fin N), (M i j).IsSymmetric)` is precisely \u201call entries ... are symmetric polynomials,\u201d since `MvPolynomial.IsSymmetric` means invariance under permutations of variables. Its conclusion `M.det.IsSymmetric` is exactly \u201cthen det(M) is a symmetric polynomial.\u201d The use of `Fin N` is the standard finite indexing encoding of the blueprint\u2019s `[N]`, and the determinant\u2019s `DecidableEq` and `Fintype` requirements are automatically supplied for `Fin N`, adding no mathematical hypothesis."
}