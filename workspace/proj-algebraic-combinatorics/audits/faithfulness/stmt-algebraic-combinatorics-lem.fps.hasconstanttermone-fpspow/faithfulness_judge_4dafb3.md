## TARGET AlgebraicCombinatorics.FPS.hasConstantTermOne_fpsPow (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] {f : PowerSeries K},
  AlgebraicCombinatorics.FPS.HasConstantTermOne f →
    ∀ (c : K), AlgebraicCombinatorics.FPS.HasConstantTermOne (AlgebraicCombinatorics.FPS.fpsPow f c)

Docstring: f^c has constant term 1 when f has constant term 1 
...
## INFORMAL STATEMENT
lem.fps.hasConstantTermOne-fpsPow

\leanhelper  For $f\in K[[x]]_1$ and $c\in K$, we have $f^c\in K[[x]]_1$.
...
{
  "verdict": "faithful",
  "justification": "The blueprint assumes throughout that \u201cK is \u2026 a commutative \u211a-algebra,\u201d matching the formal binders `{K : Type u_1} [CommRing K] [Algebra \u211a K]`. Its premise \u201cFor f \u2208 K[[x]]\u2081\u201d matches `HasConstantTermOne f`, whose body is exactly `PowerSeries.constantCoeff f = 1`. The quantifier \u201cc \u2208 K\u201d matches `\u2200 (c : K)`. Finally, `fpsPow f c` is defined as `fpsExp (c \u2022 fpsLog f)`, corresponding to the blueprint definition `f^c := Exp(c Log f)`, and the conclusion `HasConstantTermOne (fpsPow f c)` is exactly `f^c \u2208 K[[x]]\u2081`. Thus the formal declaration states the same claim under the same algebraic setting."
}