## TARGET Nat.Partition.addOneToEach (def) — ELABORATED SIGNATURE
{n k : ℕ} → (p : n.Partition) → p.parts.card = k → (n + k).Partition

Body:
fun {n k} p hk => Nat.Partition.ofSums (n + k) (Multiset.map (fun x => x + 1) p.parts) ⋯

Docstring: Add 1 to each part of a partition to get a partition of n + k. 

## TARGET Nat.Partition.subtractOneFromEach (def) — ELABORATED SIGNATURE
{n k : ℕ} → (p : n.Partition) → p.parts.card = k → (∀ x ∈ p.parts, x > 1) → (n - k).Partition

Body:
fun {n k} p hk h => Nat.Partition.ofSums (n - k) (Multiset.map (fun x => x - 1) p.parts) ⋯

Docstring: Subtract 1 from each part of a partition (when all parts > 1) to get a partition of n - k. 
...
## INFORMAL STATEMENT
def.pars.subtractOneFromEach-addOneToEach

\leanhelper  Given a partition of $n$ into $k$ parts with all parts $>1$, subtracting~ $1$ from each part yields a partition of $n-k$ into $k$ parts. Conversely, adding~ $1$ to each part of a partition of $n$ into $k$ parts produces a partition of $n+k$ into $k$ parts.
...
{
  "verdict": "faithful",
  "justification": "The two definitions implement exactly the two constructions in the blueprint. For subtraction, the binders `(p : n.Partition) \u2192 p.parts.card = k \u2192 (\u2200 x \u2208 p.parts, x > 1) \u2192 (n - k).Partition` match \u201ca partition of n into k parts with all parts > 1,\u201d and the body maps every part by `fun x => x - 1`. Since every part is greater than 1, no mapped part is zero, so `Nat.Partition.ofSums`, which only removes zeros, preserves the `k` parts. For addition, `(p : n.Partition) \u2192 p.parts.card = k \u2192 (n + k).Partition` and `Multiset.map (fun x => x + 1) p.parts` match \u201cadding 1 to each part of a partition of n into k parts\u201d; mapped parts remain positive and retain cardinality `k`. Natural-number subtraction is not a mismatch here because the hypothesis that all `k` positive parts exceed 1 ensures the relevant total satisfies `n \u2265 k`, so saturation at zero does not alter `n-k`."
}