## TARGET Equiv.Perm.sigma_zero_eq_lehmerEntry (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (σ : Equiv.Perm (Fin n)) (hn : n > 0), ↑(σ ⟨0, hn⟩) = σ.lehmerEntry ⟨0, hn⟩

Docstring: Key characterization: σ(0) equals the Lehmer entry at position 0.
...
## INFORMAL STATEMENT
lem.perm.sigma_zero_eq_lehmerEntry

\leanhelper  For $\sigma \in S_n$ with $n > 0$, we have $\sigma (0) = \ell _0(\sigma )$. This is because $\ell _0(\sigma )$ counts how many elements among $\sigma (1), \ldots , \sigma (n-1)$ are less than $\sigma (0)$, and since $\sigma $ is a bijection onto $\{ 0, \ldots , n-1\} $, this count equals $\sigma (0)$.
...
{
  "verdict": "faithful",
  "justification": "The formal binders `\u2200 {n : \u2115} (\u03c3 : Equiv.Perm (Fin n)) (hn : n > 0)` exactly encode \u201cFor \u03c3 \u2208 S_n with n > 0\u201d under the stated `Fin n = {0, \u2026, n-1}` convention. The conclusion `\u2191(\u03c3 \u27e80, hn\u27e9) = \u03c3.lehmerEntry \u27e80, hn\u27e9` is precisely `\u03c3(0) = \u2113\u2080(\u03c3)`: the coercion `\u2191` takes the value of `\u03c3(0) : Fin n` to its natural-number value, while `lehmerEntry` is defined as `{j | i < j \u2227 \u03c3 i > \u03c3 j}.card`, matching the informal Lehmer-entry definition. The hypothesis `hn` is not added drift; it is explicitly required informally and supplies the proof that `0 : Fin n` exists."
}