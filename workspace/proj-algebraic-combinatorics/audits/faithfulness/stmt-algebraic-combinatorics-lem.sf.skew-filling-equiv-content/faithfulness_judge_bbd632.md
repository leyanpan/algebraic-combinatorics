## TARGET skewFillingEquiv_content (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} [inst : NeZero N] (lam mu : NPartition N) (f : SkewFilling lam mu) (i : Fin N),
  fillingContent f i = AlgebraicCombinatorics.contentTableau ((skewFillingEquiv lam mu) f) i

Docstring: Content bridge: the content of a filling equals the content of the corresponding tableau
...
## PROJECT DEPENDENCY fillingContent (def)
{N : ℕ} → [inst : NeZero N] → {lam mu : NPartition N} → SkewFilling lam mu → Fin N → ℕ

Body:
fun {N} [NeZero N] {lam mu} f i => {c | f c = i}.card

Docstring: The content of a filling: the number of cells with each entry value.
...
## INFORMAL STATEMENT
lem.sf.skew-filling-equiv-content

\leanhelper  The filling bijection preserves content: for any filling $f$ and index $i$, the content of $f$ at $i$ equals the content of the corresponding filling in the 1-indexed representation at $i$.
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.sf.skew-filling-equiv
def.sf.skew-filling-equiv

\leanhelper  The \emph{filling bijection} converts fillings between the two skew diagram representations by composing with the cell bijection.
...
{
  "verdict": "drift",
  "justification": "The formal theorem is restricted by the binder `\u2200 {N : \u2115} [inst : NeZero N]`, whereas the blueprint says \u201cfor any filling `f` and index `i`\u201d and none of its definitions assumes `N \u2260 0`. Thus the declaration excludes `N = 0`, even though `Fin 0` and an empty diagram are mathematically meaningful and the content assertion is vacuous there. This restriction enters through the dependencies `SkewFilling`, `fillingContent`, `skewYoungDiagram`, `skewCellEquiv`, and `skewFillingEquiv`, all of which carry `[NeZero N]`. Apart from that restriction, the equality faithfully compares the number of cells with entry `i` before and after the stated cell/filling bijection. To make it faithful, remove `[NeZero N]` from those dependencies and the target, using the all-`N` Young-diagram representation where necessary; alternatively, the blueprint would need to state explicitly that `N > 0`."
}