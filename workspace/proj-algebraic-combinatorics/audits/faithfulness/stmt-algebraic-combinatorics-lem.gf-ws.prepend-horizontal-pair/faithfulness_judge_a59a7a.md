## TARGET DominoTilingsZ.prependHorizontalPair (def) — ELABORATED SIGNATURE
(m : ℕ) →
  DominoTilingsZ.Tiling (DominoTilingsZ.Rectangle m 2) → DominoTilingsZ.Tiling (DominoTilingsZ.Rectangle (m + 2) 2)

Body:
fun m T =>
  {
    dominos :=
      {DominoTilingsZ.Domino.horizontal 1 1, DominoTilingsZ.Domino.horizontal 1 2} ∪
        (fun d => d.shiftNat 2) '' T.dominos,
    pairwise_disjoint := ⋯, cover := ⋯ }

Docstring: Prepend a pair of horizontal dominos to a tiling of Rectangle m 2, giving a tiling of Rectangle (m+2) 2.
...
## INFORMAL STATEMENT
lem.gf-ws.prepend-horizontal-pair

\leanhelper  Given a tiling $T$ of $R_{m,2}$, prepending horizontal dominos $\{ (1,1),(2,1)\} $ and $\{ (1,2),(2,2)\} $ and shifting all of $T$’s dominos right by $2$ gives a valid tiling of $R_{m+2,2}$.
...
{
  "verdict": "faithful",
  "justification": "The target quantifies exactly over \u201cGiven a tiling T of R_{m,2}\u201d via `(m : \u2115) \u2192 Tiling (Rectangle m 2)` and returns a tiling of the required rectangle via `Tiling (Rectangle (m + 2) 2)`. Its body gives the asserted domino set exactly: `{horizontal 1 1, horizontal 1 2} \u222a (fun d => d.shiftNat 2) '' T.dominos`. By `Domino.toShape`, these two constructors cover `{(1,1),(2,1)}` and `{(1,2),(2,2)}`; by `shiftNat` and `shift`, every old domino is shifted horizontally right by 2. Finally, constructing `Tiling` includes proofs of pairwise disjointness and that the union equals the rectangle, so \u201cgives a valid tiling\u201d is fully represented. There are no added hypotheses or narrowed quantifiers."
}