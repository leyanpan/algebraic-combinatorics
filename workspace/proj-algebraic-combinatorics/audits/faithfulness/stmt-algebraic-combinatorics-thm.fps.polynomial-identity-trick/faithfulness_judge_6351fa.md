## TARGET AlgebraicCombinatorics.FPS.polynomial_identity_trick (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_2} [inst : CommRing R] [IsDomain R] [CharZero R] (p : Polynomial R),
  (∀ (n : ℕ), Polynomial.eval (↑n) p = 0) → p = 0

Docstring: The polynomial identity trick: If a polynomial has infinitely many roots, it is zero.
...
## INFORMAL STATEMENT
thm.fps.polynomial-identity-trick

\leanhelper  Let $R$ be a commutative domain of characteristic~ $0$, and let $p\in R[x]$ be a polynomial such that $p(n)=0$ for all $n\in \mathbb {N}$. Then $p=0$.
...
{
  "verdict": "faithful",
  "justification": "The blueprint assumes \u201ca commutative domain of characteristic 0,\u201d matched by the binders `[CommRing R] [IsDomain R] [CharZero R]`; the base-library documentation explicitly says `[CommRing R] [IsDomain R]` gives an integral domain, while `CharZero R` makes the natural-number map injective. The blueprint\u2019s `p \u2208 R[x]` is represented by `(p : Polynomial R)`. Its hypothesis \u201c`p(n)=0` for all `n\u2208\u2115`\u201d is exactly `(\u2200 (n : \u2115), Polynomial.eval (\u2191n) p = 0)`, using the canonical natural-number cast into `R`, and the conclusion is identically `p = 0`."
}