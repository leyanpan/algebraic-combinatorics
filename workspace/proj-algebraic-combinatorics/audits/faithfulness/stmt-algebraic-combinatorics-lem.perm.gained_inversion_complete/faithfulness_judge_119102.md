## TARGET Equiv.Perm.gained_inversion_complete (theorem) — ELABORATED SIGNATURE
∀ {n : ℕ} (σ : Equiv.Perm (Fin n)) (i j a b : Fin n),
  i < j →
    σ j < σ i →
      (a, b) ∈ (σ * Equiv.swap i j).inversions \ σ.inversions → a ∈ σ.setA i j ∧ b = i ∨ a = j ∧ b ∈ σ.setB i j

Docstring: Completeness: every gained inversion is one of the two types. 
...
## PROJECT DEPENDENCY Equiv.Perm.setA (def)
{n : ℕ} → Equiv.Perm (Fin n) → Fin n → Fin n → Finset (Fin n)

Body:
fun {n} σ i j => {a | a < i ∧ σ j < σ a ∧ σ a < σ i}

Docstring: The set A: elements a < i with σ(j) < σ(a) < σ(i).
These contribute to both lost and gained inversions (paired). 

## PROJECT DEPENDENCY Equiv.Perm.setB (def)
{n : ℕ} → Equiv.Perm (Fin n) → Fin n → Fin n → Finset (Fin n)

Body:
fun {n} σ i j => {b | j < b ∧ σ j < σ b ∧ σ b < σ i}

Docstring: The set B: elements b > j with σ(j) < σ(b) < σ(i).
These contribute to both lost and gained inversions (paired). 
...
## INFORMAL STATEMENT
lem.perm.gained_inversion_complete

\leanhelper  Completeness of the gained inversion classification: every gained inversion $(a,b) \in \operatorname {inv}(\sigma \cdot t_{i,j}) \setminus \operatorname {inv}(\sigma )$ (with $\sigma (j) < \sigma (i)$) is one of two types: 

\begin{enumerate} \item $a \in A$ and $b = i$, 
...
## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.perm.seta
def.perm.setA

\leanhelper  The set $A$ from the transposition length proof: $A = \{ a < i \mid \sigma (j) < \sigma (a) < \sigma (i)\} $. These elements produce paired lost and gained inversions: $(a, j)$ is lost and $(a, i)$ is gained.

## INFORMAL DEFINITION stmt-algebraic-combinatorics-def.perm.setb
def.perm.setB

\leanhelper  The set $B$ from the transposition length proof: $B = \{ b > j \mid \sigma (j) < \sigma (b) < \sigma (i)\} $. These elements produce paired lost and gained inversions: $(i, b)$ is lost and $(j, b)$ is gained.
...
{
  "verdict": "drift",
  "justification": "The target adds the contentful hypothesis `i < j` in `\u2200 ... (i j a b : Fin n), i < j \u2192 \u03c3 j < \u03c3 i \u2192 ...`. The informal statement specifies only that the gained inversion is considered \u201cwith `\u03c3(j) < \u03c3(i)`\u201d; its definition of `t_{i,j}` requires merely that `i` and `j` be distinct, not that `i < j`. Thus the Lean theorem covers only one ordering of the transposition indices and is weaker than the blueprint statement. This restriction enters directly through the target binder `i < j \u2192`. To be faithful, either the informal statement must explicitly assume `i < j`, or the formal declaration must remove that hypothesis and formulate/prove the corresponding classification for all distinct `i,j` (with the classification adjusted if necessary for the opposite ordering). The use of `Fin n` rather than `[n] = {1,\u2026,n}` is an order-preserving finite-index encoding and is not itself drift."
}