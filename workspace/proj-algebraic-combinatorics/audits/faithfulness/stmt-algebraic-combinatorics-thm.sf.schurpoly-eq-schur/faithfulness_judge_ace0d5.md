## TARGET SSYTEquiv.schurPoly_eq_schur (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} [inst : NeZero N] (lam : NPartition N),
  schurPoly lam = SymmetricFunctions.schur (SSYTEquiv.schurBasicsNPartition_to_SF lam)

Docstring: Corollary: The two Schur polynomial definitions are equal up to NPartition conversion.
...
## PROJECT DEPENDENCY SSYTEquiv.schurBasicsNPartition_to_SF (def)
{N : ℕ} → NPartition N → SymmetricFunctions.NPartition N

Body:
fun {N} lam => { parts := lam.parts, weaklyDecreasing := ⋯ }

Docstring: Convert from SchurBasics.NPartition to SymmetricFunctions.NPartition.
...
## PROJECT DEPENDENCY NPartition.monotone (theorem)
∀ {N : ℕ} (μ : NPartition N) (i j : Fin N), i ≤ j → μ.parts j ≤ μ.parts i

Docstring: Alias for `antitone` to match SchurBasics.lean naming convention.
...
## INFORMAL STATEMENT
thm.sf.schurPoly-eq-schur

\leanhelper  Corollary of Theorem~ \ref{thm.sf.schur-eq-schurPoly-int}: the filling-based Schur polynomial equals the bounded-representation Schur polynomial (with appropriate conversion of the partition).
...
{
  "verdict": "drift",
  "justification": "The elaborated target quantifies as `\u2200 {N : \u2115} [inst : NeZero N] (lam : NPartition N), ...`, so it only states the equality when `N \u2260 0`. The blueprint says simply that \u201cthe filling-based Schur polynomial equals the bounded-representation Schur polynomial\u201d for an `N`-partition, while `def.sf.Npar` defines an `N`-partition without requiring `N > 0`. Thus `[NeZero N]` narrows the intended range. This restriction enters through the dependencies `schurPoly`, `Filling`, `ssytFillingsYoung`, and `fillingMonomialYoung`, each of which carries `[NeZero N]`; it is not needed by the bounded-representation definition `SymmetricFunctions.schur`. To make the formalization faithful, remove the `NeZero N` requirement from the filling-based definitions and from the target, or explicitly add the assumption `N \u2260 0` to the blueprint statement. Apart from this restriction, the equality and the conversion `SSYTEquiv.schurBasicsNPartition_to_SF lam` match the stated comparison."
}