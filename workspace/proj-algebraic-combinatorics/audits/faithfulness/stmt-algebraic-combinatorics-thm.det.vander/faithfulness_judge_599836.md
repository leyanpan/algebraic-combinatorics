## TARGET AlgebraicCombinatorics.Determinants.vandermonde_det_b (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (a : Fin n → R),
  (AlgebraicCombinatorics.Determinants.vandermondeMat a).transpose.det = ∏ i, ∏ j ∈ Finset.Ioi i, (a i - a j)

Docstring: Vandermonde determinant, part (b): det(aⱼⁿ⁻ⁱ) = ∏_{i<j} (aᵢ - aⱼ)
This is the transpose of part (a).
Label: thm.det.vander (b) 

## TARGET AlgebraicCombinatorics.Determinants.vandermonde_det_a (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (a : Fin n → R),
  (AlgebraicCombinatorics.Determinants.vandermondeMat a).det = ∏ i, ∏ j ∈ Finset.Ioi i, (a i - a j)

Docstring: Vandermonde determinant, part (a): det(aᵢⁿ⁻ʲ) = ∏_{i<j} (aᵢ - aⱼ)
...
## TARGET AlgebraicCombinatorics.Determinants.vandermonde_det_c (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (a : Fin n → R),
  (AlgebraicCombinatorics.Determinants.vandermondeMat' a).det = ∏ i, ∏ j ∈ Finset.Iio i, (a i - a j)

Docstring: Vandermonde determinant, part (c): det(aᵢʲ⁻¹) = ∏_{j<i} (aᵢ - aⱼ)
Label: thm.det.vander (c) 

## TARGET AlgebraicCombinatorics.Determinants.vandermonde_det_d (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (a : Fin n → R),
  (AlgebraicCombinatorics.Determinants.vandermondeMat' a).transpose.det = ∏ i, ∏ j ∈ Finset.Iio i, (a i - a j)

Docstring: Vandermonde determinant, part (d): det(aⱼⁱ⁻¹) = ∏_{j<i} (aᵢ - aⱼ)
This is the transpose of part (c).
Label: thm.det.vander (d) 

## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.vandermondeMat (def)
{R : Type u_1} → [CommRing R] → {n : ℕ} → (Fin n → R) → Matrix (Fin n) (Fin n) R

Body:
fun {R} [CommRing R] {n} a => Matrix.of fun i j => a i ^ (n - 1 - ↑j)

Docstring: The Vandermonde matrix with entries `a i ^ (n - 1 - j)` for 0-indexed i, j.
...
Body:
fun {R} [CommRing R] {n} a => Matrix.of fun i j => a i ^ ↑j

Docstring: The standard Vandermonde matrix with entries `a i ^ j`.
...
## BASE-LIBRARY REF Matrix.transpose
{m : Type u_2} → {n : Type u_3} → {α : Type v} → Matrix m n α → Matrix n m α

Docstring: The transpose of a matrix. 
...
## BASE-LIBRARY REF Finset.Iio
{α : Type u_1} → [inst : Preorder α] → [LocallyFiniteOrderBot α] → α → Finset α

Docstring: The finset $(-∞, b)$ of elements `x` such that `x < b`. Basically `Set.Iio b` as a finset. 

## BASE-LIBRARY REF Fin.instLocallyFiniteOrderBot
(n : ℕ) → LocallyFiniteOrderBot (Fin n)
...
\[  \det \left( \left( a_i^{n-j} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = \prod _{1 \leq i < j \leq n} (a_i - a_j).  \]

\textbf{(b)} We have 

\[  \det \left( \left( a_j^{n-i} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = \prod _{1 \leq i < j \leq n} (a_i - a_j).  \]

\textbf{(c)} We have 

\[  \det \left( \left( a_i^{j-1} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = \prod _{1 \leq j < i \leq n} (a_i - a_j).  \]

\textbf{(d)} We have 

\[  \det \left( \left( a_j^{i-1} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) = \prod _{1 \leq j < i \leq n} (a_i - a_j).  \]
...
{
  "verdict": "faithful",
  "justification": "All four declarations match the corresponding blueprint formulas. The shared binders `\u2200 {R : Type u_1} [CommRing R] {n : \u2115} (a : Fin n \u2192 R)` formalize \u201cLet n \u2208 \u2115\u201d and n elements of the commutative ring K, without adding a mathematically substantive restriction. For (a), `vandermondeMat a` has entry `a i ^ (n - 1 - \u2191j)`, which under 0-based `Fin n` indexing is the blueprint entry `a_i^(n-j)` under 1-based indexing; `\u220f i, \u220f j \u2208 Finset.Ioi i, (a i - a j)` is exactly `\u220f_{1 \u2264 i < j \u2264 n} (a_i-a_j)`. For (b), transposition changes that entry to `a j ^ (n - 1 - \u2191i)`, matching `a_j^(n-i)`, while preserving the stated right-hand side. For (c), `vandermondeMat' a` has entry `a i ^ \u2191j`, corresponding to `a_i^(j-1)`, and `\u220f i, \u220f j \u2208 Finset.Iio i, (a i - a j)` is exactly the product over `j<i`. For (d), its transpose has entry `a j ^ \u2191i`, corresponding to `a_j^(i-1)`, with the same `j<i` product. `Matrix.det` is explicitly the Leibniz determinant, agreeing with the informal determinant definition."
}