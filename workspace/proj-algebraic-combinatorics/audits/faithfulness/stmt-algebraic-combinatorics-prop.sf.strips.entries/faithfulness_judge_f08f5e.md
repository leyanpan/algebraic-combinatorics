## TARGET SymmetricFunctions.SkewPartition.verticalStrip_iff_entries (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} (s : SymmetricFunctions.SkewPartition N),
  s.isVerticalStrip ↔ ∀ (i : Fin N), s.inner.parts i ≤ s.outer.parts i ∧ s.outer.parts i ≤ s.inner.parts i + 1

Docstring: Vertical strip characterization (Proposition prop.sf.strips.entries(b))
Label: prop.sf.strips.entries

A skew partition λ/μ is a vertical strip iff
μᵢ ≤ λᵢ ≤ μᵢ + 1 for each i ∈ [N]. 

## TARGET SymmetricFunctions.SkewPartition.horizontalStrip_iff_entries (theorem) — ELABORATED SIGNATURE
∀ {N : ℕ} (s : SymmetricFunctions.SkewPartition N),
  s.isHorizontalStrip ↔
    ∀ (i : Fin N) (hi : ↑i + 1 < N), s.outer.parts i ≥ s.inner.parts i ∧ s.inner.parts i ≥ s.outer.parts ⟨↑i + 1, hi⟩

Docstring: Horizontal strip characterization (Proposition prop.sf.strips.entries(a))
Label: prop.sf.strips.entries
...
## PROJECT DEPENDENCY SymmetricFunctions.SkewPartition.isVerticalStrip (def)
{N : ℕ} → SymmetricFunctions.SkewPartition N → Prop

Body:
fun {N} s => ∀ (i : Fin N), s.outer.parts i ≤ s.inner.parts i + 1

Docstring: A skew partition λ/μ is a vertical strip if no two boxes of Y(λ/μ)
lie in the same row.
...
\textbf{(a)} The skew partition $\lambda /\mu $ is a horizontal strip if and only if we have

\[  \lambda _{1}\geq \mu _{1}\geq \lambda _{2}\geq \mu _{2}\geq \cdots \geq \lambda _{N}\geq \mu _{N}.  \]

\textbf{(b)} The skew partition $\lambda /\mu $ is a vertical strip if and only if we have
...
{
  "verdict": "drift",
  "justification": "Both declarations restrict the proposition to already-contained pairs through the binder `(s : SymmetricFunctions.SkewPartition N)`. The restriction enters in `SymmetricFunctions.SkewPartition.mk ... \u2192 inner \u2264 outer \u2192 SkewPartition N`, where `inner \u2264 outer` means `\u2200 i, inner.parts i \u2264 outer.parts i`. By contrast, the blueprint begins with arbitrary \u201ctwo N-partitions\u201d and defines every pair `(\u03bc, \u03bb)` to be a skew partition; containment is part of being a horizontal or vertical strip, not a prerequisite for quantification. Thus the formal declarations omit all non-contained pairs and are weaker than the blueprint. On the restricted domain, the entry conditions are correct: vertical containment supplies `\u03bc\u1d62 \u2264 \u03bb\u1d62`, while `isVerticalStrip` supplies `\u03bb\u1d62 \u2264 \u03bc\u1d62+1`; horizontally, bundled containment supplies every `\u03bb\u1d62 \u2265 \u03bc\u1d62` (including the final one), and `isHorizontalStrip` supplies `\u03bc\u1d62 \u2265 \u03bb\u1d62\u208a\u2081`. To make both statements faithful, quantify arbitrary `(outer inner : NPartition N)` and use unbundled strip predicates that themselves include `inner \u2264 outer`; alternatively, redefine `SkewPartition` as an unrestricted pair and put containment into `isHorizontalStrip` and `isVerticalStrip`."
}