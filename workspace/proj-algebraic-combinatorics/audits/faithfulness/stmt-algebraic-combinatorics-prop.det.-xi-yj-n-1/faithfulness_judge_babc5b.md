## TARGET AlgebraicCombinatorics.Determinants.det_sum_pow (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {n : ℕ} (x y : Fin n → R),
  (AlgebraicCombinatorics.Determinants.sumPowMat x y).det =
    (↑(∏ k, (n - 1).choose ↑k) * ∏ i, ∏ j ∈ Finset.Ioi i, (x i - x j)) * ∏ i, ∏ j ∈ Finset.Ioi i, (y j - y i)

Docstring: Proposition prop.det.(xi+yj)n-1:
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.Determinants.sumPowMat (def)
{R : Type u_1} → [CommRing R] → {n : ℕ} → (Fin n → R) → (Fin n → R) → Matrix (Fin n) (Fin n) R

Body:
fun {R} [CommRing R] {n} x y => Matrix.of fun i j => (x i + y j) ^ (n - 1)

Docstring: The matrix with entries (xᵢ + yⱼ)ⁿ⁻¹.
Label: prop.det.(xi+yj)n-1 
...
\begin{align*} &  \det \left( \left( (x_i + y_j)^{n-1} \right)_{1 \leq i \leq n,\;  1 \leq j \leq n} \right) \\ & = \left( \prod _{k=0}^{n-1} \binom {n-1}{k} \right) \left( \prod _{1 \leq i < j \leq n} (x_i - x_j) \right) \left( \prod _{1 \leq i < j \leq n} (y_j - y_i) \right). \end{align*}
...
{
  "verdict": "faithful",
  "justification": "The target quantifies `\u2200 {R : Type u_1} [CommRing R] {n : \u2115} (x y : Fin n \u2192 R)`, matching the blueprint\u2019s fixed commutative ring `K`, natural number `n`, and indexed elements `x\u2081,\u2026,x\u2099`, `y\u2081,\u2026,y\u2099`. Its dependency defines `sumPowMat x y` entrywise as `(x i + y j) ^ (n - 1)`, exactly the matrix in the informal determinant. The factor `\u2191(\u220f k, (n - 1).choose \u2191k)` ranges over `k : Fin n`, hence over `0,\u2026,n-1`, and casts the natural-number product of binomial coefficients into `R`. Each `\u220f i, \u220f j \u2208 Finset.Ioi i` ranges over ordered indices with `i < j`, yielding respectively `(x i - x j)` and `(y j - y i)` as required. The use of zero-based `Fin n` rather than one-based `[n]` is only a reindexing, and universal quantification over all commutative rings is at least as general as the blueprint\u2019s fixed commutative ring setting."
}