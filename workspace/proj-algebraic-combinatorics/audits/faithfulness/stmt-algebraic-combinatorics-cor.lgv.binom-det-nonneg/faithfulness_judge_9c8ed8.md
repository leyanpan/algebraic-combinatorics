## TARGET LGV.binom_det_nonneg (theorem) — ELABORATED SIGNATURE
∀ {k : ℕ} (a b : Fin k → ℕ),
  (∀ (i j : Fin k), i ≤ j → a j ≤ a i) →
    (∀ (i j : Fin k), i ≤ j → b j ≤ b i) → 0 ≤ (Matrix.of fun i j => ↑((a i).choose (b j))).det
...
{
  "verdict": "faithful",
  "justification": "The formal binders `\u2200 {k : \u2115} (a b : Fin k \u2192 \u2115)` exactly encode a length-`k` pair of sequences of nonnegative integers; `Fin k` is the same finite ordered index set as the blueprint\u2019s `1 \u2264 i \u2264 k`, up to the order-preserving shift from zero-based to one-based indexing. The assumptions `(\u2200 (i j : Fin k), i \u2264 j \u2192 a j \u2264 a i)` and the analogous condition for `b` are precisely the weakly decreasing conditions `a\u2081 \u2265 \u22ef \u2265 a\u2096` and `b\u2081 \u2265 \u22ef \u2265 b\u2096`. The matrix entry `\u2191((a i).choose (b j))` is the integer cast of the natural binomial coefficient `\\binom{a_i}{b_j}`, and `Matrix.det` is the Leibniz determinant. Thus the conclusion `0 \u2264 (Matrix.of fun i j => \u2191((a i).choose (b j))).det` is exactly the claimed nonnegativity of the binomial-coefficient determinant. The implicit use of `\u2124` supplies the ordered ring in which determinant nonnegativity is stated and adds no mathematical restriction."
}