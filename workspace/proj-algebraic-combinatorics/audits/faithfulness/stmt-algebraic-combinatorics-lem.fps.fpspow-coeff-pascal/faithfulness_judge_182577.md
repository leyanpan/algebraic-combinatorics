## TARGET AlgebraicCombinatorics.FPS.fpsPow_coeff_pascal (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] [inst_1 : Algebra ℚ K] (c : K) (k : ℕ),
  (PowerSeries.coeff (k + 1)) (AlgebraicCombinatorics.FPS.fpsPow (1 + PowerSeries.X) c) =
    (PowerSeries.coeff k) (AlgebraicCombinatorics.FPS.fpsPow (1 + PowerSeries.X) (c - 1)) +
      (PowerSeries.coeff (k + 1)) (AlgebraicCombinatorics.FPS.fpsPow (1 + PowerSeries.X) (c - 1))

Docstring: Pascal identity for fpsPow coefficients: coeff (k+1) ((1+X)^c) = coeff k ((1+X)^(c-1)) + coeff (k+1) ((1+X)^(c-1)) 
...
## INFORMAL STATEMENT
lem.fps.fpsPow-coeff-pascal

\leanhelper  The coefficients of $(1+x)^c$ satisfy Pascal’s recurrence: 
...
{
  "verdict": "faithful",
  "justification": "The elaborated binders `\u2200 {K : Type u_1} [CommRing K] [Algebra \u211a K] (c : K) (k : \u2115)` exactly encode the section convention that `K` is a commutative `\u211a`-algebra, with arbitrary `c \u2208 K` and `k \u2208 \u2115`. The equality uses `PowerSeries.coeff (k + 1)` and `PowerSeries.coeff k`, matching `[x^{k+1}]` and `[x^k]`. Moreover, `fpsPow f c` has body `fpsExp (c \u2022 fpsLog f)`, matching the informal definition `f^c := Exp(c Log f)`, and it is applied to `1 + PowerSeries.X`, matching `(1+x)`. Thus the formal conclusion is precisely `[x^{k+1}](1+x)^c = [x^k](1+x)^{c-1} + [x^{k+1}](1+x)^{c-1}`, with no additional mathematically contentful hypotheses."
}