## TARGET PowerSeries.coeff_prod_range_eq_of_ge (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_1} [inst : CommRing K] {f : ℕ → PowerSeries K} {n N : ℕ},
  (∀ j ≥ N, ∀ k ≤ n, (PowerSeries.coeff k) (f j) = if k = 0 then 1 else 0) →
    ∀ {i : ℕ},
      i ≥ N →
        ∀ {k : ℕ},
          k ≤ n →
            (PowerSeries.coeff k) (∏ j ∈ Finset.range (i + 1), f j) =
              (PowerSeries.coeff k) (∏ j ∈ Finset.range (N + 1), f j)

Docstring: For a multipliable family, once the index exceeds N (where N witnesses the
...
## INFORMAL STATEMENT
lem.fps.lim.coeff-prod-range-eq-of-ge

\leanhelper  For a multipliable family $(f_j)$, if $f_j \equiv 1 \pmod{x^{n+1}}$ for all $j \ge N$, then for all $i \ge N$ and $k \le n$, 

\[  [x^k] \prod _{j=0}^{i} f_j = [x^k] \prod _{j=0}^{N} f_j.  \]

 The proof proceeds by induction on $i$, repeatedly applying Lemma~ \ref{lem.fps.lim.coeff-prod-extend}.
...
{
  "verdict": "faithful",
  "justification": "The formal hypothesis `(\u2200 j \u2265 N, \u2200 k \u2264 n, coeff k (f j) = if k = 0 then 1 else 0)` exactly expands the blueprint condition \u201c`f_j \u2261 1 mod x^{n+1}` for all `j \u2265 N`\u201d: through degree `n`, the constant coefficient is `1` and every positive coefficient is `0`. Likewise, `Finset.range (i + 1)` and `Finset.range (N + 1)` are respectively the index sets `{0,\u2026,i}` and `{0,\u2026,N}`, so the formal coefficient equality for `i \u2265 N` and `k \u2264 n` matches the displayed conclusion. The target does not assume that `f` is multipliable, although the blueprint begins \u201cFor a multipliable family\u201d; this makes the Lean theorem strictly more general, not weaker, and is therefore faithful. The `CommRing K` binder supplies the algebraic setting for the FPS and finite products used by the statement."
}