## TARGET AlgebraicCombinatorics.alternant_sum_split (theorem) — ELABORATED SIGNATURE
∀ {R : Type u_1} [inst : CommRing R] {N : ℕ} {lam mu nu : Fin N → ℕ},
  ∑ T, AlgebraicCombinatorics.alternant (nu + AlgebraicCombinatorics.contentTableau ↑T + AlgebraicCombinatorics.rho N) =
    ∑ T,
...
Docstring: The alternant sum over all semistandard tableaux equals the sum over Yamanouchi tableaux
...
## PROJECT DEPENDENCY AlgebraicCombinatorics.nonYamanouchiTableau_fintype (def)
{N : ℕ} →
  (lam mu nu : Fin N → ℕ) →
    Fintype { T // AlgebraicCombinatorics.IsSemistandard T ∧ ¬AlgebraicCombinatorics.IsYamanouchi nu T }
...
Docstring: The type of non-Yamanouchi semistandard tableaux is finite. 
...
## INFORMAL STATEMENT
Alternant sum splits into Yamanouchi and non-Yamanouchi

\leanhelper  The sum of alternants over all semistandard tableaux equals the sum over Yamanouchi tableaux plus the sum over non-Yamanouchi tableaux: 

\[  \sum _{T \in \mathrm{SSYT}} a_{\nu + \operatorname {cont}(T) + \rho } = \sum _{\substack {T \in \mathrm{SSYT} \\ \nu \text{-Yam}}} a_{\nu + \operatorname {cont}(T) + \rho } + \sum _{\substack {T \in \mathrm{SSYT} \\ \text{not } \nu \text{-Yam}}} a_{\nu + \operatorname {cont}(T) + \rho }.  \]
...
{
  "verdict": "faithful",
  "justification": "The target states exactly the required partition of the finite semistandard-tableau sum. Its three inferred summation types are supplied respectively by `semistandardTableau_fintype`, `yamanouchiTableau_fintype`, and `nonYamanouchiTableau_fintype`, whose types are `{ T // IsSemistandard T }`, `{ T // IsYamanouchi nu T }`, and `{ T // IsSemistandard T \u2227 \u00ac IsYamanouchi nu T }`. Thus the formal equality matches \u201cthe sum of alternants over all semistandard tableaux equals the sum over Yamanouchi tableaux plus the sum over non-Yamanouchi tableaux.\u201d Each summand is precisely `alternant (nu + contentTableau \u2191T + rho N)`, matching `a_{\u03bd + cont(T) + \u03c1}`. Moreover, `IsYamanouchi` has body `IsSemistandard T \u2227 \u2200 j > 0, IsNPartition (nu + contentColGeq T j)`, agreeing with the informal definition. The signature quantifies arbitrary `{lam mu nu : Fin N \u2192 \u2115}` rather than requiring them to be N-partitions (and does not require `\u03bc \u2286 \u03bb`), so it is more general than the blueprint\u2019s partition setting, not weaker. The binder `[inst : CommRing R]` matches the fixed commutative coefficient ring `K`."
}