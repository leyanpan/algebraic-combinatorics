## TARGET PowerSeries.Exp_sum (theorem) — ELABORATED SIGNATURE
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] {I : Type u_3} (g : I → ↑PowerSeries.PowerSeries₀)
  (hg : PowerSeries.SummableFPS₀ g) (hg_mul : optParam (PowerSeries.MultipliableFPS₁ fun i => PowerSeries.Exp (g i)) ⋯),
  PowerSeries.Exp (PowerSeries.summableFPS₀Sum g hg) =
    PowerSeries.multipliableFPS₁Prod (fun i => PowerSeries.Exp (g i)) hg_mul

Docstring: Exp converts infinite sums to infinite products.
...
## PROJECT DEPENDENCY PowerSeries.SummableFPS₀ (def)
{K : Type u_2} → [inst : CommRing K] → {I : Type u_3} → (I → ↑PowerSeries.PowerSeries₀) → Prop

Body:
fun {K} [CommRing K] {I} f => ∀ (n : ℕ), {i | (PowerSeries.coeff n) ↑(f i) ≠ 0}.Finite

Docstring: A family of FPS in `K⟦X⟧₀` is summable if for each coefficient index n,
only finitely many family members have nonzero n-th coefficient.

This is the notion of summability appropriate for `K⟦X⟧₀`. 

## PROJECT DEPENDENCY PowerSeries.MultipliableFPS₁ (def)
{K : Type u_2} → [inst : CommRing K] → {I : Type u_3} → (I → ↑PowerSeries.PowerSeries₁) → Prop

Body:
fun {K} [CommRing K] {I} f => PowerSeries.Multipliable fun i => ↑(f i)

Docstring: A family of FPS in `K⟦X⟧₁` is multipliable if for each coefficient index n,
...
## PROJECT DEPENDENCY PowerSeries.Exp_multipliable_of_summable (theorem)
∀ {K : Type u_2} [inst : CommRing K] [inst_1 : Algebra ℚ K] {I : Type u_3} (g : I → ↑PowerSeries.PowerSeries₀),
  PowerSeries.SummableFPS₀ g → PowerSeries.MultipliableFPS₁ fun i => PowerSeries.Exp (g i)
...
## PROJECT DEPENDENCY PowerSeries.summableFPS₀Sum (def)
{K : Type u_2} →
  [inst : CommRing K] →
    {I : Type u_3} → (f : I → ↑PowerSeries.PowerSeries₀) → PowerSeries.SummableFPS₀ f → ↑PowerSeries.PowerSeries₀

Body:
fun {K} [CommRing K] {I} f _hf => ⟨PowerSeries.mk fun n => ∑ᶠ (i : I), (PowerSeries.coeff n) ↑(f i), ⋯⟩

Docstring: The coefficient-wise sum of a summable family in `K⟦X⟧₀`.
...
## PROJECT DEPENDENCY PowerSeries.multipliableFPS₁Prod (def)
{K : Type u_2} →
  [inst : CommRing K] →
    {I : Type u_3} → (f : I → ↑PowerSeries.PowerSeries₁) → PowerSeries.MultipliableFPS₁ f → ↑PowerSeries.PowerSeries₁

Body:
fun {K} [CommRing K] {I} f hf => ⟨PowerSeries.tprod (fun i => ↑(f i)) hf, ⋯⟩

Docstring: The product of a multipliable family in `K⟦X⟧₁` as an element of `K⟦X⟧₁`. 
...
## PROJECT DEPENDENCY PowerSeries.MultipliableFPS₁.prod_mem (theorem)
∀ {K : Type u_2} [inst : CommRing K] {I : Type u_3} (f : I → ↑PowerSeries.PowerSeries₁)
  (hf : PowerSeries.MultipliableFPS₁ f), PowerSeries.tprod (fun i => ↑(f i)) hf ∈ PowerSeries.PowerSeries₁

Docstring: For a multipliable family in `K⟦X⟧₁`, the product is also in `K⟦X⟧₁`. 
...
{
  "verdict": "faithful",
  "justification": "The blueprint assumes a commutative \u211a-algebra and a summable family in `K[[x]]\u2080`; the target has exactly `[CommRing K] [Algebra \u211a K]`, `g : I \u2192 \u2191PowerSeries.PowerSeries\u2080`, and `hg : PowerSeries.SummableFPS\u2080 g`. The definitions match the blueprint coefficientwise notion of summability and its infinite sum. The conclusion `PowerSeries.Exp (PowerSeries.summableFPS\u2080Sum g hg) = PowerSeries.multipliableFPS\u2081Prod (fun i => PowerSeries.Exp (g i)) hg_mul` is precisely the stated exponential-of-sum equals product-of-exponentials identity. The other blueprint conclusions are encoded by types or derivable without an extra mathematical assumption: `summableFPS\u2080Sum ... : PowerSeries\u2080` states that the sum lies in `K[[x]]\u2080`; `Exp` has codomain `PowerSeries\u2081`; and multipliability follows from `PowerSeries.Exp_multipliable_of_summable`. Although the elaborated signature includes `hg_mul : optParam (PowerSeries.MultipliableFPS\u2081 ...) \u22ef`, its default is already a proof derived from `hg`, so this optional proof argument imposes no additional restriction and merely enables construction of the typed infinite product."
}